...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		ВебСервис = WSСсылки.CRM_YandexSpeller.СоздатьWSПрокси("http://speller.yandex.net/services/spellservice", "SpellService", "SpellServiceSoap12");
	Исключение
	...
	Если РезультатПроверки = Неопределено Тогда
		ТекстОшибки = НСтр("ru='Ошибка проверки правописания в Яндекс.Спеллер!';en='Error of spell check in Yandex. Speller!'");
		ОшибкаПодключения = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	Если ОшибкаПодключения Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + Нстр("ru='Если для интернет-соединения используется прокси-сервер,';en='If for the Internet connection the proxy-server are us,'")
		+ Символы.ПС + Нстр("ru='то заполните его параметры и создайте файл настроек прокси';en='that fill it parameters and create a file of settings of a proxy'");
	...
	ИначеЕсли СписокОшибок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Проверка правописания завершена.';en='Spell check is completed.'") + Символы.ПС + НСтр("ru='Ошибок не найдено!';en='Errors it is not f!'"));
		Отказ = Истина;
	Иначе
		НомерОшибки 		= 0;
		Сдвиг				= 0;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ТекстИзменен Тогда
		...
		ПоказатьВопрос(ОбратныйВызов, НСтр("ru='Сохранить изменения текста?';en='Save changes of the text?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;	
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыВариантыЗамены
...
//
Процедура ВариантыЗаменыПриАктивизацииСтроки(Элемент)
	ПодключитьОбработчикОжидания("ВариантыЗаменыПриАктивизацииСтрокиОбработчик", 0.1, Истина);
КонецПроцедуры // ВариантыЗаменыПриАктивизацииЯчейки()

&НаКлиенте
Процедура ВариантыЗаменыПриАктивизацииСтрокиОбработчик()
	ТД = Элементы.ВариантыЗамены.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		ПроверяемоеСлово = "";
	Иначе		
		ПроверяемоеСлово = ТД.Значение;
	КонецЕсли;	
КонецПроцедуры // ВариантыЗаменыПриАктивизацииЯчейки()
...
Процедура УстановитьТекущийЭлемент()
	ТекущийЭлемент = Элементы.РедактируемыйТекст;
	//Если ВариантыЗамены.Количество() = 0 Тогда
...
Процедура ПерейтиНаСледующееСлово(ВыводитьПредупреждение)
	Если НомерОшибки = (СписокОшибок.Количество() - 1) Тогда
		...
		Если ВыводитьПредупреждение Тогда
			ПоказатьПредупреждение(, НСтр("ru='Проверка правописания завершена!';en='Spell check is completed!'"));
		КонецЕсли;	
	Иначе	
...
Процедура ВыделитьСловоВТексте()
	...
	#Если ВебКлиент Тогда
		ВыделитьСловоВТекстеСервер(ЗакладкаНачала, ЗакладкаКонца);
	#Иначе
...
&НаСервере
Процедура ВыделитьСловоВТекстеСервер(ЗакладкаНачала, ЗакладкаКонца)
	Элементы.РедактируемыйТекст.УстановитьГраницыВыделения(ЗакладкаНачала, ЗакладкаКонца);
КонецПроцедуры
