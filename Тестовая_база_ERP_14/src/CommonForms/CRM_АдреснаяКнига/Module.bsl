...
Процедура СтрокаПоискаПриИзменении(Элемент)
	...
	Если СтрокаПоиска <> "" Тогда
		...
		Иначе
			...
			ТекстВопроса = НСтр("ru='Индекс полнотекстового поиска неактуален. Обновить индекс?';en='Full text search index is out of date. Update index?'");
...
Процедура СтрокаПоискаПриИзмененииЗавершение(Результат, ДополнительныеПараметры) экспорт
	...
	Если ОбновитьИндексПоиска Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Обновление индекса полнотекстового поиска.';en='Update the full-text search index.'"));
	КонецЕсли;
	ТекстОшибки = НайтиКлиентов(ОбновитьИндексПоиска);
...
Процедура ДобавитьПолучателей(ИмяСписка, СтруктураТекущейГруппы)
	...
	ИначеЕсли ИмяСписка = "СписокКлиентов" Или ИмяСписка = "СписокКонтактныхЛиц" Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |ИЗ
		               |	Справочник."+?(ИмяСписка = "СписокКлиентов","Партнеры","КонтактныеЛицаПартнеров")+".КонтактнаяИнформация КАК КонтактнаяИнформация
		               |ГДЕ
