...
Процедура ПриОткрытии(Отказ)
	Элементы.НадписьЛинияПодключения.Заголовок				= НСтр("ru='Линия не выбрана';en='The line are not selected'");
	Элементы.НадписьСостояниеПодключения.Заголовок			= НСтр("ru='Не подключен';en='It is not connected'");
	...
	Если ЗначениеЗаполнено(LocalPhoneNum) Тогда
		...
		Элементы.НадписьСостояниеПодключения.Заголовок			= НСтр("ru='Готов к работе';en='Ready to operation'");
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыЗвонки
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПропущенныеЗвонки
...
Процедура ДобавитьОбновитьЗвонок(СтруктураЗвонка)
	...
	Если СтрокаЗвонка = Неопределено Тогда
		...
		СтрокаЗвонка.ЗаголовокВидЗвонка			= НСтр("ru='Вх./Исх. звонок:';en='Inc./Out. Call:'");	
		СтрокаЗвонка.ЗаголовокНомерТелефона		= НСтр("ru='Номер телефона:';en='Phone number:'");	
		СтрокаЗвонка.ЗаголовокКлиент			= НСтр("ru='Клиент:';en='Number of item item'");	
		СтрокаЗвонка.ЗаголовокКонтактноеЛицо	= НСтр("ru='Контактное лицо:';en='The login and password for connecting to the Internet-based user support services will be removed from the program."
""
"Disable Internet support?'");
	...
	Если ЗначениеЗаполнено(СтруктураЗвонка.Контакт) Тогда
		...
		КонецЕсли;	
	ИначеЕсли ЗначениеЗаполнено(СтруктураЗвонка.МассивЗвонящих) И (СтруктураЗвонка.МассивЗвонящих.Количество() > 1) Тогда
		СтрокаЗвонка.Клиент				= НСтр("ru='Несколько совпадений номера';en='Multiple number matches'");
		СтрокаЗвонка.КонтактноеЛицо		= "";
	Иначе	
		СтрокаЗвонка.Клиент				= "";
		СтрокаЗвонка.КонтактноеЛицо		= "";
	...
	Если СтруктураЗвонка.ВходящийЗвонок Тогда
		...
		Если СтрокаПропущенногоЗвонка = Неопределено Тогда
			...
			СтрокаПропущенногоЗвонка.ЗаголовокВремяЗвонка		= НСтр("ru='Дата/время';en='Date/time'");	
			СтрокаПропущенногоЗвонка.ЗаголовокНомерТелефона		= НСтр("ru='Номер телефона:';en='Phone number:'");	
			СтрокаПропущенногоЗвонка.ЗаголовокКлиент			= НСтр("ru='Клиент:';en='Number of item item'");	
			СтрокаПропущенногоЗвонка.ЗаголовокКонтактноеЛицо	= НСтр("ru='Контактное лицо:';en='The login and password for connecting to the Internet-based user support services will be removed from the program."
""
"Disable Internet support?'");
		КонецЕсли;
...
//
Процедура ВыбратьАдреснаяКнига(РезультатВыбора, ДополнительныеПараметры)
...
//	ДополнительныеПараметры	- Структура	- Структура дополнительных параметров
//
Процедура ВыполнитьПередатьЗвонок(СтруктураКонтакта, ДополнительныеПараметры)
