...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	CRM_Отчеты.ИнициализироватьНастройкиОтчета(НастройкиОтчета);
	...
	ИначеЕсли Параметры.Свойство("Отчет") 
			И ТипЗнч(Параметры.Отчет) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
	...
	//	Элементы.ПредставлениеПериодаКнопка.Шрифт = Новый Шрифт(Элементы.ПредставлениеПериодаКнопка.Шрифт,, 13);
	//	Элементы.ПредставлениеПериодаКнопка.ШрифтЗаголовка = Новый Шрифт(Элементы.ПредставлениеПериодаКнопка.ШрифтЗаголовка,, 13);
	//Иначе
	//	Элементы.ПредставлениеПериодаКнопка.Шрифт = Новый Шрифт(Элементы.ПредставлениеПериодаКнопка.Шрифт,, 9);
	//	Элементы.ПредставлениеПериодаКнопка.ШрифтЗаголовка = Новый Шрифт(Элементы.ПредставлениеПериодаКнопка.ШрифтЗаголовка,, 9);
	//КонецЕсли;
	
	Элементы.ГруппаОтправить.Видимость = ПолучитьФункциональнуюОпцию("CRM_РаботаВЛокальномРежиме");
	Элементы.ГруппаОтправитьЕще.Видимость = Элементы.ГруппаОтправить.Видимость;
...
Процедура ПредставлениеПериодаНажатие(Элемент)
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если НастройкиОтчета.Свойство("ВариантСтандартногоПериода") Тогда
			...
			ОткрытьФорму("ОбщаяФорма.CRM_ВыборСтандартногоПериода", ПараметрыВыбора, Элементы.ПредставлениеПериодаКнопка, , , , Оповещение);
		Иначе
...
Процедура ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыполненноеДействие="РучнаяНастройкаОформления" Тогда
		...
		ОткрытьФорму("ОбщаяФорма.CRM_ФормаОформления", СтруктураОткрытия, ЭтаФорма,,,, Оповещение);
	ИначеЕсли ВыполненноеДействие="СписокБыстрыхНастроекОформления" Тогда
		СтруктураОткрытия = Новый Структура;
		СтруктураОткрытия.Вставить("ТипЗначения", Данные.ТипЗначения);
		ОткрытьФорму("ОбщаяФорма.CRM_ФормаБыстрыхНастроекОформления", СтруктураОткрытия, ЭтаФорма);
...
Процедура ДобавитьОформление(Поле, Оформление)
	...
	CRM_ОтчетыКлиентСервер.ДобавитьУсловноеОформление(Отчет.КомпоновщикНастроек.Настройки, Поле, Оформление);
	ОтметитьНеактуальным();
...
Процедура ДиаграммаСерииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение="<Прочие>" Тогда
		...
		ОткрытьФорму("ОбщаяФорма.CRM_ФормаСпискаПолей", СтруктураПараметров, ЭтаФорма,,,, Оповещение);
...
Процедура СвернутьРазвернутьПанельНастроек(Элемент)
	...
	Элементы.СкрытьПоказатьНастройки.Заголовок = ?(НовоеЗначениеВидимость, НСтр("ru = 'Скрыть настройки отчета'"), НСтр("ru = 'Показать настройки отчета'"));
...
Процедура РаскрытьНаВесьЭкран(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.CRM_ФормаПредпросмотраОтчета", ПараметрыОткрытия, ЭтаФорма);
...
Функция НавигационнаяСсылкаВарианта(НастройкиОтчета)
	...
	Если ТипЗнч(НастройкиОтчета.ВариантСсылка) <> Тип("СправочникСсылка.ВариантыОтчетов") Тогда
		Возврат "";
	КонецЕсли;
...
Процедура МодифицироватьСхему(СхемаКомпоновкиДанных)
	...
	CRM_Отчеты.СтандартизироватьСхему(СхемаКомпоновкиДанных);
	CRM_Отчеты.ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных);
	
	// Запрет непосредственного использования поля ДинамическийПериод 
	Поле = CRM_Отчеты.ПолеСхемы(СхемаКомпоновкиДанных, "ДинамическийПериод");
...
Процедура ОпределитьПараметрыОтчетаПоУмолчанию()
	...
	Для Каждого Итератор Из ТаблицаВариантов(Ложь) Цикл
		...
		CRM_Отчеты.ИнициализироватьНастройкиВарианта(НастройкиВариантов[Итератор.КлючВарианта]);
	КонецЦикла;
	...
	Иначе
		...
		CRM_Отчеты.ИнициализироватьНастройкиВарианта(СтруктураНастроек);
...
Процедура Сформировать()
	...
	МодульОценкаПроизводительностиКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("ОценкаПроизводительностиКлиент");
		...
		Если ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") 
			...
			Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
				...
				Если НЕ ИмяПоля="ДинамическийПериод" И НЕ ИмяПоля="МесяцГода" И НЕ ИмяПоля="ДеньМесяцГода" И НЕ ИмяПоля="НеделяГода" Тогда
					...
					ПолеСхемы = CRM_Отчеты.ПолеСхемы(СхемаКД, Поле.Поле);
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли Стр.Тип=ТипПараметр() Тогда
			...
			Иначе
				ПараметрыВыбора = Неопределено;
			КонецЕсли; 
			CRM_Отчеты.ДобавитьЭлементФильтр(Стр, ЭтаФорма, ПараметрыВыбора, Неопределено);
		ИначеЕсли Стр.Тип=ТипФильтр() Тогда
			...
			Иначе
				ПараметрыВыбора = Неопределено;
			КонецЕсли; 
			CRM_Отчеты.ДобавитьЭлементФильтр(Стр, ЭтаФорма, ПараметрыВыбора, СвязиПараметровВыбора);
		КонецЕсли; 
		Стр.Создан = Истина;
	КонецЦикла;
		...
		Если Стр.ВидЭлемента="Тумблер" И Стр.ТипЗначения.СодержитТип(Тип("Булево")) Тогда
			Для каждого Реквизит Из Стр.Реквизиты Цикл
				Если Стр.ЗначениеПоУмолчанию=Истина Тогда
					ЭтаФорма[Реквизит.Ключ] = Перечисления.CRM_БулевоТриСостояния.Истина;
				ИначеЕсли Стр.ЗначениеПоУмолчанию=Ложь Тогда
					ЭтаФорма[Реквизит.Ключ] = Перечисления.CRM_БулевоТриСостояния.Ложь;
				Иначе
					ЭтаФорма[Реквизит.Ключ] = Перечисления.CRM_БулевоТриСостояния.НеИзменять;
				КонецЕсли; 
			КонецЦикла;
	 ...
	 Если ТипЗнч(ПолеСхемы)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И ПолеСхемы.Роль.НомерПериода>0 Тогда
		 Для каждого Стр Из ПоляНастроек.ПолучитьЭлементы() Цикл
			 ...
			 Если ЗначениеЗаполнено(Стр.Поле) Тогда
				 ПолеГруппировки = CRM_Отчеты.ПолеСхемы(Схема, Стр.Поле);
			 ...
			 Иначе
				 Для каждого ПодСтр Из Стр.ПолучитьЭлементы() Цикл
					 Если НЕ ПодСтр.Тип=ТипРеквизит() Тогда
						 Продолжить;
					 КонецЕсли; 
					 ПолеГруппировки = CRM_Отчеты.ПолеСхемы(Схема, ПодСтр.Поле);
					 Если ТипЗнч(ПолеГруппировки)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И НЕ ПолеГруппировки.Роль.НомерПериода=0 И ПолеГруппировки.Роль.НомерПериода<ПолеСхемы.Роль.НомерПериода Тогда
						 Индекс = ?(Индекс=Неопределено, ПоляНастроек.ПолучитьЭлементы().Индекс(Стр), Мин(Индекс, ПоляНастроек.ПолучитьЭлементы().Индекс(Стр)));
						 Прервать;
					 КонецЕсли;
	...
	Для каждого ДоступноеПоле Из МассивПолей Цикл
		...
		Для каждого Тип Из ДоступноеПоле.ТипЗначения.Типы() Цикл
			Если Тип=Тип("СправочникСсылка.Валюты") ИЛИ 
				 Тип=Тип("СправочникСсылка.СтруктураПредприятия") ИЛИ 
...
Функция ПроверитьВозможностьДобавленияГруппировки(Стр, СтрГруппа)
	...
	ПолеСхемы = CRM_Отчеты.ПолеСхемы(Схема, Стр.Поле);
	Если ТипЗнч(ПолеСхемы)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И ПолеСхемы.Роль.НомерПериода>0 Тогда
		Для каждого ПодСтр Из СтрГруппа.ПолучитьЭлементы() Цикл
			ВложенноеПоле = CRM_Отчеты.ПолеСхемы(Схема, ПодСтр.Поле);
...
Процедура ДобавитьОтступ(ИмяРеквизита, Группа, Первый = Истина)
	...
	Если КартинкаОтступ = Неопределено Тогда
		...
		Иначе
			КартинкаОтступ.Картинка = БиблиотекаКартинок.КоннекторВерхНиз;
		КонецЕсли;
	Иначе
		Если Первый Тогда
			КартинкаОтступ.Картинка = БиблиотекаКартинок.КоннекторВерхПравоНиз;
...
Процедура ДобавитьКоманды(Стр, ИмяРеквизита, Группа)
	...
	Кнопка.РастягиватьПоГоризонтали = Ложь;
	Кнопка.РастягиватьПоВертикали = Ложь;
	Кнопка.Картинка = БиблиотекаКартинок.CRM_ПереместитьВверх;
	...
	Кнопка.РастягиватьПоГоризонтали = Ложь;
	Кнопка.РастягиватьПоВертикали = Ложь;
	Кнопка.Картинка = БиблиотекаКартинок.CRM_ПереместитьВниз;
...
Функция ПроверитьСовместимостьДиаграммы(ИмяПоляПроверки = "")
	...
	Если ЗначениеЗаполнено(ДиаграммаТочки) Тогда
		ПолеТочка = CRM_Отчеты.ПолеСхемы(Схема, ДиаграммаТочки);
		...
		Если ЗначениеЗаполнено(ДиаграммаСерии) Тогда
			ПолеСерия = CRM_Отчеты.ПолеСхемы(Схема, ДиаграммаСерии);
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Булево")) Тогда
		...
		Значения.Добавить(Перечисления.CRM_БулевоТриСостояния.НеИзменять, НСтр("ru = 'Все'"), Значение=Неопределено);
...
Процедура ОбновитьОтображениеОтметокФильтров()
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		ЭлементыЗаголовка.Добавить(Новый ФорматированнаяСтрока(БиблиотекаКартинок.CRM_УдалитьСиняя,,,, Стр.ИмяКнопки));
		...
		//ЭлементОтметка.ЦветФона = ЦветаСтиля.CRM_СерыйДляПодложек;
		ЭлементОтметка.ЦветТекста = ЦветаСтиля.CRM_ОсновнойГолубой;
...
Процедура РассчитатьСуммуЯчеек()
	...
	Если КоличествоВыделенныхЯчеек()>1 Тогда
		...
		Иначе
			Элементы.СтрокаОтбора1.ПодсказкаВвода = НСтр("ru = 'Сумма'")+" "+СуммаВыделенныхЯчеек;
		КонецЕсли; 
	Иначе
		Элементы.СтрокаОтбора1.ПодсказкаВвода = "";
	КонецЕсли;
	...
	Если ПустаяСтрока(Элементы.СтрокаОтбора1.ПодсказкаВвода) Тогда
		Элементы.СтрокаОтбора1.ПодсказкаВвода = НСтр("ru = 'Сумма'");
	КонецЕсли; 
...
Функция CRM_СформироватьРезультатДляСохраненияСервер()
	...
	Если НЕ CRM_ЛицензированиеСервер.ПодсистемаCRMИспользуется() Тогда
		Возврат НСтр("ru='Невозможно сохранить результат отчета. Подсистема 1С:CRM не используется! (см. Персональные настройки пользователя)';en='The result of report can not be saved. Subsystem 1C: CRM is not used! (see Personal settings of the user)'");
	КонецЕсли;
