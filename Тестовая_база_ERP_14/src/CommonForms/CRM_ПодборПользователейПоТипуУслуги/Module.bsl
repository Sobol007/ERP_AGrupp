...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТипУслуги") Тогда
		...
		НастройкаПоТипуУслуги = ПолучитьФункциональнуюОпцию("CRM_ИспользоватьТипыУслуг");
		
		ЗаполнитьМассивПользователейПоТипуУслуги();
	КонецЕсли;
	Элементы.НастройкаПоТипуУслуги.Видимость = ПолучитьФункциональнуюОпцию("CRM_ИспользоватьТипыУслуг");
	
	Если Параметры.Свойство("сфпВиднаКолонкаСтатусов") Тогда
		сфпВиднаКолонкаСтатусов = Параметры.сфпВиднаКолонкаСтатусов;
	КонецЕсли;		
	Если Параметры.Свойство("ПоказыватьНастройкаПоТипуУслуги") И ТипУслуги.Пустая() Тогда
		Элементы.НастройкаПоТипуУслуги.Видимость = Параметры.ПоказыватьНастройкаПоТипуУслуги;
		НастройкаПоТипуУслуги = Параметры.ПоказыватьНастройкаПоТипуУслуги;
	КонецЕсли;		
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Подразделение.ТекущийРуководитель) Тогда
			...
			Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
				РуководительПодразделения = Подразделение.ТекущийРуководитель;
			Иначе
				Запрос = Новый Запрос;
				...
				Запрос.УстановитьПараметр("ФизическоеЛицо", Подразделение.ТекущийРуководитель);
				Выборка = Запрос.Выполнить().Выбрать();
			...
			Если (Выборка.Пользователь = РуководительПодразделения) Тогда
...
Процедура СформироватьТаблицуПодразделений(ТипУслуги = Неопределено)
	...
	КореньДерева.Наименование = Нстр("ru='Подразделения предприятия';en='Departments'");
