...
Процедура Поиск(Команда)
	Если ПустаяСтрока(ТекущийСправочник) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='Не выбран источник поиска!';en='No search source selected!'"),,"ТекущийСправочник");
		Возврат;
	КонецЕсли;
	Если ПустаяСтрока(ПолеПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='Не заполнено значение поиска!';en='The search value is not filled!'"),,"ПолеПоиска");
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьВнутреннийНомер(Ссылка)
	...
	НоваяСтрока.Вид 			= НСтр("ru='Внутренний номер';en='Internal number'");
...
Процедура ВыполнитьПоискНаСервере()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='Не выбран режим поиска!';en='No search mode selected!'"),,"ПереключательВыборПоляПоиска");
		Возврат;
	КонецЕсли;
	Если ПереключательВыборПоляПоиска = "ПоТелефону" Тогда
		Выборка = Запрос.Выполнить().Выбрать();
