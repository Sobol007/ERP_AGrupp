...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Справочник.КонтактныеЛицаПартнеров.ПустаяСсылка) КАК Контакт1,
	|	ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка) КАК Контакт2,
	|	ЗНАЧЕНИЕ(Справочник.CRM_ПотенциальныеКлиенты.ПустаяСсылка) КАК Контакт3";
	...
	Если ЗначениеЗаполнено(ДополнительнаяИнформацияКонтактов) Тогда
		МассивСохраненныхСтрок = сфпОбщегоНазначения.сфпРазложитьСтрокуВМассивПодстрок(ДополнительнаяИнформацияКонтактов, ";");
	КонецЕсли;
