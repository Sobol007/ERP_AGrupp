...
Процедура КомандаСохранитьВФайл(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Данные должны быть записаны. Записать данные?';en='Data should be written down. Write down the data?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да, НСтр("ru='Сохранение настроек в файл';en='Saving of settings in a file'"));
	Иначе
...
Функция ПроверитьСохранитьНаСервере()
	...
	CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияСохранитьНастройки(Дерево, Не ПризнакВключенОтбор);
...
Функция ДобавитьОбъектВДерево(Корень, ТипОбъектаСтрокой, Представление = Неопределено, СохраненноеТекущееДеревоНастроек = Неопределено)
	...
	Если бЕстьРеквизитПартнер Тогда
		Если ДобавленныеРеквизиты = Неопределено Или ДобавленныеРеквизиты.Найти("Партнеры_СегментКлиентов") = Неопределено Тогда
			...
			НоваяСтрока.Представление	= НСтр("ru='Сегмент клиентов';en='Segment of customers'");
		...
		Если ДобавленныеРеквизиты = Неопределено Или ДобавленныеРеквизиты.Найти("Партнеры_ABCКласс") = Неопределено Тогда
			...
			НоваяСтрока.Представление	= НСтр("ru='ABC класс';en='ABC Class'");
		...
		Если ДобавленныеРеквизиты = Неопределено Или ДобавленныеРеквизиты.Найти("Партнеры_XYZКласс") = Неопределено Тогда
			...
			НоваяСтрока.Представление	= НСтр("ru='XYZ класс';en='XYZ class'");
			НоваяСтрока.ЭтоСсылка		= Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДеревоОбъекты(ОграничениеТипов = Неопределено, ОчищатьДерево = Истина, ПолучитьТаблицуНастроекИзВременногоХранилищаПоАдресу = Неопределено)
	...
	ОбъектыВерхнийУровень.Добавить("ДокументСсылка",		НСтр("ru='Документы';en='Documents'"),,			БиблиотекаКартинок.Документ);
	ОбъектыВерхнийУровень.Добавить("ЗадачаСсылка",			НСтр("ru='Задачи';en='Tasks'"),,				БиблиотекаКартинок.Задача);
	ОбъектыВерхнийУровень.Добавить("БизнесПроцессСсылка",	НСтр("ru='Бизнес-процессы';en='Business processes'"),,	БиблиотекаКартинок.БизнесПроцесс);
	ОбъектыВерхнийУровень.Добавить("СправочникСсылка",		НСтр("ru='Справочники';en='Reference manuals'"),,		БиблиотекаКартинок.Справочник);
