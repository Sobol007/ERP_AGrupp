...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокКомандаРазделы = НСтр("ru='Показать разделы';en='Show sections'");
...
Процедура ФормаЗаполненияЗаписать(Команда)
	...
	Если НЕ Отказ Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Изменение';en='modifiedication'"),
		,
...
Функция ПроверитьЗаполнениеАнкеты()
	...
	Пока Выборка.Следующий() Цикл
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан ответ на вопрос';en='Answer to the question is not specified'") + "- "+ СтрЗаменить(Выборка.Ссылка.ПолныйКод(),"/",".") + " " + Выборка.Формулировка);
	КонецЦикла;
...
Процедура УстановитьЗначенияРеквизитовСогласноОпросу(РеквизитыОпрос)
	...
	Вступление                            = ?(ПустаяСтрока(РеквизитыОпрос.Вступление),
	                                         НСтр("ru='Нажмите далее для заполнения анкеты.';en='Click Next to fill in the questionnaire.'"),
	                                         РеквизитыОпрос.Вступление);
	Заключение                            = ?(ПустаяСтрока(РеквизитыОпрос.Заключение),
	                                         НСтр("ru='Спасибо за то что заполнили анкету.';en='Thank you for filling out the questionnaire.'"),
...
Процедура ИзменитьВидимостьДеревоРазделов()
	...
	ЗаголовокКомандаРазделы = ?(Элементы.ГруппаДеревоРазделов.Видимость,НСтр("ru='Скрыть разделы';en='Hide sections'"),НСтр("ru='Показать разделы';en='Show sections'"));
	Элементы.СкрытьПоказатьДеревоРазделов.Заголовок         = ЗаголовокКомандаРазделы;
	Элементы.СкрытьПоказатьДеревоРазделовДокумент.Заголовок = ЗаголовокКомандаРазделы;
