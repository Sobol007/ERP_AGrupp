			...
			Для Каждого ИмяФайлаСтруктура Из МассивФайлов_ODT Цикл
				...
				Иначе
					СписокИменФайлов.Добавить(ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайлаСтруктура.ИмяФайла),
						ДополнительныеПараметры.ПараметрыВыполненияКоманды.Источник.УникальныйИдентификатор), НСтр("ru='Коммерческое предложение';en='Commercial Proposal'") + ".odt");
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Если НастройкиСохранения.ФорматDOC Тогда
		...
		Если МассивФайлов_DOC.Количество() > 0 Тогда
			Для Каждого ИмяФайлаСтруктура Из МассивФайлов_DOC Цикл
				...
				Иначе
					СписокИменФайлов.Добавить(ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайлаСтруктура.ИмяФайла),
						ДополнительныеПараметры.ПараметрыВыполненияКоманды.Источник.УникальныйИдентификатор), НСтр("ru='Коммерческое предложение';en='Commercial Proposal'") + ".doc");
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
#Если Не ВебКлиент Тогда
	Если СписокИменФайлов.Количество() > 0 Тогда
		...
		Если НастройкиСохранения.ЗапаковатьZIP Тогда
			...
			СписокФайловВложений.Добавить(СтруктураВложения.ПутьВоВременномХранилище, НСтр("ru='Коммерческое предложение';en='Commercial Proposal'") + ".zip");
		Иначе
