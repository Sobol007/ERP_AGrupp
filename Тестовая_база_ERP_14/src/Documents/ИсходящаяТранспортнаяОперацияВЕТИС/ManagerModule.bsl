...
Процедура ЗагрузитьСведенияВСДПоРезультатамЗапросаВЕТИС(ДанныеДокумента, ИзмененныеЗаписиЖурнала, ПараметрыОбмена, Предприятие, ХозяйствующийСубъект, ДокументОбъект)
	...
	Для Каждого СтрокаТоваров Из ДокументОбъектТовары Цикл
		СтрокаТоваров.ОсобыеОтметки = УдалитьНезначимыеСимволы(СтрокаТоваров.ОсобыеОтметки);
	КонецЦикла;
	...
	Для Каждого ДанныеВСД Из ЗагруженныеВСД Цикл
		...
		НайденныеСтроки = ДокументОбъектТовары.НайтиСтроки(ПараметрыОтбора);
