	...
	Для Каждого Строка Из НачисленнаяЗарплатаИВзносы Цикл
		...
		Если ЗначениеЗаполнено(Строка.СпособОтраженияЗарплатыВБухучете)
			...
			ТекстСообщения = НСтр("ru = 'Для отражения отпуска за счет резервов требуется способ отражения на статьи расходов, а не на производство в строке %1 списка ""Начисления и взносы""';
									|en = 'To record the leave at the expense of reserves, use the method of recording for expense items, not for production in line %1 of the ""Accruals and contributions"" list'");
