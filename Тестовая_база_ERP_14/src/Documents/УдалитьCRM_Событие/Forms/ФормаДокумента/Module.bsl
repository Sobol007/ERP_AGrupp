...
Процедура НастроитьВидимостьИДоступностьФормы(Форма)
	...
	СтрокаЗначениеНеВыбрано = НСтр("ru='[Не выбрано]';en='[Unspecified]'");
	Заголовок		= ?(ЗначениеЗаполнено(Форма.Объект.Проект), Строка(Форма.Объект.Проект), НСтр("ru='[Проект не задан]';en='[Project not specified]'"));
	...
	Если Форма.Объект.ВидСобытия = ПредопределенноеЗначение("Перечисление.CRM_ВидыСобытий.ТелефонныйЗвонок") Тогда
		Форма.Элементы.Место.Заголовок = НСтр("ru='Телефон';en='Phone'");
		Форма.Элементы.Место.Подсказка = НСтр("ru='Телефон';en='Phone'");
		Форма.Элементы.Место.КнопкаВыбора = Истина;	
		
	Иначе
		Форма.Элементы.Место.Заголовок = НСтр("ru='Место';en='Place'");
		Форма.Элементы.Место.Подсказка = НСтр("ru='Место';en='Place'");
		Форма.Элементы.Место.КнопкаВыбора = Истина;	
	КонецЕсли;
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	//		ТекстНадписи = НСтр("ru='Валюта: %Валюта%, курс: %Курс%';en='Currency: %Currency%, exchange rate: %Exchange rate %'");
	...
	//	Иначе
	//		ТекстНадписи = НСтр("ru='Валюта: <нет>';en='Currency: <n/a>'");
	//	КонецЕсли;
	//КонецЕсли;
	//
	//// Вид цен.
	...
	//		ТекстНадписи = ТекстНадписи + НСтр("ru='Вид цен: %ВидЦен%';en='Price type: %PriceType %'");
	//	Иначе	
	//		ТекстНадписи = ТекстНадписи + НСтр("ru='; Вид цен: %ВидЦен%';en='; Price type: %PriceType%'");
	//	КонецЕсли;	
	...
	//	Если ПустаяСтрока(ТекстНадписи) Тогда
	//		ТекстНадписи = ТекстНадписи + НСтр("ru='Вид скидки: %ВидСкидкиНаценки%';en='Discount type: %ВидСкидкиНаценки %'");
	//	Иначе
	//		ТекстНадписи = ТекстНадписи + НСтр("ru='; Вид скидки: %ВидСкидкиНаценки%';en='; Discount type: %DiscountMark-upType%'");
	//	КонецЕсли;	
	...
	//		ТекстНадписи = НСтр("ru='Сумма включает НДС';en='The total included the VAT'");
	//	Иначе		
	//		ТекстНадписи = НСтр("ru='Сумма не включает НДС';en='The total did not include the VAT'");
	//	КонецЕсли;	
	//КонецЕсли;	
	//
...
Процедура УстановитьЗаголовокТрудозатрат()
	...
	Если ПродолжительностьПериода.ВДнях = 0 Тогда
		Если ЗначениеЗаполнено(Объект.Ответственный) Тогда
			Элементы.ДекорацияТрудозатраты.Заголовок = НСтр("ru='согласно графику работ ответственного (в указанном периоде нет рабочих часов)';en='it agreed to graphics of operations responsible (in the specified period there was no working hours)'");
		Иначе
			Элементы.ДекорацияТрудозатраты.Заголовок = НСтр("ru='согласно графику работ (в указанном периоде нет рабочих часов)';en='it agreed to graphics of operations (in the specified period there was no working hours)'");
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Объект.Ответственный) Тогда
			Элементы.ДекорацияТрудозатраты.Заголовок = НСтр("ru='согласно графику работ ответственного';en='it agreed to graphics of operations of responsible'");
		Иначе
			Элементы.ДекорацияТрудозатраты.Заголовок = НСтр("ru='согласно графику работ';en='it agreed to graphics of operations'");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьКоманднуюПанельАтрибутыСобытия()
	...
	Команда.Заголовок	= НСтр("ru='[Проект не задан]';en='[Project not specified]'");
	Команда.Отображение	= ОтображениеКнопки.КартинкаИТекст;
	Кнопка = Элементы.Добавить("КнопкаПодменюПроект1", Тип("КнопкаФормы"), Элементы.ПодменюПроектСобытия);
	Кнопка.Заголовок	= НСтр("ru='[Проект не задан]';en='[Project not specified]'");
...
Процедура ЗаполнитьГруппуОповещений()
	...
	Ид = 0;
	Для Каждого СтрокаТаб Из ТаблицаОповещений Цикл
		Ид = Ид + 1;
		...
		Элемент.Подсказка					= НСтр("ru='Канал оповещения';en='Notification channel'");
		...
		Элемент.УстановитьДействие("ПриИзменении", "Подключаемый_ОповещениеПриИзменении");
		Элемент.Подсказка					= НСтр("ru='Срок до начала события, за который нужно напомнить';en='The time before the event for which you want to be reminded'");
...
Процедура ДобавитьОповещение()
	...
	Если ТаблицаОповещений.Количество() = 5 Тогда
...
Процедура УдалитьОповещение(ИмяКоманды)
	...
	МассивСтрок = ТаблицаОповещений.НайтиСтроки(Новый Структура("ИмяПоля","ПолеКанала_"+ИдПолей));
	
	Если МассивСтрок.Количество() > 0 Тогда
		ТаблицаОповещений.Удалить(ТаблицаОповещений.Индекс(МассивСтрок[0]));
	КонецЕсли;
...
Процедура ПриИзмененииОповещения(Ид)
	...
	МассивСтрок = ТаблицаОповещений.НайтиСтроки(Новый Структура("ИмяПоля","ПолеКанала_"+Ид));
	Если МассивСтрок.Количество() > 0 Тогда
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.УдалитьCRM_Событие") Тогда
		...
		Если ЗначениеЗаполнено(Объект.ДокументОснование.Результат) Тогда
			Объект.Описание = Объект.Описание + Символы.ПС + Символы.ПС
				+ "-------------------------------"+ НСтр("ru='Результат предыдущего события';en='Result of the previous event'") + " " + Объект.ДокументОснование.Номер+" от "+Объект.ДокументОснование.Дата+ " ------------------------------------" + Символы.ПС
				+ Объект.ДокументОснование.Результат + Символы.ПС + Символы.ПС
				+ НСтр("ru='Ответственный:';en='Responsible:'") + " " + Объект.ДокументОснование.Ответственный
				+ ?(ЗначениеЗаполнено(Объект.ДокументОснование.КонтактноеЛицо),Символы.ПС + НСтр("ru='Контактное лицо:';en='The login and password for connecting to the Internet-based user support services will be removed from the program."
""
"Disable Internet support?'") + " " + Объект.ДокументОснование.КонтактноеЛицо,"");
...
Процедура ПеревестиТрудозатратыВЧасы()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В поле ""Трудозатраты"" введены некорректные данные: число минут должно быть меньше 60!';en='In the field of ""Expense of labor"" malformed data was entered: the number of minutes should be less than 60!'"),, "ТрудозатратыЧасыМинуты");
...
Функция ПроверитьНаКорректность()
	...
	Если Не ЗначениеЗаполнено(Объект.ОкончаниеСобытия) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата окончания должна быть заполнена!';en='The expiry date should be fill!'"),, "ДатаОкончание");
		Возврат Ложь;
	КонецЕсли;
	
	Если Объект.ОкончаниеСобытия < Объект.Дата Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата окончания должна больше даты начала!';en='The expiry date owed more date started!'"),, "ДатаОкончание");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверитьДатыОповещений()
	...
	//		КонецЕсли;
	//		ТекстВопроса = ТекстВопроса + НСтр("ru='Срок оповещения №';en='Period of the notification No.'") + Строка(ИД) + НСтр("ru = ' меньше текущей даты.'");
	//		Всего = Всего + 1;
	//	КонецЕсли;
	//	ИД = ИД + 1;
	//КонецЦикла;
	...
	//	Возврат Ложь;
	//	
	//	ТекстВопроса = ТекстВопроса + Символы.ПС + ?(Всего=1,НСтр("ru='Это оповещение не будет создано.';en='This notification will not be created.'"),НСтр("ru='Эти оповещения не будут созданы.';en='These notifications will not be created.'")) + " " + НСтр("ru='Продолжить?';en='Continue?'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоНовоеСобытие			= Объект.Ссылка.Пустая();
	...
	Если НЕ ЭтоНовоеСобытие И (Объект.Периодическое ИЛИ Объект.ИзмененоВРяде) И НЕ ЗначениеЗаполнено(Объект.РядСобытий) Тогда
	...
	Если	ПолучитьФункциональнуюОпцию("ФиксироватьПервичныйИнтерес")
		...
		//
		НайденнаяСсылка = ПланыВидовХарактеристик.КаналыРекламныхВоздействий.НайтиПоНаименованию(НСтр("ru='Акция';en='Action'"), Истина);
	...
	Если ЭтоНовоеСобытие И ЗначениеЗаполнено(Объект.ДокументОснование) 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЭтоНовоеСобытие И Параметры.ОбновлятьКалендарь = 1 Тогда
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТаблицаНастроекОповещений = ДанныеФормыВЗначение(ТаблицаОповещений, Тип("ТаблицаЗначений"));
...
Процедура ОчиститьТабличнуюЧасть(ИмяТаблицы)
	...
	//	Ответ = Вопрос(НСтр("ru='Табличная часть содержит записи. Продолжить?';en='The tabular part contained records. Continue?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
	//							
...
Процедура ВывестиСписокВведенныхНаОсновании(Команда)
	...
	СтруктураОтбора = Новый Структура("Ссылка",СписокВведенныхНаОсновании);
...
Процедура бзПроверитьИндексППД()
	...
	Если ЭтотОбъект["бзИндексППДАктуален"] Тогда
		...
		КонецЕсли;
	Иначе	
		Элементы["бзДекорацияИндексППНеактуален"].Заголовок = НСтр("ru='Индекс полнотекстового поиска неактуален';en='Full text search index is out of date'");
	КонецЕсли;	
...
Процедура бзОбновлениеИндексаПДД()
	Если НЕ ЭтотОбъект["бзИндексППДАктуален"] Тогда
		...
		бзВыполнитьПП();
	ИначеЕсли Элементы.бзДекорацияИндексППНеактуален.Заголовок = НСтр("ru='Ничего не найдено. Создать новое решение?';en='It is found nothing. Create the new decision?'") Тогда
...
Процедура бзЗаполнитьТекстВопросаHTML()
	...
	Иначе
		ТекстПоляHTML	= ТекстПоляHTML + "<FONT size=1>" + Нстр("ru='Основная категория: '") + "<b>" + НСтр("ru='[не задана]';en='[not specified]'") + "</b></FONT></p><p>";
	КонецЕсли;	
	...
	Иначе
		ТекстПоляHTML	= ТекстПоляHTML + "<FONT size=1>" + Нстр("ru='Проект: '") + "<b>" + НСтр("ru='[не задан]';en='[not specified]'") + "</b></FONT></p><p>"; 
	КонецЕсли;	
	...
	Иначе
		ТекстПоляHTML	= ТекстПоляHTML + "<FONT size=1>" + Нстр("ru='Автор: '") + "<b>" + НСтр("ru='[не задан]';en='[not specified]'") + "</b></FONT></p><br>";
	КонецЕсли;	
	...
	Иначе	
		...
		Для Каждого ТекОтвет Из ЭтотОбъект["бзТаблицаОтветов"] Цикл
			...
			ТекстПоляОтветаHTML = "<p></p><p><b>" + Нстр("ru='Ответ №:';en='Answer No.:'") + Строка(НомерОтвета) + " ";
			...
			Иначе
				ТекстПоляОтветаHTML = ТекстПоляОтветаHTML + "<FONT size=1>" + НСтр("ru='Автор: '") + "<b>" + НСтр("ru='[не задан]';en='[not specified]'") + "</FONT></b></p><br>";
			КонецЕсли;	
	...
	ПозицияОтвета		= Найти(ТекстПоляHTML, НСтр("ru='Ответ №:';en='Answer No.:'"));	
...
СообщениеОНекорректностиВремениОповещения = НСтр("ru='(Время события не корректно, оповещение не будет создано!)';en='(The event time is not correct, the notification will not be created!)'");
