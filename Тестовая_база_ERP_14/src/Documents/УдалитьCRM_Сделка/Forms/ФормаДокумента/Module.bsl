...
Процедура ВвестиОбъект(Команда)
	...
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Задача не принята к исполнению!';en='The task is not accepted for execution!'"));
	//	Возврат;
	//КонецЕсли;
	//
...
Процедура ВыполнитьЗадачуБезУсловия(Команда)
	...
	//ПоказатьОповещениеПользователя(НСтр("ru='Задача выполнена';en='Task completed'"), ПолучитьНавигационнуюСсылку(ТекущаяЗадачаСтруктура.Ссылка), Строка(ТекущаяЗадачаСтруктура.Ссылка));
	//
	//Оповестить("ОбновитьНапоминания", Новый Структура("ОткрыватьАктивизироватьФормуНапоминаний", Ложь), ЭтотОбъект);
	//
	//ЗаписатьРеквизитТекущейЗадачи(ТекущаяЗадачаСсылка, "РезультатВыполнения", РезультатВыполнения);
		...
		Если НЕ ТекущийЭтап = СтрокаОбъекта.ТочкаМаршрута Тогда
			...
			Если МассивСтрок.Количество() > 0 Тогда
				...
				Если МассивСтрок.Количество() > 0 Тогда
					...
					НоваяКомандаФормы.Подсказка					= НСтр("ru='Создать новый документ на основании документа этапа';en='Create a new document based on the stage document'");
					
					НоваяКомандаФормы = Команды.Добавить("Команда_Выбрать"+ТекущийЭтап.Имя);
					НоваяКомандаФормы.Заголовок                 = НСтр("ru='Выбрать';en='Select'");
					НоваяКомандаФормы.Действие                  = "ВыбратьОбъект";
					НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
					НоваяКомандаФормы.Подсказка					= НСтр("ru='Выбрать существующий документ';en='Select the exist document'");
					...
					НоваяКомандаФормы.Действие                  = "ВвестиОбъект";
					НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
					НоваяКомандаФормы.Подсказка					= НСтр("ru='Создать новый документ на основании текущего этапа';en='Create a new document based on the current stage'");
				КонецЕсли;
				...
				Если МассивСтрок.Количество() = 1 Тогда
					...
					НоваяКомандаФормы.Заголовок                 = НСтр("ru='Создать документ';en='Create the document'");
					НоваяКомандаФормы.Действие                  = "ВвестиОбъект";
					НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
					НоваяКомандаФормы.Подсказка					= НСтр("ru='Создать документ %Синоним% на основании документа этапа';en='Create a document% Synonym% based on the stage document'");
					...
					НовыйПунктМеню.Заголовок			= НСтр("ru='Создать документ';en='Create the document'");
					...
					НоваяКомандаФормы.Заголовок                 = НСтр("ru='На основании этапа';en='Based on the step'");
					НоваяКомандаФормы.Действие                  = "ВвестиОбъект";
					НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
					НоваяКомандаФормы.Подсказка					= НСтр("ru='Создать новый документ %Синоним% на основании текущего этапа';en='Create a new document% Synonym% based on the current stage'");
					НоваяКомандаФормы.Подсказка = СтрЗаменить(НоваяКомандаФормы.Подсказка, "%Синоним%", Синоним);		
					...
					НовыйПунктМеню.Заголовок			= НСтр("ru='На основании этапа';en='Based on the step'");
					НовыйПунктМеню.Картинка				= БиблиотекаКартинок.СоздатьЭлементСписка;
					НовыйПунктМеню.Отображение			= ОтображениеКнопки.КартинкаИТекст;
					
					// Выбрать объект
					НоваяКомандаФормы = Команды.Добавить("Команда_Выбрать_"+ТекущийЭтап.Имя+"_"+СтрЗаменить(ПолноеИмя, ".", "_"));
					НоваяКомандаФормы.Заголовок                 = НСтр("ru='Добавить';en='Upload'");
					НоваяКомандаФормы.Действие                  = "ВыбратьОбъект";
					НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
					НоваяКомандаФормы.Подсказка					= НСтр("ru='Выбрать документ %Синоним%';en='Select the document %Synonym%'");
					НоваяКомандаФормы.Подсказка = СтрЗаменить(НоваяКомандаФормы.Подсказка, "%Синоним%", Синоним);
					...
					НовыйПунктМеню.Заголовок			= НСтр("ru='Выбрать документ';en='Select the document'");
					НовыйПунктМеню.Отображение			= ОтображениеКнопки.КартинкаИТекст;
				...
				ИначеЕсли МассивСтрок.Количество() > 1 Тогда
					...
					ПунктМенюВвести.Заголовок	= НСтр("ru='Создать документ';en='Create the document'");
					...
					ПунктМенюДобавить.Заголовок		= НСтр("ru='На основании этапа';en='Based on the step'");
					...
					ПунктМенюВыбрать.Заголовок = НСтр("ru='Выбрать документ';en='Select the document'");
					...
					Для Каждого СтрокаТаб Из МассивСтрок Цикл
						...
						НоваяКомандаФормы.Действие                  = "ВвестиОбъект";
						НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
						НоваяКомандаФормы.Подсказка					= НСтр("ru='Добавить документ %Синоним%';en='Add the document %Synonym%'");
						НоваяКомандаФормы.Подсказка = СтрЗаменить(НоваяКомандаФормы.Подсказка, "%Синоним%", Синоним);			
				...
				НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
				НоваяКомандаФормы.Подсказка					= НСтр("ru='Удалить текущий элемент (Del)';en='Delete the current element (Del)'");
				...
				НовыйПунктМеню.Заголовок			= НСтр("ru='Удалить';en='Delete'");
		...
		Если ЗначениеЗаполнено(пПартнер) Тогда
			...
			Если Интерес <> Неопределено И ЗначениеЗаполнено(Интерес.КаналПервичногоИнтереса) Тогда
				...
				Если ТипЗнч(Интерес.КаналПервичногоИнтереса.ТипЗначения.ПривестиЗначение()) 
						...
						Тогда
					Надпись = НСтр("ru='Ранее зарегистрирован актуальный источник привлечения по каналу %КПИ%; источник %ИПИ%';en='Previously, the actual source of attraction through the channel% KPI%; source% IPI%'");
				...
				Иначе
					Надпись = НСтр("ru='Ранее зарегистрирован актуальный источник привлечения по каналу %КПИ%';en='Previously, the actual source of attracted by channel% KPI%'");
					Надпись = СтрЗаменить(Надпись, "%КПИ%", Интерес.КаналПервичногоИнтереса);
				...
				Если ТипЗнч(Интерес.КаналПервичногоИнтереса.ТипЗначения.ПривестиЗначение()) = Тип("СправочникСсылка.МаркетинговыеМероприятия") Тогда
						Если Не Интерес.Актуальность Тогда
							ДопНадпись = НСтр("ru='Срок действия маркетингового мероприятия истек.';en='Period of validity of marketing action effusing.'");
						Иначе
							ДопНадпись = "";
						КонецЕсли;
...
Процедура ИсполнителиДатаВыполненияПриИзменении(Элемент)
	...
	//	ВызватьИсключение(НСтр("ru='Срок выполнения задачи не может быть раньше даты начала!';en='The deadline for the task can not be earlier than the start date!'"));
	//	Возврат;
	//КонецЕсли;
	//
	...
	//		Ответ = Вопрос(НСтр("ru='Пересчитать сроки для следующих этапов?';en='Recalculate periods for the following stages?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ИсполнителиПередНачаломИзменения(Элемент, Отказ)
	...
	//			ЭлементСписка = Элементы.Исполнители.ТекущиеДанные.ВидыСоздаваемыхДокументов.ВыбратьЭлемент(НСтр("ru='Выберите вид создаваемого документа:';en='Select the type of document you are creating:'"));
...
Процедура КомандаКонтекстногоМенюКалендаряОбщая(Команда)
	...
	Если ИмяКоманды = "КомандаСоздатьКонтрольнуюТочку" Тогда
		ТипСобытия = "Документ.CRM_ЭтапКалендарногоПлана";
	Иначе
