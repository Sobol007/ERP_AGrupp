...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если (Объект.ДатаНачала > Объект.ДатаОкончания) И (Объект.ДатаОкончания <> Дата(1,1,1)) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='Дата начала не может быть больше чем дата окончания.';en='The date started could not be more than an expiry date.'"),,"Объект.ДатаНачала");
		Отказ = Истина;
...
Процедура ЗаполнитьПоСегменту(Команда)
	...
	Если ТипЗнч(Объект.ТипРеспондентов) = Тип("СправочникСсылка.Пользователи") Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для пользователей заполнение по сегменту не используется!';en='For users filling on a segment are not us!'"), 10);
		Возврат
	КонецЕсли;
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ЕстьЗаписи", (Объект.Респонденты.Количество() > 0));
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ТекстВопроса = НСтр("ru='Очистить список респондентов?';en='Clear the list of respondents?'");
	Если Объект.Респонденты.Количество() > 0 Тогда
