...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеСервер.ПодсистемаCRMИспользуется() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно открыть форму отчета о работе. Подсистема 1С:CRM не используется! (см. Персональные настройки пользователя)';en='Unable to open the job report form. Subsystem 1C: CRM is not used! (see Personal settings of the user)'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		// Проверка рабочих дат на выходные дни.
			...
			Если Не ПустаяСтрока(СтрСтроки) Тогда
				...
				ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Рабочие даты попадают на выходные дни. Строки:';en='Working dates got for the weekend. Lines:'")
					+ " " + СтрСтроки + ". "
					+ Символы.ПС +НСтр("ru='Продолжить?';en='Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура КомандаЗаполнитьПоСуществующемуОтчету(Команда)
	Если Объект.СтрокиОтчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоСуществующемуОтчетуЗавершение", ЭтотОбъект), НСтр("ru='Заполнить с очисткой табличной части?';en='Fill with cleaning of a tabular part?'"), РежимДиалогаВопрос.ДаНетОтмена);
        Возврат;
	КонецЕсли;
...
Процедура КомандаЗаполнитьПоРабочемуВремени(Команда)
	Если Объект.СтрокиОтчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоРабочемуВремениЗавершение", ЭтотОбъект), НСтр("ru='Заполнить с очисткой табличной части?';en='Fill with cleaning of a tabular part?'"), РежимДиалогаВопрос.ДаНетОтмена);
        Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДатыНачалаИОкончанияПериода(Дата, НачалоПериода, КонецПериода, Сотрудник)
	...
	Для Каждого СтрокаТаблицы Из Объект.СтрокиОтчета Цикл
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Этап) Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				Ошибки = Ошибки
					+ ?(Ошибки = "", "", Символы.ПС)
					+ НСтр("ru='Не заполнена колонка ""Этап"" в строке';en='The column ""Stage"" in line is not filled in'")
					+ " " + СтрокаТаблицы.НомерСтроки + " "
					+ НСтр("ru='списка ""Строки отчета""';en='list ""Line of report""'");
				//
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПроверкаКорректностиЗаполненияНаКлиенте()
	Если Не ЗначениеЗаполнено(Объект.НачалоПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Начало периода не заполнено!';en='The beginning of the period is not filled in!'"),, "Объект.НачалоПериода");
		//
		Возврат Ложь;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Конец периода не заполнен!';en='The end of the period is not filled in!'"),, "Объект.КонецПериода");
		//
		Возврат Ложь;
	КонецЕсли;
	Если Объект.НачалоПериода > Объект.КонецПериода Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Начало периода больше конца периода!';en='The beginning of the period are more than end of the period!'"),, "Объект.НачалоПериода");
		//
		Возврат Ложь;
	КонецЕсли;
	Если Объект.Дата < НачалоДня(Объект.НачалоПериода) Или Объект.Дата > КонецДня(Объект.КонецПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Дата документа должна находится в выбранном периоде!';en='Document date must fall within the specified period!'"),, "Объект.Дата");
		//
		Возврат Ложь;
	КонецЕсли;
	...
	Если (НачалоДня(Объект.КонецПериода) - НачалоДня(Объект.НачалоПериода)) / СекундВДне > ДнейВГоду Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Выбранный период не должен превышать один год!';en='Selected period should not exceed one year!'"),, "Объект.КонецПериода");
		//
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Объект.СтрокиОтчета Цикл
		Если СтрокаТаблицы.ДатаР < Объект.НачалоПериода Или СтрокаТаблицы.ДатаР > Объект.КонецПериода Тогда
			Ошибки = Ошибки
				+ ?(Ошибки = "", "", Символы.ПС)
				+ НСтр("ru='Строка';en='Строка'") + " " + СтрокаТаблицы.НомерСтроки + ": "
				+ НСтр("ru='дата работ находится за пределами выбранного периода!';en='date of operations are outside of the select period!'");
			//
		КонецЕсли;
	КонецЦикла;
	Если Не ПустаяСтрока(Ошибки) Тогда
