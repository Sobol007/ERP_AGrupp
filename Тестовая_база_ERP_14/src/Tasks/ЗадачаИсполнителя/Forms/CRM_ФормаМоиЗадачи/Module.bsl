...
Процедура УстановитьПометкуУдаления(Команда)
	...
	Если НЕ ТекущийЭлемент.ТекущиеДанные = Неопределено Тогда
		...
		Если ТекущийЭлемент.ТекущиеДанные.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru='Снять с ""';en='Remove from ""'") + Строка(ТекущийЭлемент.ТекущаяСтрока) + НСтр("ru='"" пометку на удаление?';en='"" deletion mark?'");
			
		Иначе
			ТекстВопроса = НСтр("ru='Пометить ""';en='Mark""'") + Строка(ТекущийЭлемент.ТекущаяСтрока) + НСтр("ru='"" на удаление?';en='"" for deleting?'");
