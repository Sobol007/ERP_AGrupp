...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаКатегории
...
Функция ПроверитьНаКорректностьЗаполнитьВозвращаемыеЗначения()
	...
	Если бЕстьОтмеченныеКатегории И Не бЕстьОсновнаяКатегория Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не назначена основная категория!';en='The main category are not assigned!'"),, "ТаблицаКатегории");
		Возврат Ложь;
	КонецЕсли;
