...
Процедура ДобавитьВариантыОформленияСумм(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		CRM_Отчеты.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=2");
		
		КлючВариантаОформления = НСтр("ru = 'Без копеек'");
		CRM_Отчеты.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=0");
		
		КлючВариантаОформления = НСтр("ru = 'В тысячах'");
		CRM_Отчеты.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=0; ЧС=3");
			
	КонецЦикла; 
	
	Если НЕ НастройкиКД=Неопределено Тогда
		Для каждого ИмяПоля Из МассивПолей Цикл
			CRM_ОтчетыКлиентСервер.ДобавитьУсловноеОформление(НастройкиКД, ИмяПоля, ВариантыОформления[ИмяПоля][КлючОсновногоВариантаОформления]); 
		КонецЦикла; 
	КонецЕсли; 
...
Процедура ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных) Экспорт
	...
	Если НЕ Поле=Неопределено Тогда
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение = "CRM_Отчеты.МесяцГода("+ИмяПоля+")";
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение = "CRM_Отчеты.ДеньМесяцГода("+ИмяПоля+")";
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение = "CRM_Отчеты.НомерНеделиГода("+ИмяПоля+")";
...
Процедура ОграничитьИспользованиеПоля(Схема, ИмяПоля) Экспорт
	...
	ПолеСКД = CRM_Отчеты.ПолеСхемы(Схема, ИмяПоля);
...
Процедура СоздатьФильтрТумблер(Стр, ЭтаФорма, Элементы)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Иначе
			...
			КонецЦикла; 
		КонецЕсли; 
		ЭтаФорма[ИмяРеквизита] = ?(ТипЗнч(ЭтаФорма[ИмяРеквизита])=Тип("ПеречислениеСсылка.CRM_БулевоТриСостояния"), 
		ПредопределенноеЗначение("Перечисление.CRM_БулевоТриСостояния.НеИзменять"), 
		Неопределено);
		Стр.Элементы.Вставить(Элемент.Имя, Элемент.ПутьКДанным);
	КонецЦикла; 
