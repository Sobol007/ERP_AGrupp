...
Функция ПолучитьОбработчик(ТипОбъекта, Направление, Ключ) Экспорт
	сткВозврат = Новый Структура("ОбработчикНайден, Отлючен, Наименование, ПроцедураОбработки, ИдентификаторШаблона, Версия, Статус", Ложь, Истина);
	...
	Если Не текРезультат.Пустой() Тогда
		...
		сткВозврат.Отлючен			 	= текВыборка.Статус = Перечисления.сшпСтатусыОбработчиков.Отключен;
...
// Функция - ПолучитьКодСостоянияОбработки
//
// Параметры:
...
//
Функция ПолучитьКодСостоянияОбработки(Состояние) Экспорт
...
//
Функция ПолучитьСостоянияОбработкиСообщенияПоКоду(КодСостояния) Экспорт
