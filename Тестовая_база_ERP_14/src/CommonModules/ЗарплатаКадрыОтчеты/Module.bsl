...
Процедура ПередНачаломКомпоновкиРезультата(ОбъектОтчета) Экспорт
	...
	Для Каждого ВыбранноеПоле Из Настройки.Выбор.Элементы Цикл
		...
		ВычисляемоеПоле = ОбъектОтчета.СхемаКомпоновкиДанных.ВычисляемыеПоля.Найти(ПутьКДанным);
		
		Если ВычисляемоеПоле <> Неопределено Тогда
			
			Для Каждого КоллекцияПолей Из ДополнительныеПоля Цикл
				
				ОписаниеПолей = КоллекцияПолей.Значение.Найти(ПутьКДанным, "ПутьПоляСКД");
				Если ОписаниеПолей <> Неопределено Тогда
					...
					ВычисляемоеПоле.Выражение = ПутьКДанным;
