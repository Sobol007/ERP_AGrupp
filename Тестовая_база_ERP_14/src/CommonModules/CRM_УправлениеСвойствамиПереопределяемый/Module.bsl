...
Функция ПолучитьНаборыСвойствДляКартМаршрута(Объект) Экспорт
	...
	Наборы.Добавить(НаборСвойств, НСтр("ru='Свойства карты маршрута';en='Route Map Properties'"));
...
Функция ПолучитьНаборыСвойствНоменклатуры(НоменклатурнаяГруппа = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(НоменклатурнаяГруппа) Тогда
		Наборы.Добавить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоменклатурнаяГруппа, "НаборСвойствХарактеристики"), НСтр("ru='Для номенклатурной группы';en='For the nomenclature group'"));
	КонецЕсли;
	Возврат Наборы;
КонецФункции // ПолучитьНаборыСвойствДляПартнеров()
