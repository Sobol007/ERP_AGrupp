...
//
Процедура ОбработкаОповещенияПанелиЗвонка(ЭтаФорма, ИмяСобытия, Параметр, Источник) Экспорт
	...
	ИначеЕсли ИмяСобытия = "сфпКонецРазговора" Тогда
		Если Параметр.ИдентификаторЗвонка = ЭтаФорма.ИдентификаторЗвонка ИЛИ ПустаяСтрока(ЭтаФорма.ИдентификаторЗвонка) Тогда
			...
			Попытка
				ЭтаФорма.ОбновитьКоличествоЗвонков();
...
#КонецЕсли
	...
	//	НСтр("ru='Для открытия ссылки ""%1"" необходимо установить расширение работы с файлами.';en='Open the link ""%1"", you need to install an extension to work with files.'"),
	//	НавигационнаяСсылка);
...
// Не используется. Процедура - Обработчик нового сообщения обсуждение журнал звонков.
//
// Параметры:
...
Процедура ОбработчикНовогоСообщенияОбсуждениеЖурналЗвонков(Сообщение, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеОповещения.ТипЗвонка = "НовыйЗвонок" Тогда
		Если Сообщение.Получатели.Содержит(сфпСофтФонПроСерверПовтИсп.ИдентификаторТекущегоПользователяСистемыВзаимодействия()) Тогда
	...
	ИначеЕсли ДанныеОповещения.ТипЗвонка = "НачалоРазговора" Тогда
		Если Сообщение.Получатели.Содержит(сфпСофтФонПроСерверПовтИсп.ИдентификаторТекущегоПользователяСистемыВзаимодействия()) Тогда
			...
			Если ДанныеЗвонка.ВходящийЗвонок Тогда
				  НаименованиеДействия = сфпСофтФонПроСервер.сфпПолучитьЗначениеНастройкиПользователя("сфпДействиеПриВходящемЗвонке");
...
Процедура ПослеЗаписиФайлаРазговора(ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.РежимПолученияЗаписи = "play" Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеЗапускаПриложения", сфпСофтФонПроКлиент);
		...
		Иначе		
			Если ПустаяСтрока(ДополнительныеПараметры.ИдентификаторЗаписи) Тогда
				Если сфпСофтФонПроСервер.сфпИспользоватьCLON() Тогда
					...
					НайденнаяЗапись = сфпСофтФонПроКлиент.сфпНайтиЗаписьРазговораCLON(ТелефонА, ТелефонБ, ДополнительныеПараметры.ДатаНачала, ДополнительныеПараметры.ДатаОкончания, ДополнительныеПараметры.Входящий); 
					Если НайденнаяЗапись <> Неопределено Тогда
						// Запускаем запись на прослушивание
						сфпСофтФонПроКлиент.сфпПолучитьФайлРазговора(НайденнаяЗапись.file, ДополнительныеПараметры.ВнутреннийНомер);
			...
			Иначе
				сфпСофтФонПроКлиент.сфпПолучитьФайлРазговора(ДополнительныеПараметры.ИдентификаторЗаписи, ДополнительныеПараметры.ВнутреннийНомер);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция сфпПроверитьДоступностьСофтФон(ВыводитьПредупреждения = Истина) Экспорт
	...
	Если сфпСофтФонПроСервер.ИспользуемаяВерсияСофтФона() = ПредопределенноеЗначение("Перечисление.сфпВерсииСофтФон.СофтФотPROSTO") Тогда
		...
		ИначеЕсли НЕ сфпСофтФонПроСервер.сфпПолучитьЗначениеНастройкиПользователя("сфпИспользоватьСофтФон") Тогда	
			Если ВыводитьПредупреждения Тогда
				ПоказатьПредупреждение(, НСтр("ru='В персональных настройках пользователя отключено использование СофтФон';en='In personal settings of the user usage of SoftPhone is disabled'"), 5);
			КонецЕсли;	
	...
	Иначе
		...
		#Иначе
			...
			ИначеЕсли сфпПанельУправления = Неопределено Тогда
				Если ВыводитьПредупреждения Тогда
					ПоказатьПредупреждение(, НСтр("ru='Отсутствует подключение к серверу СофтФона';en='There are a connection to serverу of SoftPhone'"), 5);
				КонецЕсли;	
...
Процедура сфпПерезаполнитьРегистрПоискаПоНомерам() Экспорт
	...
	Для Сч = 0 по ТипыОбъектовДляПерезаполнения.Количество() - 1 Цикл
		Состояние(НСтр("ru='Выполняется обновление регистра поиска по номеру телефона';en='Update of the register of search in phone number are fulfill'"), Индикатор,, БиблиотекаКартинок.ДлительнаяОперация48);
...
Процедура сфпПерезаполнитьСоответствиеСостоянияЛиний(МассивНомеров) Экспорт
	...
	Для Каждого ЭлементМассива Из МассивНомеров Цикл
		...
		ОчищенныйНомер = сфпСофтФонПроСервер.сфпУбратьИзНомераТелефонаВсеПрефиксы(ЭлементМассива.Number, Истина);
...
//	Структура	- Структура данных звонка
//
Функция сфпНайтиЗвонокПоОсновнымДанным(hCall, LineName, Origin, ВходящийЗвонок, ВнешнийЗвонок, НомерТелефона, CallerID, CalledId, StartDate, AnswerDate, EndDate, RecordID, ContactID) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(hCall) Тогда
			...
			Если СтруктураЗвонка = Неопределено Тогда
				...
				СтруктураЗвонка = сфпНайтиЗвонокПоОсновнымДанным(hCall, НомерЛинии, Origin, ВходящийЗвонок, ВнешнийЗвонок, НомерТелефона, CallerID, CalledId, StartDate, AnswerDate, EndDate, RecordID, ContactID);
			КонецЕсли;
...
Процедура сфпOnLinesStatus(StrData) Экспорт
	...
	ТекстЗаписи = "StrData: " + StrData;
	//сфпСофтФонПроСервер.сфпЗаписьЖурналаРегистрации("OnLinesStatus", ТекстЗаписи);
...
Функция сфпВыполнитьЗвонок(ДополнительныеПараметры)
	...
	Если сфпСофтФонПроСервер.ИспользуемаяВерсияСофтФона() = ПредопределенноеЗначение("Перечисление.сфпВерсииСофтФон.СофтФотPROSTO") Тогда
		...
		//сфпСофтФонПроСервер.ПозвонитьПоНомеру(ДополнительныеПараметры.Телефон, ДополнительныеПараметры.СсылкаНаОбъект, Событие, Ошибка, СтрокаПояснения);
		...
		сфпСофтФонПроКлиент.ОбработатьРезультатВызова(Ошибка, СтрокаПояснения);
	...
	Иначе
		...
		Исключение
			...
			сфпДанныеЗаполнения	= Неопределено;
			Состояние(НСтр("ru='Не удалось создать звонок';en='It were not possible to create a call'"));	
		КонецПопытки;
	КонецЕсли;
...
Функция сфпПозвонить(Телефон, Объект = Неопределено, ДопПараметры = Неопределено, ЗадержкаДобавочногоНомера = 0) Экспорт
	...
	Если СтрДлина(Телефон) > 10 Тогда
		...
		ТекстЗаголовка	= НСтр("ru='ВНИМАНИЕ!!! Выполняется звонок в другую страну';en='ATTENTION!!! The call to other country are fulfill'");
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да,	НСтр("ru='Набрать номер';en='Dial number'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет,	НСтр("ru='Отменить';en='Cancel'"));
...
Процедура сфпПозвонитьВыбравТелефон(СписокОбъектов, ДанныеЗаполнения = Неопределено) Экспорт
	Если СписокОбъектов.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбран абонент';en='No caller selected'"), 5);
		Возврат;
	КонецЕсли;
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ДанныеЗаполнения",	ДанныеЗаполнения);
	...
	Иначе	
		...
		СписокТелефонов.ПоказатьВыборЭлемента(ОписаниеВыбора, НСтр("ru='Выбор телефона';en='Select a phone'"));
	КонецЕсли;
...
Процедура сфпЗапуститьМастерНастройкиAgatUX() Экспорт
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru='Работа мастера настройки AgatUX возможна только в тонком клиенте 1С';en='Operation of the master of settings of AgatUX are possible only in the thin customer 1С'"), 5);
	#Иначе
...
Процедура сфпЗапуститьМастерНастройкиAgatUXЗавершение(КаталогВременныхФайлов, ДополнительныеПараметры) Экспорт
	...
	#Если НЕ ВебКлиент Тогда
		...
		Попытка 
			...
			ОписаниеОшибки = НСтр("ru='Установка мастера настройки AgatUX';en='Setting of the master of settings of AgatUX'") + Символы.ПС + ОписаниеОшибки;
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось запустить установку мастера настройки AgatUX';en='It were not possible to launch setting of the master of settings of AgatUX'"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки());
		КонецПопытки;
	#КонецЕсли
КонецПроцедуры // сфпЗапуститьМастерНастройкиAgatUX()
...
Процедура сфпПолучитьФайлРазговора(ИдентификаторЗаписи, НомерЛинии = "") Экспорт
	...
	#Иначе
		...
		Если сфпСофтФонПроСервер.сфпИспользоватьCLON() Тогда
			...
			КонецЕсли;	
			Если КодВозврата = -1 Тогда
				ПоказатьПредупреждение(, НСтр("ru='Ошибка получения файла записи разговора!';en='Error of obtaining of a file of a conversation recording!'"), 5);
			КонецЕсли;
		КонецЕсли;
	#КонецЕсли
...
Процедура ПродолжитьПодключениеВнешнейКомпонентыCLON(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		Если сфпObjCLON = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не удалось загрузить компоненту записи разговоров CLON';en='It were not possible to load a component of record of talks of CLON'"), 5);
			Возврат;
		...
		Иначе
			...
			Если ВерсияКомпонентыCLON < "1.0.8.0" Тогда
				сфпObjCLON = Неопределено;
				ПоказатьПредупреждение(, НСтр("ru='Не удалось обновить компоненту записи разговоров CLON. Удалите вручную файл компоненты';en='Could not update the CLON conversation record component. Uninstall the component file manually'")
	...
	Иначе	
		ПоказатьПредупреждение(, НСтр("ru='Не удалось подключить компоненту записи разговоров CLON';en='Could not connect CLON call recording component'"), 5);
		Возврат;
	КонецЕсли;
...
Процедура ЗакончитьПодключениеВнешнейКомпонентыCLON()
	...
	Если КодВозврата = -1 Тогда
		...
		Если КодВозврата = 8 Тогда
			  ТекстПредупреждения = НСтр("ru='Проверьте параметры подключения к системе записи телефонных переговоров CLON!';en='Check up parameters of connection to system of record of telephone negotiations of CLON!'");
		Иначе ТекстПредупреждения = НСтр("ru='Не удалось подключиться к системе записи разговоров CLON';en='Could not connect to CLON call recording system'");
		КонецЕсли;	
