...
Процедура ТабельЗаполнитьИтогиПоСотруднику(МассивСтрокПоСотруднику, ОбозначенияВидовВремени, ОписаниеВидовВремени = Неопределено) Экспорт
	...
	МассивСтрокПоСотруднику[0].ВремяИтог =  ТабельПредставлениеИтогаПоВидуВремени(ПервыйВидВремени, ИтогиПоВидамВремени, ОбозначенияВидовВремени, ОписаниеВидовВремени);
	...
	Для Каждого КлючЗначение Из ИтогиПоВидамВремени Цикл
		Если КлючЗначение.Ключ <> ПервыйВидВремени Тогда  
			...
			ПредставлениеИтогаПоВидуВремени = ТабельПредставлениеИтогаПоВидуВремени(КлючЗначение.Ключ, ИтогиПоВидамВремени, ОбозначенияВидовВремени, ОписаниеВидовВремени);
...
Функция ТабельПредставлениеВремениПоВиду(ОбозначениеВремени, КоличествоЧасов, ОписаниеВидовВремени = Неопределено) Экспорт 
	
	ЧасыПредставление = ТабельПредставлениеЧасовПоВидуВремени(ОписаниеВидовВремени, ОбозначениеВремени, КоличествоЧасов);
	
	Если Не ПустаяСтрока(ОбозначениеВремени) Или ЧасыПредставление <> 0 Тогда
		Возврат ОбозначениеВремени + " " + Формат(ЧасыПредставление, "ЧГ=");
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ТабельПредставлениеИтогаПоВидуВремени(ВидВремени, ИтогиПоВидамВремени, ОбозначенияВидовВремени, ОписаниеВидовВремени)
	ПредставлениеИтогаПоВидуВремени = "";
	...
	Если ОбозначениеВидаВремени <> Неопределено Тогда
		...
		ЧасыПредставление = ТабельПредставлениеЧасовПоВидуВремени(ОписаниеВидовВремени, ОбозначениеВидаВремени, ИтогиПоВидуВремени.Часы);
		ПредставлениеИтогаПоВидуВремени = БуквенноеОбозначение + ?(БуквенноеОбозначение = "", "" , " ") 
										+ Строка(ИтогиПоВидуВремени.Дни) + НСтр("ru = ' д.';
																				|en = ' d.'") 
										+ ?(ЧасыПредставление > 0, " " + Строка(ЧасыПредставление) + НСтр("ru = ' ч.';
																											|en = ' h.'") , "");
	КонецЕсли;										
...
Функция ТабельПредставлениеЧасовПоВидуВремени(ОписаниеВидовВремени, ОбозначениеВремени, Часы)
	
	ЧасыПредставление = Часы;
	Если ОписаниеВидовВремени <> Неопределено
			И ОписаниеВидовВремени.Получить(ОбозначениеВремени) <> Неопределено
			И (ОписаниеВидовВремени[ОбозначениеВремени].Целосменное Или ОписаниеВидовВремени[ОбозначениеВремени].ВидВремениСплошнойРегистрации) Тогда
			
		ЧасыПредставление = 0;
	КонецЕсли;
	
	Возврат ЧасыПредставление;
