...
////////////////////////////////////////////////////////////////////////////////
// CRM_MSExchangeВызовСервера
//  
////////////////////////////////////////////////////////////////////////////////
#Область ПрограммныйИнтерфейс

// Функция возвращает информацию о наличии объекта в регистре. 
//
// Параметры:
//  Объект1С - Сссылка - Передаваемый объект.
// 
// Возвращаемое значение:
//  Булево - Информация о наличии объекта в регистре.
//
...
Функция ПолучитьКонтактыСобытий1С(Настройка, СинхронизируемыеСобытия, МассивIDЗагруженныхОбновленныхКонтактов = Неопределено, МассивСсылокЗагруженныхОбновленныхКонтактов = Неопределено) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	               ...
	               |	ИЗ
	               |		Документ.CRM_Взаимодействие.СторонниеЛица КАК СторонниеЛица
	               |	ГДЕ
	               ...
	               |	ИЗ
	               |		Документ.CRM_Взаимодействие.СвоиЛица КАК СвоиЛица
	               |	ГДЕ
...
Функция ПолучитьОткрывающийТегКлиент()
	Возврат "/*** " + НСтр("ru='Клиент:';en='Number of item item'") + " ";
КонецФункции

Функция ПолучитьОткрывающийТегУчастниковСобытия()
	Возврат "/*** " + НСтр("ru='Участники события:';en='Participants of the event:'");
КонецФункции
...
Функция СформироватьСписокУчастников(Событие)
	...
	Если МассивСотрудников.Количество() > 0 Тогда
		...
		|", "") +
		НСтр("ru='Сотрудники:';en='Employees:'");
	...
	Если МассивСтороннихУчастников.Количество() > 0 Тогда
		...
		|
		|", "") +
		НСтр("ru='Сторонние участники:';en='Indirect participants:'");
...
Функция СформироватьДанныеДляСозданияОбновленияСобытия(Настройка, Событие, ДляОбновления = Ложь)
	...
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки, "Start", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Событие.ПлановаяДата, Настройка.СмещениеВремениВМинутах));
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки, "End",   CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Событие.ПлановаяДатаЗавершение, Настройка.СмещениеВремениВМинутах));
		...
		Иначе
			...
			Если (ТипЗнч(ОбъектMSExchange) = Тип("ТаблицаЗначений")) И (ОбъектMSExchange.Количество() > 0) Тогда
				...
				Иначе
					...
					ИначеЕсли СтрокаТаблицы.Событие.СтатусВзаимодействия.ТипСостояния = Перечисления.CRM_ТипыСостоянийСобытий.Завершено Тогда
...
Функция ЗагрузитьОбновитьСобытиеВ1С(Настройка, СтрокаТаблицы, СтрокаОшибки, СоответствияАдресовИКонтактов, МассивСсылокЗагруженныхОбновленныхСобытий, ДатаВремяНачалаОбмена = Неопределено)
	...
	Если ЗначениеЗаполнено(СобытиеСсылка) И (МассивСсылокЗагруженныхОбновленныхСобытий.Количество() > 0) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если ЗначениеЗаполнено(СобытиеСсылка) И НЕ (ТипЗнч(СобытиеСсылка) = Тип("ДокументСсылка.CRM_Взаимодействие")) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	КонецПопытки;
	Попытка
		DateTimeCreated = СтрокаТаблицы.DateTimeCreated;
	Исключение
		DateTimeCreated = Неопределено;
	КонецПопытки;
	Если НЕ ЗначениеЗаполнено(Start) ИЛИ НЕ ЗначениеЗаполнено(End) Тогда
		// События без дат пропускаем (таких не должно быть).
		Возврат Неопределено;
	КонецЕсли;
	ДанныеЗаполнения.Вставить("ПлановаяДата", CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(Start, Настройка.СмещениеВремениВМинутах));
	ДанныеЗаполнения.Вставить("ПлановаяДатаЗавершение", CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(End, Настройка.СмещениеВремениВМинутах));
	ДанныеЗаполнения.Вставить("Дата", CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(DateTimeCreated, Настройка.СмещениеВремениВМинутах));
	Попытка
		Subject = СтрокаТаблицы.Subject;
	Исключение
		Subject = НСтр("ru='Тема не указана';en='The subject are not specif'");
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(Body) Тогда
		...
		Если ПозицияУчастники > 0 Тогда
			...
			КонецЕсли;
		КонецЕсли;
		СтруктураЗаполнения.Вставить("Содержание", ТекстОписания);
	КонецЕсли;
	Попытка
	...
	ДанныеЗаполнения.Вставить("ВидВзаимодействия", Справочники.CRM_ВидыВзаимодействий.Встреча);
	Попытка
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	Если (ТипЗнч(СобытиеСсылка) = Тип("ДокументСсылка.CRM_Взаимодействие")) И ЗначениеЗаполнено(СобытиеСсылка) Тогда
...
Функция ЗагрузитьОбновитьПочтовоеСообщениеВ1С(Настройка, ТипПочтовогоСообщения, СтрокаТаблицы, СтрокаОшибки, СоответствияАдресовИКонтактов, МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений, ДатаВремяНачалаОбмена = Неопределено, УчетнаяЗаписьЭлектроннойПочты = Неопределено)
	...
	Если ЗначениеЗаполнено(ПочтовоеСообщениеСсылка) И ТипЗнч(ПочтовоеСообщениеСсылка) <> ТипДокумента И ТипЗнч(ПочтовоеСообщениеСсылка) <> Тип("ДокументСсылка.CRM_Взаимодействие") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ЗагрузитьОбновитьСписокРассылкиВ1С(Настройка, Подключение, СтрокаТаблицы, СтрокаОшибки, ТолькоОбъектыБезСостава = Ложь, ДатаВремяНачалаОбмена = Неопределено)
	...
	КонецЕсли;
	Если Не ЗначениеЗаполнено(СписокРассылкиОбъект.Наименование) Тогда
		СписокРассылкиОбъект.Наименование = НСтр("ru='Список рассылки';en='Mailing list'");
	КонецЕсли;
...
Процедура СинхронизироватьПоНастройке(Настройка, СтрокаОшибки = "") Экспорт
	Если НЕ ЗначениеЗаполнено(Настройка.УчетнаяЗапись) Тогда
		ВызватьИсключение НСтр("ru='В настройке не заполнена учетная запись!';en='In setting the login account is not filled in!'");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Настройка.Подключение) Тогда
		ВызватьИсключение НСтр("ru='В настройке не заполнено подключение!';en='In setting the login account is not filled in!'");
	КонецЕсли;
	Если НЕ (Настройка.Подключение.СерверMSExchange = Настройка.УчетнаяЗапись.СерверMSExchange) Тогда
		ВызватьИсключение НСтр("ru='Реквизит ""Сервер MS Exchange"" в подключении отличается от значения, указанного в учетной записи!';en='The detail of ""server MS Exchange"" in connection differs from the value specified in the login account!'");
	КонецЕсли;
	...
	Если Настройка.СинхронизироватьКлиенты ИЛИ Настройка.СинхронизироватьКонтактныеЛица Тогда
		...
		Если Направление1С Тогда
			...
			Иначе
				...
				Иначе
					ВызватьИсключение НСтр("ru='Ошибка при получении списка контактов!';en='Error when obtaining the contact list!'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		...
		Если Направление1С Тогда
			...
			Иначе
				Если ЗначениеЗаполнено(СтрокаОшибки) Тогда
					ВызватьИсключение СтрокаОшибки;
				Иначе
					ВызватьИсключение НСтр("ru='Ошибка при получении списка событий!';en='Error when obtaining the event list!'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
					...
					Если ЗначениеЗаполнено(УчетнаяЗаписьЭлектроннойПочты) Или Настройка.СоздаватьСобытияПоПочтовымСообщениям Тогда
						...
						Иначе
							Если ЗначениеЗаполнено(СтрокаОшибки) Тогда
								ВызватьИсключение СтрокаОшибки;
							Иначе
								ВызватьИсключение НСтр("ru='Ошибка при получении списка писем!';en='Error when obtaining the list of letters!'");
							КонецЕсли;
						КонецЕсли;
		...
		Если Направление1С Тогда
			...
			Иначе
				Если ЗначениеЗаполнено(СтрокаОшибки) Тогда
					ВызватьИсключение СтрокаОшибки;
				Иначе
					ВызватьИсключение НСтр("ru='Ошибка при получении списка задач!';en='An error occurred while retrieving the task list!'");
				КонецЕсли;
			КонецЕсли;
	...
	Если Настройка.ЗагружатьСпискиРассылок Тогда
		...
		Иначе
			Если ЗначениеЗаполнено(СтрокаОшибки) Тогда
				ВызватьИсключение СтрокаОшибки;
			Иначе
				ВызватьИсключение НСтр("ru='Ошибка при получении списков рассылок!';en='Error when obtaining lists of mailings!'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьТекстЗапросаКонтакты(Настройка) Экспорт
	...
	Если Настройка.СинхронизироватьКлиенты Тогда
		...
		|	//ТекстДляЗаменыУсловияЭтоГруппаДляCRM
		|	И
		|	(
		|		(ЕСТЬNULL(СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации, ДАТАВРЕМЯ(1,1,1)) = ДАТАВРЕМЯ(1,1,1)
		|			ИЛИ ЕСТЬNULL(СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации, ДАТАВРЕМЯ(1,1,1)) >= &ДатаПоследнейМодификации)
		...
		Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ТекстДляЗаменыУсловияЭтоГруппаДляCRM", "И НЕ Партнеры.ЭтоГруппа");
		КонецЕсли;
	...
	Если Настройка.СинхронизироватьКонтактныеЛица Тогда
		...
		|	РегистрСведений.CRM_СоответствиеОбъектовMSExchange КАК СоответствиеОбъектовMSExchange
		|	ПО
		|		КонтактныеЛицаПартнеров.Ссылка = СоответствиеОбъектовMSExchange.Объект1С
		|		И СоответствиеОбъектовMSExchange.СерверMSExchange = &СерверMSExchange
		|	
		|ГДЕ
		|	КонтактныеЛицаПартнеров.Владелец.ОсновнойМенеджер = &Пользователь
		|	И
		|	(
		|		(ЕСТЬNULL(СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации, ДАТАВРЕМЯ(1,1,1)) = ДАТАВРЕМЯ(1,1,1)
		|			ИЛИ ЕСТЬNULL(СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации, ДАТАВРЕМЯ(1,1,1)) >= &ДатаПоследнейМодификации)
		...
		|	(
		|		ЕСТЬNULL(СоответствиеОбъектовMSExchange.ID, НЕОПРЕДЕЛЕНО) НЕ В (&МассивIDЗагруженныхОбновленныхКонтактов)
		|			И КонтактныеЛицаПартнеров.Ссылка НЕ В (&МассивСсылокЗагруженныхОбновленныхКонтактов)
		|	)
		|	
		|	// Если в регистре нет записи соответствия - пропускаем с пометкой удаления
		|	И НЕ (СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации ЕСТЬ NULL И КонтактныеЛицаПартнеров.ПометкаУдаления)
		|";
		бДобавленоРазрешенные = Истина;
	КонецЕсли;
	Возврат ТекстЗапроса;
...
Функция СформироватьТекстЗапросаСобытия(Настройка) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ГДЕ
	               |		CRM_СоответствиеОбъектовMSExchange.СерверMSExchange = &СерверMSExchange
	               |		И CRM_СоответствиеОбъектовMSExchange.Объект1С ССЫЛКА Документ.CRM_Взаимодействие
	               |		И CRM_СоответствиеОбъектовMSExchange.УчетнаяЗапись = &УчетнаяЗапись
	               |		И CRM_СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации >= &ДатаПоследнейМодификации) КАК Соответствия
	               |		ПОЛНОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |			CRM_Взаимодействие.Ссылка КАК Событие,
	               |			CRM_Взаимодействие.ПлановаяДата КАК Дата
	               |		ИЗ
	               |			Документ.CRM_Взаимодействие КАК CRM_Взаимодействие
	               |		ГДЕ
	               |			CRM_Взаимодействие.Ответственный = &Ответственный
	               |			И CRM_Взаимодействие.Дата >= &ДатаПоследнейМодификации
	               |			И НЕ(CRM_Взаимодействие.ПометкаУдаления
	               |						ИЛИ CRM_Взаимодействие.СтатусВзаимодействия.ТипСостояния = ЗНАЧЕНИЕ(Перечисление.CRM_ТипыСостоянийСобытий.Завершено))) КАК События
...
Процедура ВыполнитьРегламентныйОбменДаннымиСMSExchange() Экспорт
	...
	ЗаписьЖурналаРегистрации("Регламентный обмен данными с MS Exchange",
			...
			НСтр("ru='Начат регламентный обмен данными с MS Exchange';en='The regulated data interchange with MS Exchange are beg'"));
	//
	...
	Попытка
		...
		ЗаписьЖурналаРегистрации("Регламентный обмен данными с MS Exchange",
				УровеньЖурналаРегистрации.Информация,,,
				НСтр("ru='Закончен регламентный обмен данными с MS Exchange';en='The regulated data interchange with MS Exchange are finish'"));
		//
