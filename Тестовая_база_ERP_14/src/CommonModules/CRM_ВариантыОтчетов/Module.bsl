...
Процедура ОпределитьРазделыСВариантамиОтчетов(Разделы) Экспорт
	...
	Если ИмяКонфигурации = "CRM" Тогда
		...
		НСтр("ru='Отчеты по бизнес-процессам';en='Reports on business processes'"));
		...
		НСтр("ru='Отчеты по клиентам';en='Customers Reports'"));
...
Процедура РазмещениеВПодсистемеРабочегоСтола(Настройки) Экспорт
	...
	Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		...
		// после обновления БСП, возможно, придётся удалить
		НастройкиКоллекцияСтрок = Настройки.Строки;
	Иначе
		Подсиситема = Метаданные.Подсистемы["CRM_МодульCRM"].Подсистемы["CRM_ОтчетыРабочегоСтола"];
		// после обновления БСП, возможно, придётся удалить
		НастройкиКоллекцияСтрок = Настройки;
	КонецЕсли;
	ПодсистемыОтчетов = РазмещениеОтчетовВПодсистемах(,Подсиситема);
	КэшФлажкаХранилища = Неопределено;
	Для Каждого НастройкаОтчета Из НастройкиКоллекцияСтрок Цикл
