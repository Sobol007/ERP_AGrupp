...
Процедура ЗарегистрироватьНовуюДатуПолученияДохода(Регистратор, Движения, МенеджерВременныхТаблиц, ДатаВыплаты, ДатаОперации, Отказ, Записывать = Ложь) Экспорт
	...
	|	СУММА(СведенияОДоходахНДФЛ.ВаловаяСуммаВычета) КАК СуммаВычета
	...
	|	ЕСТЬNULL(ВаловыеСуммы.СуммаДохода, 0) КАК ВаловаяСуммаДохода,
	|	ЕСТЬNULL(ВаловыеСуммы.СуммаВычета, 0) КАК ВаловаяСуммаВычета,
	...
	|		ПО СведенияОДоходахНДФЛ.ГоловнаяОрганизация = ВаловыеСуммы.ГоловнаяОрганизация
	|			И СведенияОДоходахНДФЛ.ФизическоеЛицо = ВаловыеСуммы.ФизическоеЛицо
	|			И СведенияОДоходахНДФЛ.МесяцНалоговогоПериода = ВаловыеСуммы.МесяцНалоговогоПериода
	|			И СведенияОДоходахНДФЛ.КодДохода = ВаловыеСуммы.КодДохода
	|			И СведенияОДоходахНДФЛ.КодВычета = ВаловыеСуммы.КодВычета
	|			И СведенияОДоходахНДФЛ.КатегорияДохода = ВаловыеСуммы.КатегорияДохода
	|			И СведенияОДоходахНДФЛ.СтавкаНалогообложения = ВаловыеСуммы.СтавкаНалогообложения
	|			И СведенияОДоходахНДФЛ.ИсточникДоходаЗаПределамиРФ = ВаловыеСуммы.ИсточникДоходаЗаПределамиРФ
	|			И СведенияОДоходахНДФЛ.РегистрацияВНалоговомОргане = ВаловыеСуммы.РегистрацияВНалоговомОргане
	|			И СведенияОДоходахНДФЛ.Организация = ВаловыеСуммы.Организация
	|			И СведенияОДоходахНДФЛ.Начисление = ВаловыеСуммы.Начисление
	|			И СведенияОДоходахНДФЛ.Подразделение = ВаловыеСуммы.Подразделение
	|			И СведенияОДоходахНДФЛ.Сотрудник = ВаловыеСуммы.Сотрудник
	|			И СведенияОДоходахНДФЛ.ДатаПолученияДохода = ВаловыеСуммы.ДатаПолученияДохода
	|			И СведенияОДоходахНДФЛ.ДоходМежрасчетногоПериода = ВаловыеСуммы.ДоходМежрасчетногоПериода
	|			И СведенияОДоходахНДФЛ.ВключатьВДекларациюПоНалогуНаПрибыль = ВаловыеСуммы.ВключатьВДекларациюПоНалогуНаПрибыль
	|			И СведенияОДоходахНДФЛ.ПодразделениеСотрудника = ВаловыеСуммы.ПодразделениеСотрудника
	|			И СведенияОДоходахНДФЛ.УдалитьОрганизация = ВаловыеСуммы.УдалитьОрганизация
	|			И СведенияОДоходахНДФЛ.НеУчитыватьДоходВ6НДФЛ = ВаловыеСуммы.НеУчитыватьДоходВ6НДФЛ
	|			И СведенияОДоходахНДФЛ.ДатаПолученияДоходаФиксирована = ВаловыеСуммы.ДатаПолученияДоходаФиксирована
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ПеренесенныйДоход.ОбщаяСуммаДохода = Выборка.ВаловаяСуммаДохода;
	...
	ПеренесенныхДоходы.Свернуть("ФизическоеЛицо, ДокументОснование, КатегорияДохода", "СуммаДохода, ОбщаяСуммаДохода");
