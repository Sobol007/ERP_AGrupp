...
////////////////////////////////////////////////////////////////////////////////
// Процедуры и функции общего назначения сервер (из БСП для Софтфона)
//  
////////////////////////////////////////////////////////////////////////////////
...
Функция сфпЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты) Экспорт
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение сфпПодставитьПараметрыВСтроку(НСтр("ru='Неверный тип второго параметра Реквизиты: %1';en='Incorrect type of the second parameterAttributes: %1'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
//  Если необходимо зачитать реквизит независимо от прав текущего пользователя то следует 
//  использовать предварительный переход в привилегированный режим.
//
// Функция не предназначена для получения значений реквизитов пустых ссылок.
// 
// Параметры:
//  Ссылка       - Ссылка - Ссылка на элемент справочника, документ и т.д.
...
//
// Параметры:
//  Тип	 - Тип	 - переданны тип значения.
// 
// Возвращаемое значение:
//  Булево - результат проверки.
// 
...
//  ПодключатьПоРасписанию	 - Булево	 - признак расписания.
// 
...
// Параметры:
//  ПараметрыНапоминания	 - Структура - параметры напоминания.
//  ОбновитьСрокНапоминания	 - Булево	 - признак обновления срока.
//
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(ИмяПользователя())
			   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
				...
				КонецЕсли;
				ВременныйАдминистраторИБ.Имя = НСтр("ru='Временный первый администратор';en='Temporary first administrator'")
			...
			ВременныйПользовательИБ.Имя = НСтр("ru='Временный пользователь';en='Temporary user'")
				+ " (" + Строка(Новый УникальныйИдентификатор) + ")";
...
Функция ОписаниеОшибкиПриЗаписиПользователяИБ(ШаблонОшибки,
	...
	Если ЗаписатьВЖурнал Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Пользователи.Ошибка записи пользователя ИБ';en='Users.An error occurred when writing infobase user'"),
			...
			сфпОбщегоНазначенияКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
				"""" + ИмяДляВхода + """ (" + ?(ЗначениеЗаполнено(ИдентификаторПользователяИБ),
					НСтр("ru='Новый';en='New'"), Строка(ИдентификаторПользователяИБ)) + ")",
