...
//
// Параметры:
//  НапоминанияОВходящихПисьмах	 - Булево - Признак напоминания о входящих письмах.
// 
// Возвращаемое значение:
//  Массив - Массив напоминаний.
//
Функция ПолучитьНапоминания(НапоминанияОВходящихПисьмах = Ложь) Экспорт
	...
	Если НапоминанияОВходящихПисьмах Тогда
		...
		Для каждого УчетнаяЗапись Из СоответствиеУчетныхЗаписейИКоличестваПисем Цикл
			...
			Если УчетнаяЗапись.Значение > МаксимальноеКоличествоНапоминанийОднойУчетнойЗаписи Тогда
				...
				ТекстНапоминания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По учетной записи %1 новых писем: %2';en='%1 for new messages by account'"),
																						   УчетнаяЗапись.Ключ,
...
Функция ПолучитьКлючиНапоминанийОВходящихЭлектронныхПисьмахУчетнойЗаписиПользователя(УчетнаяЗапись, Пользователь) Экспорт
	...
	Запрос.Текст ="ВЫБРАТЬ
	              ...
	              |	НапоминанияПользователя.Предмет,
	              |	НапоминанияПользователя.ДатаНачала,
	              |	НапоминанияПользователя.ДатаОповещения
	              |ИЗ
...
// Процедура - Создать напоминание о новом интересе
//
// Параметры:
//  Ссылка	 - ДокументСсылка	 - Ссылка на документ Интерес.
//
	...
	Для Каждого УчетнаяЗаписьИПисьма Из МассивПисем Цикл
		...
		Для Каждого АвторИПисьма Из УчетнаяЗаписьИПисьма.Письма Цикл
			...
			Если АвторИПисьма.Письма.Количество() > 5 Тогда
				...
				ТекстНапоминания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По учетной записи %1 не отправлено %2 писем. Письма перенесены в ""Черновики"". Проверьте адрес получателя и настройки учетной записи';en=""No %2 emails sent by %1. The letters have been moved to Drafts. Check the recipient's address and account settings"""),
