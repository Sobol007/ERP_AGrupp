...
//
// Параметры:
//  ЭтаФорма - УправляемаяФорма - Передаваемая форма.
//
...
Процедура ИсключитьФормыИзЗакладок() Экспорт
	...
	Для каждого Пользователь из ПользователиИБ Цикл
		ОбновитьСоставОткрытыхФорм("Отчет.CRM_ВоронкаПродаж.Форма.Форма", Новый Структура, Истина, Пользователь.Имя);
	КонецЦикла;
КонецПроцедуры
...
Функция ПолучитьТекущийСоставОткрытыхФорм(Пользователь=Неопределено) Экспорт
	// +Рабочий стол
	Если CRM_РабочийСтолСервер.ПолучитьНастройкиОткрытия().ОкрыватьРабочийСтол ИЛИ CRM_РабочийСтолСервер.ПолучитьНастройкиОткрытия().БлокироватьИнтерфейс Тогда
