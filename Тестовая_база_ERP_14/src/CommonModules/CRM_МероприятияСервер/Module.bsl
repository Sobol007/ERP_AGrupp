...
Процедура ИзменитьСобытияРяда(СобытиеИнициатор, Знач СпособИзмененияРяда, Знач ДатаНачала = Неопределено, Знач ДатаОкончания = Неопределено, АдресНастроек) Экспорт
	...
	Для Каждого ДатаВремяРасписания Из МассивДатРасписания Цикл
		...
		// Так как событие было создано/изменено, то оповещения  надо заново транслировать на ряд
...
// 
// Возвращаемое значение:
//  Булево - Если Истина, то запрещены изменения события,
//			 иначе разрешены.
//
...
Процедура ЗаполнитьОбъектМероприятияПоМероприятию(МероприятиеОбъект, МероприятиеШаблон, ДатаПоРасписанию, ЭтоНовыйОбъект)
	...
	МероприятиеОбъект.ПлановаяДата				= НачалоДня(ДатаПоРасписанию) + (МероприятиеШаблон.ПлановаяДата - НачалоДня(МероприятиеШаблон.ПлановаяДата));
	МероприятиеОбъект.ПлановаяДатаЗавершение	= НачалоДня(ДатаПоРасписанию) + (МероприятиеШаблон.ПлановаяДатаЗавершение - НачалоДня(МероприятиеШаблон.ПлановаяДатаЗавершение));
