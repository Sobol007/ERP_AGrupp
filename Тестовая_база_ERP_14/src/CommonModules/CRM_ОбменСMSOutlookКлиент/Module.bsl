...
//
// Параметры:
//  OutlookObj				 - COMObject - COM объект MS Outlook 
//  ТаблицаОбъектов			 - ТаблицаЗначений - Таблица с объектами.
//  СоответствиеСОМОбъекты	 - Соответствие	 - Соответствие COM объектов.
//
...
//
// Параметры:
//  СсылкаПрограммаOutlook	 - СправочникСсылка - Ссылка на справочник CRM_ПрограммыMSOutlook. 
//  СтруктураСвойств		 - Структура  - Передаваяемая структура свойств.
//
...
//
// Параметры:
//  ТаблицаОбъектовOutlook	 - Массив - Таблица объектов. 
//
...
//
// Параметры:
//  СтруктураСвойств	 - Структура - Структура свойств. 
//  Настройка			 - Структура - Структура настроек. 
...
// Параметры:
//  СтруктураСвойств - Структура - Структура свойств.
//  Настройка		 - Структура - Структура с настройками.
//
		...
		Иначе
			...
			#Если НЕ ВебКлиент Тогда
				Если ЗначениеЗаполнено(ИмяКомпьютера) И НЕ (ВРег(ИмяКомпьютера) = ВРег(ИмяКомпьютера())) Тогда
					...
					+ " """ + ИмяКомпьютера + """ "
					+ НСтр("ru='запущено приложенме MS Outlook, попробуйте закрыть его на время обмена.';en='prilozhenme of MS Outlook are triggered, try to close it for the period of an exchange.'")
					//
				КонецЕсли;
			#КонецЕсли
			Попытка
...
// Параметры:
//  СтруктураСвойств - Структура - Структура свойств.
//  Настройки		 - Структура - Структура с настройками.
// 
// Возвращаемое значение:
//  Массив - Массив контактов.
//
...
//
// Параметры:
//  мСтруктураСвойств - Структура - Структура свойств.
//  мНастройка		 - Структура - Структура с настройками. 
//
...
Процедура СопоставитьСобытияOutlook(СтруктураСвойств, Настройка, ТаблицаСинхронизируемыхСобытийOutlook)
	...
	Для Каждого СтрокаСобытийOutlook Из ТаблицаСинхронизируемыхСобытийOutlook Цикл
		...
		Иначе
			НоваяСтрокаСобытия.Объект_1С		= ПредопределенноеЗначение("Документ.удалитьCRM_Мероприятие.ПустаяСсылка");
		КонецЕсли;
		СтруктураСвойств.События.Добавить(НоваяСтрокаСобытия);
		НоваяСтрокаСОМОбъекта					= ПолучитьСтруктуруТаблицыCOMОбъектовСобытий();
		НоваяСтрокаСОМОбъекта.СтрокаТЧ			= НоваяСтрокаСобытия.ID;
...
Процедура СопоставитьКонтакты1С(СтруктураСвойств, Настройка, ТаблицаСинхронизируемыхКонтактов1С, ТолькоНовые = Ложь)
	...
	Если Настройка.ТаблицаПапокOutlook.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В настройке не указаны папки Outlook для получения данных из 1С. Проверьте настройки!';en='In the configuration, Outlook folders are not specified to receive data from 1C. Check the settings!'"));
		Возврат;
	КонецЕсли;
	Для Каждого ПапкаOutlookДляЗагрузкиИз1С Из Настройка.ТаблицаПапокOutlook Цикл
...
Процедура ЗаполнитьСтруктуруСобытияПоНастройке(СтруктураРеквизитов, Настройки)
	...
	КонецЕсли;
	Если НЕ СтруктураРеквизитов.Свойство("ПлановаяДата")
		ИЛИ (СтруктураРеквизитов.Свойство("ПлановаяДата") И НЕ ЗначениеЗаполнено(СтруктураРеквизитов.ПлановаяДата)) Тогда
		СтруктураРеквизитов.Вставить("ПлановаяДата", ТекущаяДата());
	КонецЕсли;
	Если НЕ СтруктураРеквизитов.Свойство("ПлановаяДатаЗавершение")
		ИЛИ (СтруктураРеквизитов.Свойство("ПлановаяДатаЗавершение")
		И (НЕ ЗначениеЗаполнено(СтруктураРеквизитов.ПлановаяДатаЗавершение)
		ИЛИ (ТипЗнч(СтруктураРеквизитов.ПлановаяДатаЗавершение) = Тип("Дата")
		И СтруктураРеквизитов.ПлановаяДатаЗавершение < СтруктураРеквизитов.ПлановаяДата))) Тогда
		СтруктураРеквизитов.Вставить("ПлановаяДатаЗавершение", СтруктураРеквизитов.ПлановаяДата);
	КонецЕсли;
...
Процедура ЗаполнитьСтруктуруСобытияПоОбъектуOutlook(СтруктураРеквизитов, ОбъектOutlook)
	...
	СтруктураРеквизитов.Вставить("Содержание",			ПолучитьРеквизитОбъектаOutlook(ОбъектOutlook,	"СодержаниеСобытия"));
	СтруктураРеквизитов.Вставить("Дата",				ПолучитьРеквизитОбъектаOutlook(ОбъектOutlook,	"НачалоСобытия"));
	СтруктураРеквизитов.Вставить("ПлановаяДата",				ПолучитьРеквизитОбъектаOutlook(ОбъектOutlook,	"НачалоСобытия"));
	СтруктураРеквизитов.Вставить("ПлановаяДатаЗавершение",	ПолучитьРеквизитОбъектаOutlook(ОбъектOutlook,	"ОкончаниеМероприятия"));
	СтруктураРеквизитов.Тема		= CRM_ОбменСMSOutlookКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтруктураРеквизитов.Тема);
	СтруктураРеквизитов.Содержание	= CRM_ОбменСMSOutlookКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтруктураРеквизитов.Содержание);
КонецПроцедуры
...
Функция СформироватьТекстЗапросаСобытия(Настройки)
	...
	//|	удалитьCRM_Мероприятие.Ссылка КАК Событие,
	//|	удалитьCRM_Мероприятие.Ответственный,
	//|	удалитьCRM_Мероприятие.Описание,
	//|	удалитьCRM_Мероприятие.Дата,
	//|	удалитьCRM_Мероприятие.ПометкаУдаления,
	//|	СоответствиеОбъектовOutlook.ДатаПоследнейМодификации КАК ДатаПоследнейМодификации
	//|ИЗ
	...
	//|	РегистрСведений.CRM_СоответствиеОбъектовOutlook КАК СоответствиеОбъектовOutlook
	//|ПО
	//|	СоответствиеОбъектовOutlook.СобственнаяСсылка = удалитьCRM_Мероприятие.Ссылка
	//|ГДЕ
	...
	|	CRM_Взаимодействие.Тема КАК Описание,
	|	CRM_Взаимодействие.ПометкаУдаления КАК ПометкаУдаления,
	|	СоответствиеОбъектовOutlook.ДатаПоследнейМодификации КАК ДатаПоследнейМодификации
	|ИЗ
	|	Документ.CRM_Взаимодействие КАК CRM_Взаимодействие
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.CRM_СоответствиеОбъектовOutlook КАК СоответствиеОбъектовOutlook
	|		ПО (СоответствиеОбъектовOutlook.СобственнаяСсылка = CRM_Взаимодействие.Ссылка)
	|ГДЕ
	|	(СоответствиеОбъектовOutlook.ДатаПоследнейМодификации ЕСТЬ NULL
	|			ИЛИ СоответствиеОбъектовOutlook.ДатаПоследнейМодификации = ДАТАВРЕМЯ(1, 1, 1)
	|			ИЛИ СоответствиеОбъектовOutlook.ДатаПоследнейМодификации > &ДатаПоследнейМодификации)
...
Процедура ДобавитьВложенияЭлектронногоПисьма(Объект_1С, ОбъектOutlook)
	#Если Не ВебКлиент Тогда
	Если Не ЗначениеЗаполнено(Объект_1С) Тогда
		Возврат;
	КонецЕсли;
