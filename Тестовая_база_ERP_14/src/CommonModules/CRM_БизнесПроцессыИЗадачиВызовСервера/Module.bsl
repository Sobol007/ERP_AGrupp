...
// Заблокировать данные для редактирования ведущей задачи.
//
// Параметры:
//  БизнесПроцесс		 - БизнесПроцессСсылка - Ссылка на бизнес процесс.
//  ПродолжитьВыполнение - Булево - Прекратить выполнение блокировки задачи.
// 
// Возвращаемое значение:
//  Структура - Структура с описанием ошибки.
//
...
Процедура Выполнить_Задачу(ЗадачаСсылка, ДействиеПоУмолчанию = Ложь, РазрешитьВыполнитьСПодчиненнымиЗадачами = Ложь) Экспорт
	...
	Если Не РазрешитьВыполнитьСПодчиненнымиЗадачами И CRM_БизнесПроцессыИЗадачиВызовСервера.ЕстьПодчиненныеЗадачи(ЗадачаСсылка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='У задачи';en='At the task'") + " """ + Строка(ЗадачаСсылка) + """ "
...
//
// Параметры:
//  ЗадачаСсылка			 - ЗадачаСсылка - Ссылку на задачу. 
//  ВариантВыполнения		 - Число - Вариант выполнения. 
//  ОтложитьНаДату			 - Дата	 - Отложить на дату.
//  РезультатВыполнения		 - Строка - Результат выполнения.
	...
	Для Каждого Задача Из МассивЗадач Цикл
		...
		Если ЗадачаВыполнена Тогда
			...
			Если ТолькоПроверка Тогда
				ПричинаОтказа = НСтр("ru='Невозможно перенаправить уже выполненную задачу.';en='It is impossible to redirect already carried  out task.'");
				Прервать;
			КонецЕсли;
		КонецЕсли;	
		Если НЕ CRM_БизнесПроцессыСервер.ЗадачаДоступнаДляПринятияКИсполнению(Задача.Ссылка) Тогда
			Результат = Ложь;
			Если ТолькоПроверка Тогда
				ПричинаОтказа = НСтр("ru='Невозможно перенаправить чужую задачу.';en=""It is impossible to redirect someone else's task.""");
				Прервать;
			КонецЕсли;
		КонецЕсли;
...
Функция ФормаВыполненияЗадачи(Знач ЗадачаСсылка, Знач ИзКалендаря = 0) Экспорт
	...
	Если ТипЗнч(ЗадачаСсылка) <> Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный тип параметра ЗадачаСсылка (передан: %1; ожидается: %2)';en='Irregular type of parameter of Zadachassylka (it is transfer: %1; it is expected: %2)'"),
...
Процедура НазначитьРодителя(Задача, Родитель, Партнер = Неопределено, КонтактноеЛицо = Неопределено, Проект = Неопределено, ДополнительныеРеквизиты = Неопределено, НазначитьВсемВИерархии = Истина) Экспорт
	...
	Если бОшибочныйРодитель Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нельзя переносить задачу внутрь собственной иерархии';en='It is impossible to transfer the task in own hierarchy'")
			+ ": """ + Строка(Задача) + """!");
		//
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьСобытие(СобытиеСсылка)  Экспорт
	...
	Попытка
		СобытиеОбъект.Записать();
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось изменить состояние, вероятно событие заблокировано';en='It were not possible to change status, possibly event it is disabl'"));	
	КонецПопытки;
...
Процедура ИзменитьМаркетинговоеМероприятие(СправСсылка) Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось изменить состояние, вероятно элемент справочника заблокирован';en='It were not possible to change status, possibly an element of the reference manual it is disabl'"));	
	КонецПопытки;
...
Процедура ЗадачаИсполнителяМодульОбъектаОбработкаЗаполнения(Объект, ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ЗадачаОбъект.ЗадачаИсполнителя") Тогда
		ЗаполнитьЗначенияСвойств(Объект, ДанныеЗаполнения, 
			...
			+ "ДатаНачала,РезультатВыполнения,Предмет,CRM_ТочкаМаршрута,CRM_Итерация,CRM_ВариантВыполнения,CRM_Личная,CRM_ОсновнаяКатегория,CRM_Партнер, CRM_СостояниеИнтереса");
		Объект.Дата = ТекущаяДатаСеанса();
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту")
	ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана")
	...
	ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_Интерес")
	ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_Телемаркетинг") Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.МаркетинговыеМероприятия") Тогда
			...
			КонецЕсли;
		ИначеЕсли НЕ (ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту") 
				ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_СообщениеМессенджера") 
				ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_Интерес")) Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана")
			ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_Телемаркетинг") Тогда
		...
		КонецЕсли;
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
...
Процедура ЗадачаИсполнителяМодульОбъектаПередЗаписью(Объект, Отказ) Экспорт
	...
	Если НЕ ИсходныеРеквизиты.Выполнена И Объект.Выполнена Тогда
		...
		Если Объект.СостояниеБизнесПроцесса = Перечисления.СостоянияБизнесПроцессов.Остановлен Тогда
			ВызватьИсключение НСтр("ru='Нельзя выполнять задачи остановленных бизнес-процессов.';en='It is impossible to carry out of the task of the stop business processes.'");
		КонецЕсли;	
	...
	Если Не Отказ Тогда
		Если Объект.CRM_Личная
			...
			И (ТипЗнч(Объект.Предмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее")
				ИЛИ ТипЗнч(Объект.Предмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее")
				ИЛИ ТипЗнч(Объект.Предмет) = Тип("ДокументСсылка.CRM_СообщениеМессенджера")) Тогда
			//
