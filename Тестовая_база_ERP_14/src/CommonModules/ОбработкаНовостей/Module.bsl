	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"ОбработкаНовостей.ДобавитьПрограммныйОтборОбщий");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В модели сервиса метод [%1] разрешено вызывать только в неразделенном режиме.';
					|en = 'In the service model the method [%1] can be called in the undivided mode only.'"),
				"ОбработкаНовостей.УстановитьПрограммныйОтборОбщий");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
	...
	Если (ИспользоватьНомерОбластиДанных = Истина) Тогда
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"ОбработкаНовостей.ПолучитьПрограммныйОтборДляОбластиДанных");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В модели сервиса метод [%1] разрешено вызывать:
					...
					| - either in the divided mode
					| - or you should transfer the positive number in the parameter НомерОбластиДанных.'"),
				"ОбработкаНовостей.ПолучитьПрограммныйОтборПользовательский");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
