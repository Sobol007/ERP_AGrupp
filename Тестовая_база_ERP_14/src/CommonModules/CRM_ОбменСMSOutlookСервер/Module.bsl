...
//
// Параметры:
//  мСтруктур	 - Структура - Структура, которую необходимо преобразовать.
//  стрКолонки	 - Строка	 - Имя колонки.
// 
...
//
// Параметры:
//  ПрограммаOutlook - СправочникСсылка	 - Ссылка на справочник CRM_ПрограммыMSOutlook.
//  Копирование		 - Булево - Признак копирования.
//  ТекущаяНастройка - Структура - Текущая настройка.
// 
// Возвращаемое значение:
//  Структура - Новая настройка.
//
...
// Параметры:
//  ПрограммаOutlook - СправочникСсылка	 - Ссылка на справочник CRM_ПрограммыMSOutlook.
//  ИмяНастройки	 - Строка	 - Строка с настройками.
// 
// Возвращаемое значение:
// 	Структура - Структура настройки.
//
...
//  Отбор				 - ОтборКомпоновкиДанных - Отбираемые поля.
//  ДатаПоследнегоОбмена - Дата	 - Дата последнего обмена.
// 
// Возвращаемое значение:
//  Массив - Массив событий.
//
...
Функция СоздатьСобытие(СтруктураЗаполнения) Экспорт
	Если СтруктураЗаполнения = Неопределено Тогда 
		Возврат Неопределено;
	КонецЕсли;
	Событие = Документы.удалитьCRM_Мероприятие.СоздатьДокумент();	
...
Функция ЗаполнитьМероприятиеПоСтруктуре(Ссылка, СтруктураОбъекта, ОчиститьУчастников = Ложь, СтрокаСобытия) Экспорт
	Попытка 
		...
		Иначе	
			мОбъект = Документы.CRM_Взаимодействие.СоздатьДокумент();	
		КонецЕсли;
	Исключение
	КонецПопытки;
	Попытка
	...
	Исключение
	КонецПопытки;
	мОбъект.ВидВзаимодействия = Справочники.CRM_ВидыВзаимодействий.Встреча;
	мОбъект.СтатусВзаимодействия = Справочники.CRM_СостоянияСобытий.Запланировано;
	Попытка
		мОбъект.Записать();
		//Если ТипЗнч(мОбъект) = Тип("ДокументОбъект.удалитьCRM_Мероприятие") Тогда
...
Функция ПолучитьСтруктуруСвойствСобытия(Ссылка) Экспорт
	...
	СвойстваОбъекта.Дата = Ссылка.ПлановаяДата;
	СвойстваОбъекта.ОкончаниеМероприятия = Ссылка.ПлановаяДатаЗавершение;
	СвойстваОбъекта.Описание = Ссылка.Содержание;
