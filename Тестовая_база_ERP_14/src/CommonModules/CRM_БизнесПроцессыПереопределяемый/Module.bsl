...
////////////////////////////////////////////////////////////////////////////////
// Бизнес-процессы переопределяемый
//  
////////////////////////////////////////////////////////////////////////////////
...
Процедура ТочкаСтартаПередСтартом(ТочкаМаршрута, БизнесПроцесс, Отказ, ТекстСообщения = "") Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Перед стартом"" в точке';en='Run error of the handler ""Before start"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
...
Процедура ТочкаСтартаПриСтарте(ТочкаМаршрута, БизнесПроцесс,ФормируемыеЗадачи) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При старте"" в точке';en='Run error of the handler ""At start"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ТочкаСтартаПослеСтарта(ТочкаМаршрута, БизнесПроцесс, ФормируемыеЗадачи) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""После старта"" в точке';en='Run error of the handler ""After start"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ТочкаЗавершенияПриЗавершении(ТочкаМаршрута, БизнесПроцесс) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При завершении"" в точке';en='Run error of the handler ""At completion"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
// Формирование описания бизнес процесса.
//
// Параметры:
//  Задача	 - ЗадачаСсылка	 - Задача.
// 
// Возвращаемое значение:
//  Строка - Описание бизнес процесса. 
//
Функция СформироватьОписаниеБизнесПроцесса(Задача) Экспорт
	...
	СписокРеквизитов.Добавить("Организация"						,НСтр("ru='Организация:';en='Organization:'"));
	СписокРеквизитов.Добавить("Подразделение"					,НСтр("ru='Подразделение:';en='Department:'"));
	СписокРеквизитов.Добавить("Автор"							,НСтр("ru='Автор:';en='Author:'"));
	СписокРеквизитов.Добавить("Партнер"							,НСтр("ru='Клиент:';en='Number of item item'"));
	СписокРеквизитов.Добавить("КонтактноеЛицо"					,НСтр("ru='Контакт:';en='Contact'"));
	
	СписокРеквизитов_1 = Новый СписокЗначений;
	СписокРеквизитов_1.Добавить("Дата"							,НСтр("ru='Создан:';en='0'"));
	СписокРеквизитов_1.Добавить("ДатаСтарта"					,НСтр("ru='Стартован:';en='Started:'"));
	СписокРеквизитов_1.Добавить("ПланируемаяДатаЗавершения"		,НСтр("ru='Завершится (план):';en='Finished (plan):'"));
	СписокРеквизитов_1.Добавить("Проект"						,НСтр("ru='Проект:';en='Project:'"));
	// СписокРеквизитов_1.Добавить("Ответственный"				,НСтр("ru='Ответственный:';en='Responsible:'"));
	СписокРеквизитов_1.Добавить("Сумма"							,НСтр("ru='Сумма:';en='Amount:'"));
...
// Формирование истории бизнес процесса
//
// Параметры:
//  Задача	 - ЗадачаСсылка	 - Задача.
//  Сделка	 - Булево	 - Признак принадлежности к этапу сделки.
// 
// Возвращаемое значение:
//  Строка - История бизнес процесса.
//
Функция СформироватьИсториюБизнесПроцесса(Задача, Сделка = Ложь) Экспорт
	
	ТекущаяЗадача = "";
	ИсторияВыполнения = "";
	//|<P><span style='font-size: 10pt;'><span style='font-size: 10pt;'><EM><STRONG>" + НСтр("ru='История выполнения';en='Execution history'")+"</STRONG></EM></span></P>";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТабОбъектов.Количество() > 0 Тогда
			Если НЕ Сделка Тогда
				СтрокаОбъектовПоЗадаче = "
					|<P><span style='font-size: 10pt;'><EM>&ensp;&ensp;&ensp;" + НСтр("ru='Введенные на этапе объекты:';en='The objects enter at a stage:'")+"</EM></span></P>";
			Иначе
				СтрокаОбъектовПоЗадаче = "
										 |" + НСтр("ru='Введенные на этапе объекты:';en='The objects enter at a stage:'") + "
										 |";
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ИсторияВыполнения = "" Тогда
		Если НЕ Сделка Тогда
			ИсторияВыполнения = "<P><span style='font-size: 10pt;'><span style='font-size: 10pt;'><EM><STRONG>"
				+ НСтр("ru='История выполнения';en='Execution history'")+"</STRONG></EM></span></P>" + ИсторияВыполнения;
		Иначе
			ИсторияВыполнения = НСтр("ru='История выполнения';en='Execution history'") + "
							|" + ИсторияВыполнения + "
							|";
		КонецЕсли;
	КонецЕсли;
...
// Возвращает схему компоновки по имени объекта.
//
// Параметры:
//  ИмяОбъекта	 - Строка - Имя объекта.
// 
// Возвращаемое значение:
//  СхемаКомпоновкиДанных - Схема компоновки данных по имени объекта. 
//
...
// Функция - Скомпоновать настройки
//
// Параметры:
//  КомпоновщикНастроек			 - КомпоновщикНастроекКомпоновкиДанных - Компоновщик настроек. 
//  АдресСхемыКомпоновкиДанных	 - Строка - Адрес во временном хранилище.
// 
// Возвращаемое значение:
...
// Добавление элементов отбора в настройки схемы.
//
// Параметры:
...
// Пример добавления
//  МассивИдентификаторов.Добавить(Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторОбъектаМетаданныхПоПолномуИмени("Справочник.Номенклатура"));
//  Указанный тип метаданных должен входить в Определяемый тип CRM_ПроцессныеОбъекты
//
// Параметры:
//  МассивИдентификаторов	 - Массив - Массив идентификаторов. 
//
Процедура ИспользуемыеТипыГрупповыхПредметовПроцессов(МассивИдентификаторов) Экспорт
...
Функция ПолучитьИменаПолейДляОтбораПоИмениОбъекта(ИмяОбъекта) Экспорт
	...
	Иначе
		...
		ЭтоДокумент			= (ОбъектМетаданных = Неопределено);
		Если ЭтоДокумент Тогда
			Если НЕ Метаданные.Документы[ИмяОбъекта].Реквизиты.Найти("Организация") = Неопределено 
					И ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(Метаданные.Документы[ИмяОбъекта].Реквизиты.Организация) Тогда
...
Процедура ТочкаДействияПриВыполнении(ТочкаМаршрута, Задача, Отказ, ТекстСообщения = "") Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При выполнении"" в точке';en='Run error of the handler ""At execution"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиДействияПослеВыполнения(ТекущаяЗадача, ФормируемыеЗадачи) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""После выполнения"" в точке';en='Run error of the handler ""After execution"" in a point'") + " " + ТекущаяЗадача.CRM_ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиДействияПередСозданиемЗадач(БизнесПроцесс ,ТочкаМаршрута, ФормируемыеЗадачи, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Перед созданием задач"" в точке';en='Run error of the handler ""Before creation of tasks"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиДействияПриСозданииЗадач(БизнесПроцесс ,ТочкаМаршрута, ФормируемыеЗадачи, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При создании задач"" в точке';en='Run error of the handler ""At creation of tasks"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ОбработчикТочкиВыборВарианта(БизнесПроцесс, ВходящаяТочка, ТочкаМаршрута, ЗНАЧ ВариантВыполнения) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			...
			Возврат ВариантВыполнения;
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Выбор варианта"" в точке';en='Run error of the handler ""Choice of an option"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ОбработчикТочкиУсловияПроверкаУсловия(БизнесПроцесс, ВходящаяТочка, ТочкаМаршрута, ЗНАЧ ВариантВыполнения) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			...
			Возврат ВариантВыполнения;
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Проверка условия"" в точке';en='Run error of the handler ""Check of a condition"" in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиВложенныйБизнесПроцессПриСозданииВложенногоБизнесПроцесса(БизнесПроцесс, ВедущаяЗадача) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При создании вложенного бизнес-процесса"" в точке';en='Run error of the handler ""At creation of nested business process"" in a point'") + " " + ВедущаяЗадача.CRM_ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиВложенныйБизнесПроцессПослеСозданияВложенногоБизнесПроцесса(БизнесПроцесс, ВедущаяЗадача) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""После создания вложенного бизнес-процесса"" в точке';en='Run error of the handler ""After creation of nested business process"" in a point'") + " " + ВедущаяЗадача.CRM_ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ТочкаОбработкиОбработка(ТочкаМаршрута, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Обработка"" в точке';en='Run error of the Handling handler in a point'") + " " + ТочкаМаршрута.Наименование + Символы.ПС + ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
