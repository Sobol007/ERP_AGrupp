...
//  Число -  Разность дат.
//
Функция ИсключениеВременногоПериода(Знач ДатаНачала, Знач ДатаКонца, Знач НачалоПериода = 0, Знач КонецПериода = 0) Экспорт
	...
	Если ТипЗнч(ДатаНачала) = Тип("Дата") Тогда
		...
		Всего = ИсключениеВременногоПериода(ДатаНачала - НачалоДня(ДатаНачала), Дат - НачалоДня(ДатаНачала), НачалоПериода, КонецПериода);
	...
	Иначе
		...
		Если НачалоПериода <= КонецПериода Тогда
			Если (НачалоПериода < ДатаКонца) И (КонецПериода > ДатаНачала) Тогда
				Всего = Всего - (КонецПериода-НачалоПериода) + ?(НачалоПериода<ДатаНачала, ДатаНачала-НачалоПериода, 0) + ?(КонецПериода>ДатаКонца, КонецПериода-ДатаКонца, 0);
			КонецЕсли;
		Иначе
			Всего = Всего - ?(ДатаНачала<КонецПериода, КонецПериода-ДатаНачала, 0) - ?(ДатаКонца>НачалоПериода, ДатаКонца-НачалоПериода, 0);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Если ПараметрыРаботыКлиента.ДоступноИспользованиеРазделенныхДанных И
		...
		Если CRM_ДемонстрационныйРежим.ЭтоДемоРежим() Тогда
			ПоказатьОповещениеПользователя(,, "Подготовка данных демобазы...", БиблиотекаКартинок.CRM_Ожидание);
			CRM_ДемонстрационныйРежим.ПроверкаОбновлениеДемоДанные();
			ПоказатьОповещениеПользователя(,, "Подготовка демобазы завершена.", БиблиотекаКартинок.CRM_ЗеленаяГалка);
...
//  ТаблицаИнтересов	 - ТаблицаЗначений - Таблица интересов. 
//
...
//
// Параметры:
//  ПараметрыОтбора		 - Структура - Структура с отборами.
//  Элемент				 - ПолеФормы  - Элемент формы.
...
// Установить свойства командной панели интересов в форме документа
//
// Параметры:
//  Форма	 - УправляемаяФорма	 - Форма. 
//
...
//  ЭтотОбъект			 - ДокументОбъект - Объект. 
//  ТекущийЭлемент		 - Произвольный - Текущий элемент. 
//
...
//
// Пример: 
//  ТекстПредупреждения = НСтр("ru='Закрыть помощник?';en='Close wizard?'");
...
//
Процедура ПоказатьНашеПредупреждение(ТекстПредупреждения, Таймаут = 0, Заголовок = "", ОписаниеОповещенияОЗакрытии = Неопределено) Экспорт
...
Процедура ДеревоПользовательскиеНастройкиСпискаПередУдалением(ЭтаФорма, ИдентификаторСписка, Элемент, Отказ) Экспорт
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить настройку?';en='Delete settings?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура НастройкиПолейОтображенияСодержанияСохранитьВФайл() Экспорт
	...
	ВыборФайла.Заголовок = НСтр("ru='Сохранение файла настроек полей отображения содержания';en='Saving of a file of settings of fields of display of the content'");
	ВыборФайла.МножественныйВыбор = Ложь;
	ВыборФайла.ПолноеИмяФайла = НСтр("ru='Настройки отображения содержания';en='Settings of display of the content'") + ".xml";
	ВыборФайла.Фильтр = НСтр("ru='Файлы настроек отображения содержания (*.xml)|*.xml';en='Files of settings of display of the content (*.xml)|*.xml'");
...
Процедура НастройкиПолейОтображенияСодержанияЗагрузитьИзФайла(ПоместитьТаблицуВоВременноеХранилищеПоАдресу = Неопределено, ОписаниеОповещенияЗавершение = Неопределено) Экспорт
	...
	ВыборФайла.Заголовок = НСтр("ru='Открытие файла настроек полей отображения содержания';en='opening of a file of settings of fields of display of the content'");
	ВыборФайла.МножественныйВыбор = Ложь;
	ВыборФайла.Фильтр = НСтр("ru='Файлы настроек отображения содержания (*.xml)|*.xml';en='Files of settings of display of the content (*.xml)|*.xml'");
	ВыборФайла.ПроверятьСуществованиеФайла = Истина;
...
Процедура НастройкиПолейОтображенияСодержанияЗагрузитьИзФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Все текущие настройки будут перезаписаны, продолжить?';en='All current settings will be re-record, to continue?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
	Иначе
...
Функция ПолучитьПредставлениеДняНедели(ДеньНеделиКалендаря) Экспорт
	...
	Если НомерДняНедели = 1 Тогда
		Возврат НСтр("ru='Пн';en='Пн'");
	ИначеЕсли НомерДняНедели = 2 Тогда
		Возврат НСтр("ru='Вт';en='St'");
	ИначеЕсли НомерДняНедели = 3 Тогда
		Возврат НСтр("ru='Ср';en='Ср'");
	ИначеЕсли НомерДняНедели = 4 Тогда
		Возврат НСтр("ru='Чт';en='Thu'");
	ИначеЕсли НомерДняНедели = 5 Тогда
		Возврат НСтр("ru='Пт';en='Unipolar FET'");
	ИначеЕсли НомерДняНедели = 6 Тогда
		Возврат НСтр("ru='Сб';en='Sat'");
	Иначе
		Возврат НСтр("ru='Вс';en='Вс'");
	КонецЕсли;
