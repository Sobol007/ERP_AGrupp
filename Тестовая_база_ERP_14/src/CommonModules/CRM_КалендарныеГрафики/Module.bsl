...
//
// Параметры:
//	Календарь		- СправочникСсылка.Календари - календарь, который необходимо использовать.
//	ДатаОт			- Дата - Дата, от которой нужно рассчитать количество дней.
//	МассивДней		- Массив,Число -  с количеством дней, на которые нужно увеличить дату начала.
...
//	ВызыватьИсключение - Булево - булево, если Истина вызывается исключение в случае незаполненного календаря.
//
...
Функция ПолучитьМассивДатПоКалендарю(Знач Календарь, Знач ДатаОт, Знач МассивДней, Знач РассчитыватьСледующуюДатуОтПредыдущей = Ложь, ВызыватьИсключение = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПоКалендарю = Неопределено Тогда
			СообщениеОбОшибке = НСтр("ru='Необходимо заполнить календарь ""%1"" за указанный год (в разделе Общие настройки)!';en='It is necessary to fill a calendar ""%1"" for the specified year (in section Inline tunings)!'");
...
//  ВызыватьИсключение	 - Булево - Булево, если Истина вызывается исключение в случае незаполненного графика.
//  ВпередПоКалендарю	 - Булево - Булево, если Истина, то получаем следующую дату входящую в указанный график ГрафикРаботы.
// 
// Возвращаемое значение:
