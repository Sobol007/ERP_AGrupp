...
Функция КППолучитьВарианты(КоммерческоеПредложение) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СписокВариантов.Добавить(Выборка.ИДВарианта, ?(ЗначениеЗаполнено(Выборка.ИмяВарианта), Выборка.ИмяВарианта, НСтр("ru='Вариант ';en='Variant'")+Строка(Выборка.ИДВарианта))+?(Выборка.Утвержден,НСтр("ru = ' (Утв.)'"),""),Выборка.Утвержден);
...
Процедура КПИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	|	КоммерческоеПредложениеЗапасы.Ссылка.Партнер КАК Партнер,
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК ХарактеристикаНоменклатуры,
	|	КоммерческоеПредложениеЗапасы.КоличествоУпаковок КАК Количество,
	...
	|	ВЫРАЗИТЬ((КоммерческоеПредложениеЗапасы.Цена * КоммерческоеПредложениеЗапасы.КоличествоУпаковок) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2)) КАК СуммаБезСкидок,
	|	ВЫРАЗИТЬ(КоммерческоеПредложениеЗапасы.Цена * КоммерческоеПредложениеЗапасы.КоличествоУпаковок КАК ЧИСЛО(15, 2)) КАК СуммаБезСкидокВал,
...
Процедура КП_ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		...
		КомандаПечати.Представление = НСтр("ru='Коммерческое предложение (Microsoft Word)';en='Commercial Proposal (Microsoft Word)'");
		...
		КомандаПечати.Представление = НСтр("ru='Коммерческое предложение (OpenOffice.org Writer)';en='Commercial Proposal (OpenOffice.org Writer)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = НЕ ЭтоCRM;
	...
	КомандаПечати.Представление = НСтр("ru='Коммерческое предложение';en='Commercial Proposal'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = НЕ ЭтоCRM;
...
Функция КП_СформироватьПечатнуюФормуКоммерческогоПредложения(МассивОбъектов, ОбъектыПечати, КомплектыПечати, ПараметрыПечати = Неопределено) Экспорт
	...
	Если НЕ CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДокументЗапасы.ЕдиницаИзмерения",
			"ВЫБОР КОГДА ДокументЗапасы.Упаковка<>ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка) ТОГДА ДокументЗапасы.Упаковка ИНАЧЕ ДокументЗапасы.Номенклатура.ЕдиницаИзмерения КОНЕЦ КАК ЕдиницаИзмерения");
