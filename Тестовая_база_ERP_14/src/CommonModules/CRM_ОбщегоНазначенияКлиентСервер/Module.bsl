...
Функция ПолучитьДатуИзСтроки(СтрокаСДатой) Экспорт 
	...
	РезультатДата = Дата("00010101000000");
	
	Если ПустаяСтрока(СтрокаСДатой) Тогда
		Возврат РезультатДата;
	КонецЕсли;
	
	Если Найти(СтрокаСДатой,".") > 0 Тогда
		
		Если Найти(СтрокаСДатой,":") > 0 И Найти(СтрокаСДатой," ") > 0 Тогда 
			МассивСтроки = СтрРазделить(СтрокаСДатой," ");
			Возврат ПолучитьДатуИзСтроки(МассивСтроки[0]);
		КонецЕсли;
	...
	Попытка
		...
		Если МассивСтроки.Количество() = 3 Тогда //"01.02.13" или "01.02.2013"
			Год = ?(СтрДлина(МассивСтроки[2]) = 2,2000 + МассивСтроки[2],МассивСтроки[2]);
			РезультатДата = Дата(Год,МассивСтроки[1],МассивСтроки[0]);
		ИначеЕсли МассивСтроки.Количество() = 2 Тогда //"02.13" или "02.2013";
			Год = ?(СтрДлина(МассивСтроки[1]) = 2,2000 + МассивСтроки[1],МассивСтроки[1]);
			РезультатДата = Дата(Год,МассивСтроки[0],1);
		КонецЕсли;
...
Функция ПолучитьПредставлениеПериодаВремени(ВремяНачало, ВремяОкончание)
	...
	Если РазницаВМинутах < 60 Тогда
		...
		Если РазницаВМинутах = 1 Тогда
			ПредставлениеВремя = НСтр("ru='минута';en='minute'");
		ИначеЕсли РазницаВМинутах <= 4 Тогда
			ПредставлениеВремя = НСтр("ru='минуты';en='minutes'");
		Иначе
			ПредставлениеВремя = НСтр("ru='минут';en='minutes'");
		КонецЕсли;
	Иначе
		...
		Если РазницаВЧасах = 1 Тогда
			ПредставлениеВремя = НСтр("ru='час';en='hour'");
		ИначеЕсли РазницаВЧасах <= 4 Тогда
			ПредставлениеВремя = НСтр("ru='часа';en='hours'");
		Иначе
			ПредставлениеВремя = НСтр("ru='часов';en='hours'");
		КонецЕсли;
	КонецЕсли;
...
//
Процедура ЗаполнитьСписокВыбораВремени(Список, ВремяНачало = Неопределено, ДобавлятьПредставлениеДлительности = Ложь, УчитыватьДату = Ложь) Экспорт
	...
	Если бДобавлятьПредставлениеДлительности Тогда
		...
		Если УчитыватьДату Тогда
			ЗначениеВремяНачало = ВремяНачало;
		Иначе
			ЗначениеВремяНачало = ПустаяДата + (ВремяНачало - НачалоДня(ВремяНачало));
		КонецЕсли;
		ТекВремя = ЗначениеВремяНачало + 30 * 60;
	Иначе
		Если УчитыватьДату Тогда
			ЗначениеВремяНачало = НачалоДня(ВремяНачало);
		Иначе
			ЗначениеВремяНачало = ПустаяДата;
		КонецЕсли;
		ТекВремя = ЗначениеВремяНачало;
	КонецЕсли;
	
	Пока НачалоДня(ТекВремя) <= НачалоДня(ЗначениеВремяНачало) Цикл
...
//
Процедура ВыбратьВремяИзСписка(Форма, Знач СтароеЗначение, ЭлементФормы, Знач ВремяНачало = Неопределено, ДобавлятьПредставлениеДлительности = Ложь, ОписаниеОповещения, УчитыватьДату = Ложь) Экспорт
	
	Список = Новый СписокЗначений();
	
	Если УчитыватьДату Тогда
		СтароеЗначениеОкругленное = Дата("00010101") + ( Цел((СтароеЗначение - Дата("00010101")) / (30 * 60)) * (30 * 60) );
		Если НачалоДня(СтароеЗначение) > НачалоДня(ВремяНачало) Тогда
			ВремяНачало = НачалоДня(СтароеЗначение);
	...
	CRM_ОбщегоНазначенияКлиентСервер.ЗаполнитьСписокВыбораВремени(Список, ВремяНачало, ДобавлятьПредставлениеДлительности, УчитыватьДату);
...
Функция РазобратьСтрокуСПочтовымАдресом(знач СтрокаАдреса) Экспорт
	...
	СообщениеНеверныйФорматПочтовогоАдреса	= НСтр("ru='Неверный формат адреса электронной почты.';en='Incorrect format of the e-mail address.'");
	СообщениеПробелВСтроке					= НСтр("ru='Адреса электронной почты не должены содержать пробелов.';en='E-mail addresses must not contain spaces.'");
	СообщениеСимволыРусскогоАлфавита					= НСтр("ru='Адреса электронной почты должны состоять из букв английского алфавита';en='E-mail addresses must consist of English letters'");
	СообщениеПустоеДоменноеИмя				= НСтр("ru='Доменное имя не может быть пустым';en='The domain name could not be empty'");
	ПредыдущийСимвол ="";
...
Функция НастройкиПолейОтображенияСодержанияПолучитьПустоеСодержание(СсылкаНаОбъект = Неопределено, ТекстКомментария = "") Экспорт
	...
	//ГиперссылкаЗаполнить = ?(ЗначениеЗаполнено(ТекстКомментария), "", "<DIV align=""right""><FONT size=""1""><A style=""COLOR: #594304"" href=""ЗаполнитьПоУмолчанию"">" + НСтр("ru='Заполнить список полей содержания по умолчанию';en='Fill in the list of default content fields'") + "</A></FONT></DIV>");
	Описание =
	"<html>
	|<head>
	|<meta http-equiv=Content-Type content=" + """" + "text/html; charset=utf-8"">
...
// Цвет - значение типа Цвет
// }
Функция ПолучитьСписокЦветовПоУмолчанию() Экспорт

	СписокЦветов = Новый СписокЗначений;
	
	СписокЦветов.Добавить(16, НСтр("ru='Темно-оранжевый';en='The dark orange'"));
	СписокЦветов.Добавить(22, НСтр("ru='Темно-синий';en='The dark blue'"));
	СписокЦветов.Добавить(4, НСтр("ru='Зеленый';en='Green'"));
	СписокЦветов.Добавить(8, НСтр("ru='Лиловый';en='The lilac'"));
	СписокЦветов.Добавить(20, НСтр("ru='Темно-бирюзовый';en='Darkly - cyan'"));
	СписокЦветов.Добавить(11, НСтр("ru='Темно-стальной';en='Darkly - steel'"));
	СписокЦветов.Добавить(24, НСтр("ru='Темно-бордовый';en='The maroon'"));
	СписокЦветов.Добавить(17, НСтр("ru='Темно-персиковый';en='Darkly - peach'"));
