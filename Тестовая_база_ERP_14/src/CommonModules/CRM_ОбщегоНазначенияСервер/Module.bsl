...
Процедура ПользовательскиеНастройкиСпискаЗаполнитьПодменюНастроек(ЭтаФорма, СвойстваДинамическогоСписка) Экспорт
	...
	ПодменюНастроек.Подсказка = НСтр("ru='Список настроенных пользователем отборов и группировок';en='The list of the selections adjust by the user and groupings'");
	...
	Если Команда = Неопределено Тогда
		Команда = ЭтаФорма.Команды.Добавить(ИмяКоманды);
		Команда.Действие = ИмяПроцедурыКоманды;
		Команда.Заголовок = НСтр("ru='Настройки...';en='Settings...'");;
	КонецЕсли;
	...
	Кнопка.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	Кнопка.Заголовок = НСтр("ru='Настройки...';en='Settings...'");
	Кнопка.ИмяКоманды = ИмяКоманды;
...
Функция НастройкиПолейОтображенияСодержанияПолучитьСодержание(СсылкаИлиДеревоНастроекИлиОбъект,
	...
	Иначе
		...
		Для Каждого СтрокаНастройки Из ДеревоНастроек.Строки Цикл
			...
			Иначе
				Если ВывестиПримерЗаполнения Тогда
					ПримерПредставлениеРеквизита = ?(ЗначениеЗаполнено(СтрокаНастройки.ПредставлениеРеквизита), СтрокаНастройки.ПредставлениеРеквизита, ?(ЗначениеЗаполнено(СтрокаНастройки.ОригинальноеПредставлениеРеквизита), СтрокаНастройки.ОригинальноеПредставлениеРеквизита, НСтр("ru='Реквизит';en='Detail'")));
		...
		Иначе
			ГиперссылкаНастройки = "<FONT size=""1""><br><hr noshade size=""1""></FONT><DIV align=""right""><FONT size=""1""><A style=""COLOR: #594304"" href=""НастройкаПолейОтображения"">" + НСтр("ru='Настройка';en='Settings'") + "</A></FONT></DIV>";
		КонецЕсли;
...
Функция НастройкиПолейОтображенияСодержанияПолучитьОписаниеОбъектаHTML(СсылкаНаОбъект, Представление, БезПереноса = Ложь, ГиперСсылка = Истина, ТегиЭлемента = Неопределено)
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		...
		Если НЕ (ТипЗнч(СсылкаНаОбъект) = Тип("Неопределено")) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СсылкаНаОбъект)) И ОбщегоНазначения.СсылкаСуществует(СсылкаНаОбъект) Тогда
			...
			КонецЕсли;
		Иначе
			стрОписание = ?(ЗначениеЗаполнено(Представление), "<%ЗаголовокОткрывающийТег%>" + Представление + ": " + "<%ЗаголовокЗакрывающийТег%>", "") + НСтр("ru='[Объект недоступен]';en='[Object not available]'") + ?(БезПереноса, "" , "</BR>");
		КонецЕсли;
	Иначе
		стрОписание = "";
	КонецЕсли;	
		...
		Для Каждого Строка Из ТаблицаРеквизитов Цикл
			...
			Для Каждого колонка Из  ТаблицаРеквизитов.Колонки Цикл
				...
				Если Реквизит <> Неопределено Тогда
					...
					ПримерПредставлениеРеквизита = ?(ЗначениеЗаполнено(Реквизит.ПредставлениеРеквизита), Реквизит.ПредставлениеРеквизита, ?(ЗначениеЗаполнено(Реквизит.ОригинальноеПредставлениеРеквизита), Реквизит.ОригинальноеПредставлениеРеквизита, НСтр("ru='Реквизит';en='Detail'")));
				...
				Для Каждого колонка Из  ТаблицаРеквизитов.Колонки Цикл
					...
					Если Реквизит <> Неопределено Тогда
						...
						Иначе
							...
							Иначе
								ОписаниеРеквизита = НСтр("ru='[Объект недоступен]';en='[Object not available]'");
							КонецЕсли;
...
Процедура ЗаполнитьОбъектБизнесПроцесса(Объект, БизнесПроцесс, Задача = Неопределено) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.CRM_Телемаркетинг") Тогда
		Если Задача = Неопределено Тогда
			...
			Объект.Тема			= НСтр("ru='Бизнес-процесс от';en='Business process from'") + " " + Формат(БизнесПроцесс.Дата,"ДЛФ=D")+ "/ " + БизнесПроцесс.Наименование;
		Иначе
			Объект.Основание	= Задача;
			Объект.Тема			= НСтр("ru='Задача от';en='The task from'") + " " + Формат(Задача.Дата,"ДЛФ=D")+ "/ " + Задача.Наименование;
		КонецЕсли;
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.КоммерческоеПредложениеКлиенту") Тогда
		Если Задача = Неопределено Тогда
			Объект.ДокументОснование	= БизнесПроцесс;
			Объект.CRM_Примечание		= НСтр("ru='Бизнес-процесс от';en='Business process from'") + " " + Формат(БизнесПроцесс.Дата,"ДЛФ=D")+ "/ " + БизнесПроцесс.Наименование;
		Иначе
			Объект.ДокументОснование	= Задача;
			Объект.CRM_Примечание		= НСтр("ru='Задача от';en='The task from'") + " " + Формат(Задача.Дата,"ДЛФ=D")+ "/ " + Задача.Наименование;
		КонецЕсли;
	КонецЕсли;
	// 6. Исходящее письмо
...
Функция ПолучитьСсылкуНаРазделСправки(Раздел) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("CRM_РаботаВЛокальномРежиме") Тогда
		Если Раздел = "СинхронизацияДанныхЧерезУниверсальныйФормат" Тогда
			ПутьКСправке = "http://1crm.ru/help/obmen-dannymi-cherez-universalnyy-format-obmena/";
		...
		ИначеЕсли Раздел = "АРМПочта" Тогда
			ПутьКСправке = "http://1crm.ru/help/pochtovyy-klient/";
			
		ИначеЕсли Раздел = "АРМДиалоги" Тогда
			ПутьКСправке = "http://1crm.ru/help/Dialogi-podkluchenie-vkontakte-i-telegram/";
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	Для Счетчик = 1 По ЧислоПараметров Цикл
		...
		Если ПозЗапятой > 0  Тогда
			...
			Исключение
				Сообщение = Новый СообщениеПользователю();
		        Сообщение.Текст = СтрЗаменить(НСтр("ru='Не удалось определить значение параметра организации: %Параметр%';en='It were not possible to define value of parameter of the organization: %parametr %'"), "%Параметр%", ИмяПараметра);
				Сообщение.Сообщить();
			КонецПопытки;
		КонецЕсли; 
	КонецЦикла;
...
Функция ПолучитьКонтактнуюИнформациюПоТипуКИ(ОбъектКонтактнойИнформации, ТипИнформации) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектКонтактнойИнформации) = Тип("СправочникСсылка.Партнеры") Тогда
		...
		Если ОбъектКонтактнойИнформации.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.Компания Тогда
...
Функция ПолучитьКонтактныхЛицКонтрагентаПодробно(Партнер, ТипыКИ) Экспорт
	...
	Для каждого ТипКИ из ТипыКИ Цикл
		ИмяКолонки = "КИТипа"+СтрЗаменить(Строка(ТипКИ), " ", "_");
		ИмяКолонкиТип = "ТипКИ"+СтрЗаменить(Строка(ТипКИ), " ", "_");
		...
		Для каждого Контакт из ТЗКонтактов Цикл
			КонтактнаяИнформация = CRM_ОбщегоНазначенияСервер.ПолучитьКонтактнуюИнформациюПоТипуКИ(Контакт.КонтактноеЛицо, ТипКИ);
	...
	Если бПредметДоступен Тогда
		...
		Если ЗначениеЗаполнено(ПредметДокумента) Тогда
			...
			Если Найти(ПолноеИмя, "Документ.") Или Найти(ПолноеИмя, "Задача.") Тогда
				...
				КонецЕсли;
				ПредставлениеПредмета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2';en='%1 of %2'"), ПредставлениеПредмета, 
		...
		КонецЕсли;
	Иначе	
		ПредставлениеПредмета = НСтр("ru='[Предмет недоступен]';en='[Item not available]'");
	КонецЕсли;
	Возврат ПредставлениеПредмета;
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
		Если Документ.Ссылка.Пустая() Тогда
			Если ЗначениеЗаполнено(Представление) Тогда
				ЗаголовокФормы =  НСтр("ru='    Новое письмо';en='New letter'") + Нстр("ru = ' от '") + Строка(Представление)
			Иначе				
				ЗаголовокФормы =  НСтр("ru='    Новое письмо';en='New letter'");
			КонецЕсли;				
		Иначе
...
Функция Склонение(Знач ФИО, Знач Падеж, Знач Пол = Неопределено) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка загрузки компоненты для склонения ФИО! Функции склонения недоступны!';en='Error of loading of a component for declination of Full name! Functions of declination was inaccessible!'"));
		Возврат Неопределено;
	КонецПопытки;
...
Функция CRM_ПолучитьДанныеОбъекта(СсылкаНаОбъект, знач НомерВарианта = Неопределено) Экспорт
	...
	// **************** Получим из Автотекста.
		...
		Для Каждого ЭлементСписка Из СписокВариантов Цикл
			...
			Иначе
				СтруктураВарианта.Вставить("СтрокаНДС", ?(Объект.ЦенаВключаетНДС, НСтр("ru='в том числе НДС:';en='Including VAT:'"), НСтр("ru='Сумма НДС:';en='Total of the VAT:'")));
...
//
// Параметры:
//  Форма					 - УправляемаяФорма	 - упр форма, на которой размещается группа создаваемых реквизитов.
//  Объект					 - ДокументОбъект	 - Документ объект.
//  ИмяЭлементаДляРазмещения - ПолеФормы		 - имя элемента формы, по умолчанию не задано.
...
Процедура СоздатьНеобходимыеРеквизитыИЭлементыДляГруппыИнтерес(Форма, Объект, ИмяЭлементаДляРазмещения = "", ТипОбъекта = Неопределено) Экспорт
	...
	Иначе
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("Клиент"					, Новый ОписаниеТипов("СправочникСсылка.Партнеры, СправочникСсылка.CRM_ПотенциальныеКлиенты"), , "Клиент"		, Ложь));
	...
	Элемент.Подсказка	   = НСтр("ru='По клиенту есть несколько актуальных интересов. Выберите один из них.';en='The customer has several actual leads. Choose one of them.'");
	Элемент.ПодсказкаВвода = НСтр("ru='По клиенту есть несколько актуальных интересов. Выберите один из них...';en='The customer has several actual leads. Choose one of them ...'");
	...
	НоваяКомандаФормы.Заголовок                 = НСтр("ru='Скрыть\показать интересы';en='Hide \ show leads'");
	НоваяКомандаФормы.Подсказка                 = НСтр("ru='Скрыть\показать интересы';en='Hide \ show leads'");
	...
	НоваяКнопкаФормы.Заголовок			= НСтр("ru='Скрыть\показать интересы';en='Hide \ show leads'");
...
Процедура ЗаполнитьНадписьВСтрокеИнтереса(Форма, Объект) Экспорт
	...
	Если ЭтоЗадача Тогда
		Элемент.Подсказка	   = НСтр("ru='Доступны несколько актуальных интересов. Выберите один из них.';en='Several topical leads are available. Choose one of them.'");
		Элемент.ПодсказкаВвода = НСтр("ru='Доступны несколько актуальных интересов. Выберите один из них...';en='Several topical leads are available. Choose one of them ...'");
	Иначе
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.удалитьCRM_Мероприятие")
			ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.СообщениеSMS") ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.CRM_Взаимодействие") Тогда
		...
		Если КоличествоИнтересов = 0 ИЛИ КоличествоИнтересов = Неопределено Тогда
			Элемент.Подсказка	   = НСтр("ru='По клиенту нет активных интересов. Создайте новый интерес...';en='There are no active leads for customer. Create a new lead ...'");
			Элемент.ПодсказкаВвода = НСтр("ru='По клиенту нет активных интересов. Создайте новый интерес...';en='There are no active leads for customer. Create a new lead ...'");
		ИначеЕсли КоличествоИнтересов = 1 Тогда
			Элемент.Подсказка	   = НСтр("ru='По клиенту есть активный интерес. Выберите его из списка или создайте новый интерес...';en='The customer has an active lead. Select it from the list or create a new lead ...'");
			Элемент.ПодсказкаВвода = НСтр("ru='По клиенту есть активный интерес. Выберите его из списка или создайте новый интерес...';en='The customer has an active lead. Select it from the list or create a new lead ...'");
		Иначе
			Элемент.Подсказка	   = НСтр("ru='По клиенту есть несколько активных интересов. Выберите один из них.';en='The customer has several active leads. Choose one of them.'");
			Элемент.ПодсказкаВвода = НСтр("ru='По клиенту есть несколько активных интересов. Выберите один из них...';en='The customer has several active leads. Choose one of them ...'");		
		КонецЕсли; 
...
Процедура ЗаполнитьГруппуИнтересаВФормеДокументаМероприятиеИлиСМССообщение(Объект, Форма, ПриСозданииФормыНаСервере = Ложь, ИзмененКлиентКонтакт = Ложь, ТипОбъекта = Неопределено, ЭтоНовый = Ложь)
	...
	Если ИзмененКлиентКонтакт Тогда
		...
		// Мероприятие
		Если ТипОбъекта = Тип("ДокументСсылка.удалитьCRM_Мероприятие") ИЛИ ТипОбъекта = Тип("ДокументСсылка.CRM_Взаимодействие") Тогда
...
Процедура ЗаполнитьГруппуИнтересаВФормеПрочихДокументов(Объект, Форма, ПриСозданииФормыНаСервере = Ложь, ИзмененКлиентКонтакт = Ложь, ТипОбъекта = Неопределено, ЭтоНовый = Ложь, СсылкаНаОснование=Неопределено)
	...
	Если ИзмененКлиентКонтакт Тогда
		...
		// Заявка
		Если ТипОбъекта = Тип("ДокументСсылка.CRM_Заявка") Тогда
			Если Форма.Клиент  <> Объект.Контакт Тогда
				Форма.Клиент  = Объект.Контакт;
				Форма.CRM_ТаблицаИнтересов.Очистить();
				Форма.CRM_ИнтересСтрокой	= "";
			КонецЕсли;	
		КонецЕсли;
...
Процедура СвязатьСообщениеМессенджераСИнтересом(Источник, ВсеИнтересы = Ложь) Экспорт
	...
	Если (ЗначениеЗаполнено(Источник.Контакт) 
		...
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И CRM_Интерес.Подразделение = &Подразделение", "");
		КонецЕсли;
		Запрос.Текст = ТекстЗапроса;
...
Функция ЗапуститьФоновоеЗаданиеСКонтекстомКлиента(ИмяМетода, Параметры = Неопределено, Ключ = "", Наименование = "") Экспорт
	...
	Если ТекущийРежимЗапуска() = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Запуск фонового задания с контекстом клиента возможен только при наличии клиента.';en='Running a background task with customer context is possible only if there is a customer.'");
	КонецЕсли;
...
Функция ПользовательскиеНастройкиСпискаЗаполнитьПоУмолчанию(Знач ИмяМакета = Неопределено, Знач ИмяПользователя = Неопределено) Экспорт
	...
	Попытка
		СтрокаОшибкаФормата = СтрЗаменить(НСтр("ru='Ошибка формата данных макета: %ИмяМакета%!';en='Error of a format of the data of a model: %imyamaketa %!'"), "%ИмяМакета%", ИмяМакета);
	...
	Исключение
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';en='%1'"), ПредставлениеИнформацииОбОшибке);
	КонецПопытки;
...
//
//
Процедура СвязатьВходящееПисьмоСИнтересом(Источник, ВсеИнтересы = Ложь) Экспорт
	...
	И ТипЗнч(Источник.ОтправительКонтакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров")) Тогда
	...
	ИначеЕсли (ЗначениеЗаполнено(Источник.ОтправительКонтакт) 
	И ТипЗнч(Источник.ОтправительКонтакт) = Тип("СправочникСсылка.Партнеры")) Тогда
		
		ЕстьАктивныеИнтересы = Ложь;
		
		ПарамПартнер = Источник.ОтправительКонтакт;
	...
	// Если у нас не контакт, то надо проверить на потенциального клиента.
	ИначеЕсли (ЗначениеЗаполнено(Источник.ОтправительКонтакт) 
		И ТипЗнч(Источник.ОтправительКонтакт) = Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты")) Тогда
...
//
//
Процедура СвязатьТелефонныйЗвонокСИнтересом(Источник, ВсеИнтересы = Ложь) Экспорт
...
Процедура ДополнитьСписокМодулейПодсистем(МодулиПодсистем) Экспорт
	...
	// CRM
	Если CRM_ОбщегоНазначенияСервер.НайтиВМетаданныхПоИмени("ОбщиеМодули", "CRM_ОбновлениеИнформационнойБазы") Тогда
		МодулиПодсистем.Добавить("CRM_ОбновлениеИнформационнойБазы");
	КонецЕсли;
	
	// Модуль CRM для ERP/КА2/УТ11
	Если CRM_ОбщегоНазначенияСервер.НайтиВМетаданныхПоИмени("ОбщиеМодули", "CRM_Модуль_ОбновлениеИнформационнойБазы") Тогда
