...
Функция СформироватьПечатнуюФормуАвансовогоОтчета(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ВыборкаПоказатели = МассивРезультатов[8].Выбрать();
	ВыборкаКонвертация = МассивРезультатов[11].Выбрать();
	ВыборкаОборотнаяСторона = МассивРезультатов[10].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Если ИспользуетсяРеглУчет Тогда
		ВыборкаПроводок = МассивРезультатов[14].Выбрать();
		ВыборкаСчетов = МассивРезультатов[15].Выбрать();
		ВыборкаСчетовУточнение = МассивРезультатов[15].Выбрать();
		Проводки = МассивРезультатов[14].Выгрузить();
	Иначе
		ВыборкаПроводок = МассивРезультатов[12].Выбрать();
		ВыборкаСчетов = МассивРезультатов[12].Выбрать();
		ВыборкаСчетовУточнение = МассивРезультатов[12].Выбрать();
		Проводки = МассивРезультатов[12].Выгрузить();
	КонецЕсли;
...
Функция ТекстЗапросаПечати(ИспользуетсяРеглУчет)
	...
	|	АвансовыйОтчетНаПечатьВТ1.Подразделение,
	...
	|		И ДенежныеСредства.Подразделение = АвансовыйОтчетНаПечатьВТ1.Подразделение
	...
	|		И ДенежныеСредства.Регистратор ССЫЛКА Документ.АвансовыйОтчет
	|	
	|СГРУППИРОВАТЬ ПО
	|	АвансовыйОтчетНаПечатьВТ1.Ссылка,
	|	АвансовыйОтчетНаПечатьВТ1.Организация,
	...
	|	АвансовыйОтчетНаПечатьВТ2.Подразделение,
	|	АвансовыйОтчетНаПечатьВТ2.ПодотчетноеЛицо,
	|	АвансовыйОтчетНаПечатьВТ2.Ссылка,
	|	АвансовыйОтчетНаПечатьВТ2.МоментВремени,
	|	МАКСИМУМ(ДенежныеСредства.Регистратор) КАК ПредыдущийОтчет
	|
	|ПОМЕСТИТЬ АвансовыйОтчетНаПечатьВТ2_1
	|ИЗ
	...
	|		И ДенежныеСредства.Подразделение = АвансовыйОтчетНаПечатьВТ2.Подразделение
	...
	|	АвансовыйОтчетНаПечатьВТ2.Ссылка,
	|	АвансовыйОтчетНаПечатьВТ2.МоментВремени,
	|	АвансовыйОтчетНаПечатьВТ2.ПредыдущийОтчет,
	|	ДокументАвансовыйОтчет.МоментВремени КАК МоментВремениПредыдущийОтчет
	|
	|ПОМЕСТИТЬ АвансовыйОтчетНаПечатьВТ3
	|ИЗ
	|	АвансовыйОтчетНаПечатьВТ2_1 КАК АвансовыйОтчетНаПечатьВТ2
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.АвансовыйОтчет КАК ДокументАвансовыйОтчет
	|	ПО
	|		ДокументАвансовыйОтчет.Ссылка =  АвансовыйОтчетНаПечатьВТ2.ПредыдущийОтчет
	|ИНДЕКСИРОВАТЬ ПО
	|	АвансовыйОтчетНаПечатьВТ2.Организация,
	|	АвансовыйОтчетНаПечатьВТ2.Подразделение,
	|	АвансовыйОтчетНаПечатьВТ2.ПодотчетноеЛицо,
	...
	|			(Организация, Подотчетноелицо) В (ВЫБРАТЬ Организация, Подотчетноелицо ИЗ АвансовыйОтчетНаПечатьВТ1)) КАК ДенежныеСредства
	|	ПО
	|		ДенежныеСредства.Регистратор = АвансовыйОтчетНаПечатьВТ3.Ссылка
