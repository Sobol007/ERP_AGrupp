...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеСервер.ВариантПоставкиКОРП() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Монитор показателей доступен только для ""КОРП"" поставки конфигурации!';en='The indicator monitor is only available for ""CORP"" configuration delivery!'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура СводныйОтчет(Команда)
	...
	//	Предупреждение(НСтр("ru='Сформировать сводный отчет невозможно, т.к. монитор показателей не настроен.';en='Generate a consolidated report is impossible, because the scorecard is not configured.'"),, НСтр("ru='Ошибка формирования сводного отчета';en='Error generating summary report'"));
	//	
	// ИначеЕсли СоставМонитораПустойПоОтбору Тогда
	//	Предупреждение(НСтр("ru='Сформировать сводный отчет невозможно, т.к. в текущий отбор не входит ни один показатель.';en='Generate a consolidated report is impossible, because no indicator is included in the current selection.'"),, НСтр("ru='Ошибка формирования сводного отчета';en='Error generating summary report'"));
	//	
	// Иначе 
	//	
...
ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементовКратко" + КлючУникальностиГруппы + "__" + Сч, НСтр("ru='Колонка №';en='Column No.'") + Сч, РодительскаяГруппа); // ГруппаВариантаОтображения); 			
		КонецЦикла;
	...
	Иначе
		Если Элементы.Найти("ГруппаЭлементовПодробно" + КлючУникальностиГруппы) = Неопределено Тогда
			Для Сч = 1 По СтруктураНастроек.КоличествоКолонокМонитораПоказателей Цикл 
				...
				ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаПодробныхЭлементов" + КлючУникальностиГруппы + "__" + Сч, НСтр("ru='Колонка №';en='Column No.'") + Сч, РодительскаяГруппа); // ГруппаВариантаОтображения); 
...
//  Возможные команды: "Влияющие показатели", "Зависимые показатели", "Периодичность", "Объект анализа",
//	"Отчеты", "Анализ показателя"
//
// Параметры:
...
Процедура ДобавитьКомандыВариантовАнализа(Показатель, Пользователь, ГруппаЭлементов, ИдентификаторЭлементаФормы, ВариантОтображения)
	...
	Если ВариантОтображения = ВариантыОтображения.Диаграмма 
		...
		КоманднаяПанельКомандПоказателя.Заголовок = НСтр("ru='Команды показателя';en='Indicator Commands'");
		...
		// ПодменюВсеДействия.Заголовок = НСтр("ru='Действия';en='Actions'");
		...
		Если КоличествоВариантовОтчетов Тогда
			...
			// ПодменюОтчеты.Заголовок = НСтр("ru='Отчеты';en='Reports'");
...
Процедура НарисоватьДиаграмму(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаСостояния__" + ИдентификаторЭлементаФормы, ОписаниеТипов, "", НСтр("ru='Картинка состояния';en='Status Picture'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаТренда__" + ИдентификаторЭлементаФормы, ОписаниеТипов, "", НСтр("ru='Картинка тренда';en='Trend picture'")));
	...
	Если НЕ ЗначениеЗаполнено(Динамика.Тренд) Тогда
		Тренд = НСтр("ru='Без изменений';en='No changes'");
	ИначеЕсли Динамика.Тренд > 0 Тогда
		Тренд = НСтр("ru='Рост';en='Growth'");
	Иначе
		Тренд = НСтр("ru='Падение';en='A fall'");
	КонецЕсли;
	...
	Если ОшибкаРасчета Тогда
		...
		Иначе
			ОписаниеОшибки = НСтр("ru='Ошибка расчета показателя';en='Error calculating key figure'");
		КонецЕсли;
		Диаграмма = ПолучитьДиаграммуПриОшибкахРасчета(Диаграмма, ОписаниеОшибки, ЦветаСтиля.ПоясняющийОшибкуТекст);
		
	Иначе
		// Если Не ПустойИсточникДанных Тогда
			Диаграмма = ЭтотОбъект["Диаграмма__" + ИдентификаторЭлементаФормы];
...
Процедура НарисоватьКратко(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КраткоеНаименование__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Краткое наименование';en='Short name'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("Значение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение';en='Value'")));
	...
	Если ПоказыватьТекущееЗначениеИДеталиПоказателя И СтруктураНастроек.ВариантОтображенияДеталей = "ПоказыватьТекущееЗначениеИДетали" Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ОтнИзменение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Отн. изменение';en='Rel. change'")));
		МассивРеквизитов.Добавить(Новый РеквизитФормы("АбсИзменение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Абс. изменение';en='Abs. change'")));
	КонецЕсли;
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаСостояния__" + ИдентификаторЭлементаФормы, ОписаниеТиповКартинок, "", НСтр("ru='Картинка состояния';en='Status Picture'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаТренда__" + ИдентификаторЭлементаФормы, ОписаниеТиповКартинок, "", НСтр("ru='Картинка тренда';en='Trend picture'")));
	
	ИзменитьРеквизиты(МассивРеквизитов);
	
	// Добавим элементы формы
	ГруппаСтатусов = ГоризонтальнаяГруппа("Статусы__" + ИдентификаторЭлементаФормы, НСтр("ru='Статусы';en='Statuses'"), ГруппаЭлементов);
...
//	
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦельЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Цель';en='Target'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦелевойТрендЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Целевой тренд';en='Target Trend'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦелевыеЗначенияЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Целевые значения';en='Target Values'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическоеСостояниеЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Состояние';en='State'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическийТрендЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Тренд';en='Trend'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическоеЗначениеЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Заголовок значения';en='Value Header'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ОтветственныйЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Ответственный';en='Responsible'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФормулаЗаголовок__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Формула';en='Formula'")));
//	
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦельЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение цели';en='Goal Value'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦелевойТрендЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение целевого тренда';en='Value of the target trend'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ЦелевыеЗначенияЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значения целевых значений';en='Values of target values'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическоеСостояниеЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение состояния';en='Status value'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическийТрендЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение тренда';en='Trend value'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФактическоеЗначениеЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение';en='Value'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ОтветственныйЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение ответственного';en='Meaning of responsible'")));
//	МассивРеквизитов.Добавить(Новый РеквизитФормы("ФормулаЗначение__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Значение формулы';en='The meaning of formula'")));
//	
//	ИзменитьРеквизиты(МассивРеквизитов);
//	
...
//	
//	// Заполним значения созданных полей
//	ЭтотОбъект["ЦельЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Цель:';en='Goal:'");
//	ЭтотОбъект["ЦелевойТрендЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Целевой тренд:';en='Target Trend:'");
//	ЭтотОбъект["ЦелевыеЗначенияЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Целевые значения:';en='Target values:'");
//	
...
//	КонецЕсли;
//	
//	ЭтотОбъект["ФактическоеСостояниеЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Состояние:';en='State:'");
//	ЭтотОбъект["ФактическийТрендЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Тренд:';en='Trend:'");
//	ЭтотОбъект["ФактическоеЗначениеЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Значение:';en='Value:'");
//	ЭтотОбъект["ФормулаЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Формула:';en='Formula:'");
//	ЭтотОбъект["ОтветственныйЗаголовок__" + ИдентификаторЭлементаФормы]	= НСтр("ru='Ответственный:';en='Responsible:'");
//	
...
Процедура НарисоватьТаблицу(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("НаименованиеПоказателя__" + ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Наименование показателя';en='Indicator name'")));
...
//	//	
//	// Иначе
//	//	ИмяВидаПоиска = НСтр("ru='АнализируемыйПоказатель';en='Analyzed Indicator'");
//	//	
//	// КонецЕсли;
//	//
...
Процедура СоздатьБазовуюСтруктуруЭлементовФормы(СоставМонитора)
	...
	ГруппаПоказатели = ВертикальнаяГруппа("ГруппаПоказатели", НСтр("ru='Показатели';en='Indicators'"), Элементы.ОсновнаяСтраницаМонитора);
	
	Если РежимАнализаВзаимосвязанных Тогда
	Иначе
		ГруппаПоказателиКратко = ВертикальнаяГруппа("ГруппаПоказателиКратко", НСтр("ru='Показатели (кратко)';en='Indicators (briefly)'"), ГруппаПоказатели);
		ГруппаПоказателиПодробно = ВертикальнаяГруппа("ГруппаПоказателиПодробно", НСтр("ru='Показатели (подробно)';en='Indicators (in detail)'"), ГруппаПоказатели);
...
&НаСервере
Функция ПолучитьМассивНеудаляемыхРеквизитов()
	...
	// Реквизиты рабочего стола.
	НеудаляемыеРеквизиты.Добавить("ПоддерживаетРабочийСтол");
	НеудаляемыеРеквизиты.Добавить("CRM_НастройкиРабочегоСтола");
	НеудаляемыеРеквизиты.Добавить("CRM_НастройкиРабочегоСтолаТаблицаНапоминаний");
	НеудаляемыеРеквизиты.Добавить("CRM_ЛоготипРабочегоСтола");
	//НеудаляемыеРеквизиты.Добавить("скТумблерРабочегоСтола");
	НеудаляемыеРеквизиты.Добавить("скПолеHTMLНапоминаний");
	НеудаляемыеРеквизиты.Добавить("скПолеHTMLЗаметок");
...
Процедура ОбновитьСоставМонитора(ПолученныйСоставМонитора)
	...
	НеудаляемыеРеквизиты = ПолучитьМассивНеудаляемыхРеквизитов();
	...
	Если МассивСуществующихРеквизитов.Количество() > 0 Тогда
		...
		Для Каждого СуществующийРеквизит Из МассивСуществующихРеквизитов Цикл 
			Если НеудаляемыеРеквизиты.НайтиПоЗначению(СуществующийРеквизит.Имя) = Неопределено Тогда
	...
	Для Каждого Команда Из Команды Цикл 
		...
		Если Найти(ИмяКоманды, "Обновить") 
			...
			ИЛИ Найти(ИмяКоманды, "КнопкаТумблера")
			ИЛИ Найти(ИмяКоманды, "НастройкиРабочегоСтола")
			ИЛИ Найти(ИмяКоманды, "скИндикаторНапоминаний")
			ИЛИ Найти(ИмяКоманды, "Напоминания")
			ИЛИ Найти(ИмяКоманды, "Заметки")
			ИЛИ Найти(ИмяКоманды, "CRM_Открыть")
...
Процедура ПолучитьНастройкиПоказателей()
	...
	|ГДЕ
	|	CRM_ИспользуемыеКлючевыеПоказатели.Менеджер = &Менеджер
