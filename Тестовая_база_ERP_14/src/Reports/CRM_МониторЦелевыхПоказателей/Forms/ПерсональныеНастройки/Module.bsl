...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДеревоШаблоновПоказателейВыбран
...
Процедура ПередЗакрытием_Продолжение()
	...
	ПоказатьВопрос(Оповещение, НСтр("ru='Настройки были изменены. Записать изменения настроек?';en='Settings were changed. Save changes?'"), РежимДиалогаВопрос.ДаНетОтмена);
КонецПроцедуры
