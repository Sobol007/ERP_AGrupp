...
Процедура ИзменениеСхемыКомпоновкиДанныхНаСервере(Знач Схема = Неопределено) Экспорт
	...
	Если НЕ Субконто1СтатьяЗатрат.Значение Тогда
		Для Каждого НаборДанных Из Схема.НаборыДанных.ОсновнойНаборДанных.Элементы Цикл
			ТекстЗапроса = НаборДанных.Запрос;
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			"// этот фрагмент удаляется, если выбран вариант, где Субконто1 это не СтатьяЗатрат
			|		КОГДА &Субконто1СтатьяЗатрат
			|				И ХозрасчетныйОбороты.Субконто2 = ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.МатериальныеЗатраты)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыРасходовНУ.МатериальныеРасходы)
			|		КОГДА &Субконто1СтатьяЗатрат 
			|				И ХозрасчетныйОбороты.Субконто2 = ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.СдельнаяОплатаТруда)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыРасходовНУ.ОплатаТруда)
			|		КОГДА &Субконто1СтатьяЗатрат 
			|				И ХозрасчетныйОбороты.Субконто2 = ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.Работы)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыРасходовНУ.ПрочиеРасходы)
			|// конец фрагмента",
			"");
			НаборДанных.Запрос = ТекстЗапроса;
		КонецЦикла;
