...
Процедура ПолучитьДанныеПоКонтрольнымТочкам()
	...
	|	КонтрольныеТочки.ПлановаяДатаНачала КАК Дата,
	|	КонтрольныеТочки.ФактическаяДатаОкончания КАК ДатаЗакрытия,
	|	КонтрольныеТочки.Этап			КАК Этап
	|	
	|ИЗ
	|	Документ.CRM_ЭтапКалендарногоПлана КАК КонтрольныеТочки
	|	
	|ГДЕ
	|	КонтрольныеТочки.Проект = &Проект
	|	И НЕ КонтрольныеТочки.ПометкаУдаления
	|	И КонтрольныеТочки.Статус = ЗНАЧЕНИЕ(Перечисление.CRM_СтатусыЭтаповКалендарногоПлана.Запланирована)
	|	
	|");
	Запрос.УстановитьПараметр("Проект", Проект);
	РезультатЗапроса = Запрос.Выполнить();
...
Процедура ВывестиДанныеПоПроекту()
	...
	Область.Параметры.ПроектПроцесс = ?(ДанныеПоПроекту.ЭтоПроект, НСтр("ru='проекта';en='project'"), НСтр("ru='процесса';en='process'"));
	...
	Если ДанныеПоПроекту.Завершен Тогда
		Область.Параметры.СтрокаЗавершен = "(" + НСтр("ru='завершен';en='completed'") + ")";
	КонецЕсли;
	...
	Область.Параметры.Заголовок = НСтр("ru='Данные проекта';en='Data of the project'");
	...
	Если ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаНачала) И ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаОкончания) Тогда
		Область.Параметры.ЗаголовокПлановаяДатаНачалаОкончания = НСтр("ru='Плановая дата начала/окончания';en='Planned date of the beginning/the termination'");
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаНачала) И Не ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаОкончания) Тогда
		Область.Параметры.ЗаголовокПлановаяДатаНачалаОкончания = НСтр("ru='Плановая дата начала';en='Planned date started'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаНачала) И ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаОкончания) Тогда
		Область.Параметры.ЗаголовокПлановаяДатаНачалаОкончания = НСтр("ru='Плановая дата окончания';en='Planned expiry date'");
	...
	Иначе
		...
		Область.Параметры.СтрокаПлановаяДатаНачалаОкончания = НСтр("ru='Даты не указаны';en='Dates was not specif'");
		
	КонецЕсли;
	
	Область.Параметры.Планируемый = ?(ДанныеПоПроекту.Планируемый, НСтр("ru='Да';en='Yes'"), НСтр("ru='Нет';en='No'"));
...
Процедура ВывестиДанныеПоИсполнителю()
	...
	Область = Макет.ПолучитьОбласть("ЗаголовокПодраздела");
	Область.Параметры.Заголовок = НСтр("ru='Исполнитель';en='Executor'");
	...
	Если СтруктураДанные.Свойство("КИИсполнителя") Тогда
		...
		Если ТабДокументКИ.ВысотаТаблицы > 0 Тогда
			...
			Область.Параметры.Заголовок = НСтр("ru='Контактная информация исполнителя';en='Contact information of the executor'");
			ТабДокДИ.Вывести(Область);
...
Процедура ВывестиДанныеПоЗаказчику()
	...
	Область = Макет.ПолучитьОбласть("ЗаголовокПодраздела");
	Область.Параметры.Заголовок = НСтр("ru='Заказчик';en='Customer'");
	ТаблицаОтчета.Вывести(Область);
	
	ТаблицаОтчета.НачатьГруппуСтрок();
	
	Область = Макет.ПолучитьОбласть("ДанныеСтрока");
	Область.Параметры.Заголовок = НСтр("ru='Клиент';en='Customer'");
	...
	Если СтруктураДанные.Свойство("КИЗаказчика") Тогда
		...
		Если ТабДокументКИ.ВысотаТаблицы > 0 Тогда
			Область = Макет.ПолучитьОбласть("ЗаголовокПодраздела");
			Область.Параметры.Заголовок = НСтр("ru='Контактная информация заказчика';en='Contact information of customer'");
			ТаблицаОтчета.Вывести(Область);
	...
	Если СтруктураДанные.Свойство("ДанныеПоКонтактномуЛицуЗаказчика") Тогда
		...
		Если ДанныеПоКонтактномуЛицуЗаказчика.Следующий() Тогда
			...
			Если ТабДокументКИ.ВысотаТаблицы > 0 Тогда
				Область = Макет.ПолучитьОбласть("ЗаголовокПодраздела");
				Область.Параметры.Заголовок = НСтр("ru='Контактное лицо заказчика';en='Contact person of customer'") + ": " + ПредставлениеКонтактногоЛица;
			...
			Иначе
				Область = Макет.ПолучитьОбласть("ДанныеСтрока");
				Область.Параметры.Заголовок = НСтр("ru='Контактное лицо';en='Contact person'");
...
Процедура ВывестиДанныеПоЭтапам(ТабДокументТаблицаЭтапов = Неопределено, ДеревоЭтапы = Неопределено, Знач УровеньВложенности = -1,
	...
	Если ДеревоЭтапы = Неопределено Тогда
		...
		Область.Параметры.Заголовок = НСтр("ru='Этапы проекта';en='Project Stages'");
		ТаблицаОтчета.Вывести(Область);
		
		ТаблицаОтчета.НачатьГруппуСтрок();
		
		Область = Макет.ПолучитьОбласть("ДанныеСтрокаШирокая");
		Область.Параметры.Заголовок = НСтр("ru='Плановые затраты по этапам';en='Planned expenses on stages'");
		...
		Иначе
			Область.Параметры.Значение = НСтр("ru='Отсутствуют';en='No'");
...
Процедура ВывестиДанныеПоКонтрольнымТочкам()
	...
	Область = Макет.ПолучитьОбласть("ЗаголовокТЧ");
	Область.Параметры.Заголовок = НСтр("ru='Задачи и контрольные точки';en='Tasks and control points'");
	ТаблицаОтчета.Вывести(Область);
...
Процедура ВывестиДанныеПоЗадачам()
	...
	Область = Макет.ПолучитьОбласть("ЗаголовокТЧ");
	Область.Параметры.Заголовок = НСтр("ru='Текущие задачи';en='Current tasks'");
	ТаблицаОтчета.Вывести(Область);
