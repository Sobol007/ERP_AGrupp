...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НоваяСтрока.Представление = НСтр("ru='Фамилия';en='Surname'");
	НоваяСтрока = ТаблицаПорядокРеквизитов.Добавить();
	НоваяСтрока.Имя = "Имя";
	НоваяСтрока.Представление = НСтр("ru='Имя';en='Name'");
	НоваяСтрока = ТаблицаПорядокРеквизитов.Добавить();
	НоваяСтрока.Имя = "Отчество";
	НоваяСтрока.Представление = НСтр("ru='Отчество';en='Patronymic'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОбратныйВызов, НСтр("ru='Данные были изменены. Закрыть форму?';en='Data has been changed. Close the form?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;	
КонецПроцедуры
...
Процедура КомандаДалее(Команда)
	...
	ИначеЕсли Элементы.ПанельОсновная.ТекущаяСтраница = Элементы.СтраницаВыборПорядкаРеквизитов Тогда
		...
		Если Не ОбрабатыватьПартнеров И Не ОбрабатыватьКонтактныеЛица Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не выбран ни один справочник!';en='Any reference manual are not selected!'"));
			Возврат;
		...
		Если ТаблицаПартнеры.Количество() > 0 Или ТаблицаКЛ.Количество() > 0 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Перечитать данные? Все изменения будут отменены.';en='Reread data? All changes will be cancelled.'"), РежимДиалогаВопрос.ДаНет);
		Иначе	
			ЗаполнитьТаблицы();
	...
	ИначеЕсли Элементы.ПанельОсновная.ТекущаяСтраница = Элементы.СтраницаРедактирование Тогда
		...
		Если Не бПартнерыВыбраны И Не бКЛВыбраны Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не выбран ни один элемент справочника!';en='Any element of the reference manual are not selected!'"));
			Возврат;
		КонецЕсли;
