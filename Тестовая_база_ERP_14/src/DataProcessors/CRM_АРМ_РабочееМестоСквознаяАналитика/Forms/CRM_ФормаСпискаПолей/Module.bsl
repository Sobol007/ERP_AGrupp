...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Режим", Режим) ИЛИ НЕ Параметры.Свойство("АдресСхемы") ИЛИ НЕ Параметры.Свойство("АдресНастроек") Тогда
		...
		Возврат;
	КонецЕсли;
	Параметры.Свойство("СуществующиеПоля", СуществующиеПоля);
	...
	Если Режим="ПоляГруппировки" Тогда
		Заголовок = НСтр("en='Grouping fields';ru='Поля группировки'");
		ДобавитьПоляГруппировок();
	ИначеЕсли Режим="ПоляВыбор" Тогда
		Заголовок = НСтр("en='Доступные поля';ru='Доступные поля'");
		ДобавитьПоляВыбор();
	ИначеЕсли Режим="ПоляОтбора" Тогда
		Заголовок = НСтр("en='Filter fields';ru='Поля фильтрации'");
		ДобавитьФильтры();
	КонецЕсли; 
	...
	Для каждого Поле Из ДоступныеПоля Цикл
		...
		Для каждого Реквизит Из Поле.Элементы Цикл
			...
			Если ИмяРеквизита=ИмяПредыдущегоРеквизита Тогда
				Продолжить;
			КонецЕсли; 
			Если НЕ СуществующиеПоля.НайтиПоЗначению(ИмяРеквизита)=Неопределено Тогда
				Продолжить;
			КонецЕсли;
		...
		КонецЦикла;
		ВСуществующих = НЕ СуществующиеПоля.НайтиПоЗначению(ИмяПоля)=Неопределено;
...
&НаСервере
Процедура ДобавитьПоляВыбор()
	
	ДоступныеПоля = Компоновщик.Настройки.ДоступныеПоляВыбора.Элементы;
	ИмяПредыдущегоПоля = ""; 
	Для каждого Поле Из ДоступныеПоля Цикл
...
Процедура ДобавитьФильтры()
	...
	Для каждого Поле Из Компоновщик.Настройки.Выбор.Элементы Цикл
		...
		СтрПоле = Поля.ПолучитьЭлементы().Добавить();
		СтрПоле.Поле = ИмяПоля;
		СтрПоле.Представление = ?(ЗначениеЗаполнено(Поле.Заголовок),Поле.Заголовок,ИмяПоля);
		...
		//				Если НЕ СуществующиеПоля.НайтиПоЗначению(ИмяРеквизитаТЧ)=Неопределено Тогда
		//					Продолжить;
		//				КонецЕсли; 
