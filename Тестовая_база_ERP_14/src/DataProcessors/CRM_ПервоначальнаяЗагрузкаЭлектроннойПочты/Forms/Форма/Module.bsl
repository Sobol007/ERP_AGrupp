...
Процедура КомандаПолучить(Команда)
	...
	Иначе
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Будут получены все письма с сервера. Продолжить?';en='All messages from the server will be received. Continue?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура КомандаПрервать(Команда)
	...
	ПоказатьПредупреждение(,НСтр("ru='Получение электронной почты прервано пользователем!';en='Obtaining of e-mail is interrupted by the user!'"));
КонецПроцедуры
...
Функция ПолучитьИдентификаторы(УчетнаяЗапись)
	...
	Если Идентификаторы.Количество() = 0 Тогда
		// Если на сервере сообщений нет, то удалим все записи по учетной записи 
		// в регистре сведений ИдентификаторыПолученныхЭлектронныхПисем
...
Процедура ОбновитьДекорациюПрофиль(УчетнаяЗапись, КоличествоПолучено, КоличествоВсего)
	...
	Если УчетнаяЗапись = Неопределено тогда
		СтрЗаголовок = СтрЗаголовок + НСтр("ru='Загрузка завершена';en='Loading is completed'") + ": ";
	ИначеЕсли ЗначениеЗаполнено(ПараметрУчетнаяЗапись) Тогда
		СтрЗаголовок = СтрЗаголовок + НСтр("ru='Получение почты';en='Obtaining of mail'") + СтрокаПапкиУчетнойЗаписи + ": ";
	Иначе
		СтрЗаголовок = СтрЗаголовок + Строка(УчетнаяЗапись) + СтрокаПапкиУчетнойЗаписи + ": ";
	КонецЕсли;
	Элементы.ДекорацияПрофиль.Заголовок = СтрЗаголовок
		+ НСтр("ru='получено';en='it is receiv'") + " " + Формат(КоличествоПолучено, "ЧН=0; ЧГ=")
		+ " " + НСтр("ru='из';en='of'") + " " + Формат(КоличествоВсего, "ЧН=0; ЧГ=");
	//
