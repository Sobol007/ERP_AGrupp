...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ДокументыБезИнтереса", НСтр("ru = 'Документы без интереса'"));
	...
	Если НЕ ЗначениеЗаполнено(ГруппироватьПоИнтересам) Тогда
		// это первый запуск
		ГруппироватьПоИнтересам = Нстр("ru='Да';en='Yes'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;		
	CRM_ТегиКлиент.БыстрыйОтборИзменение(ЭтотОбъект, ТекущийВидЖурнала, ОтборПоТипуОбъекта, ОтборОтветственный, ?(Элементы.ОтборПартнер.Видимость, ОтборПартнер, Неопределено));
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Форма.ПризнакФормаОткрытаПоПартнеру И НЕ Форма.Элементы.ОтборПартнер.Видимость
		...
		Если ЗначениеЗаполнено(Форма.ОтборКонтактноеЛицо) Тогда
			ЗаголовокФормы = ЗаголовокФормы + " " + НСтр("ru='по контакту';en='by contact'") + " """ + Строка(Форма.ОтборКонтактноеЛицо) + """" + " " + НСтр("ru='клиента';en='of customer'") + " """ + Строка(Форма.ОтборПартнер) + """";
		Иначе
...
Процедура УстановитьГруппировкуПоИнтересу()
	Если ПризнакФормаОткрытаПоПартнеру Тогда
		...
		Если ПервыйВызов Тогда
			...
			ТипИнтерес = ПланыВидовХарактеристик.CRM_ОбъектыЖурналаДокументов.НайтиПоНаименованию(НСтр("ru='Интерес';en='Lead'"));
		...
		Если ГруппироватьПоИнтересам = Нстр("ru='Да';en='Yes'") Тогда
