...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийВидЖурнала = Справочники.CRM_ГруппыУниверсальногоЖурнала.Контакты;
	...
	Если Параметры.Свойство("ОткрытаФормаЖурналаВзаимодействия")
		И Параметры.ОткрытаФормаЖурналаВзаимодействия <> Ложь Тогда
		ОткрытаФормаЖурналаВзаимодействия = Истина;
	КонецЕсли;
	...
	ПоказатьПолеОтображенияСодержания = Истина;
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если ОткрытаФормаЖурналаВзаимодействия Тогда
		
		СохраненныйВидЖурнала = Настройки.Получить("ТекущийВидЖурнала");
		Настройки.Удалить("ТекущийВидЖурнала");
		
		СохраненныйПартнер = Настройки.Получить("ОтборПартнер");
		Настройки.Удалить("ОтборПартнер");
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	...
	Если ОткрытаФормаЖурналаВзаимодействия Тогда
		...
		Настройки.Вставить("ТекущийВидЖурнала", СохраненныйВидЖурнала);
		Настройки.Вставить("ОтборПартнер", СохраненныйПартнер);
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Копирование Тогда
		...
		Если ТипЗнч(ОбъектСсылка) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
			...
			Иначе
				ПоказатьПредупреждение(, НСтр("ru='Копирование задач, кроме личных, запрещено!';en='Copying tasks other than personal is prohibited!'"));
			КонецЕсли;
		Иначе
	...
	Иначе
		...
		Иначе
			Если ЗначениеЗаполнено(ТекущийВидЖурнала) Тогда
				...
				Для каждого ОбъектЖурнала из ДоступныеДляОтбораОбъекты Цикл
					Если ДоступныеДляСозданияОбъекты.НайтиПоЗначению(ОбъектЖурнала.Значение)<>Неопределено Тогда
						СписокДоступных.Добавить(ОбъектЖурнала.Значение);
					КонецЕсли;
				КонецЦикла;
				Если СписокДоступных.Количество() = 1 Тогда
					ВыбранноеЗначение = СписокДоступных[0].Значение;
					СписокПередНачаломДобавленияЗавершение(ВыбранноеЗначение, Неопределено);
					Возврат;
				КонецЕсли;
			Иначе
				СписокДоступных = ДоступныеДляСозданияОбъекты.Скопировать();
			КонецЕсли;
			СтруктураПараметров = Новый Структура("СписокОбъектов", СписокДоступных);
...
Процедура КомандаСоздатьНаОсновании(Команда)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Для данного объекта не предусмотрен ввод на основании.';en='For this object there is no input based on.'"),5,);
		Возврат;
	КонецЕсли;
	...
	Если ТекДанные.ПометкаУдаления Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, """" + Строка(ТекДанные.Объект) + """ " + НСтр("ru='помечен на удаление. Продолжить?';en='it is mark on deleting. Continue?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
	Иначе
...
Процедура КомандаСоздатьНаОснованииПродолжение(Ответ, ДополнительныеПараметры) Экспорт
	...
	СтруктураПараметров = Новый Структура("СписокОбъектов,Заголовок", СписокВводимыхНаОснованииОбъектов.Скопировать(), НСтр("ru='Создать на основании';en='Create on Base'"));
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Иначе
		ЗаголовокФормы = НСтр("ru='Журнал документов';en='Documents Log'");
	КонецЕсли;
	Если Форма.ПризнакФормаОткрытаПоПартнеру И НЕ Форма.Элементы.ОтборПартнер.Видимость
		...
		ЗаголовокФормы = ЗаголовокФормы + " " + НСтр("ru='по клиенту';en='on customer'") + " """ + Строка(Форма.ОтборПартнер) + """";
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеДляСозданияОбъекты()
	...
	Иначе
		...
		УдаляемоеЗначение = ДоступныеДляСозданияОбъекты.НайтиПоЗначению(ПланыВидовХарактеристик.CRM_ОбъектыЖурналаДокументов.НайтиПоНаименованию("Взаимодействие"));
		Если УдаляемоеЗначение <> Неопределено Тогда
			ДоступныеДляСозданияОбъекты.Удалить(УдаляемоеЗначение);
		КонецЕсли;	
	КонецЕсли;
	...
	// Убираем Задачи, События, Сделки, Мероприятия и Звонки
	МассивТипов = Новый Массив();
	...
	МассивТипов = Новый Массив();
	МассивТипов.Добавить(Тип("ДокументСсылка.удалитьCRM_Мероприятие"));
	ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
	МассивСсылкаПВХ = ПолучитьМассивЭлементовПВХОбъектыЖурналаПоОписаниюТипов(ОписаниеТипов);
	Для Каждого СсылкаПВХ Из МассивСсылкаПВХ Цикл
		МассивОбъектыЗапрещенныеДляСоздания.Добавить(СсылкаПВХ);
