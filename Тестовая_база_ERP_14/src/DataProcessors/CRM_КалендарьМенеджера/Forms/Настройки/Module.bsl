...
Процедура ПериодАвтообновленияНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	СписокВыборка.Добавить(1, НСтр("ru='1';en='1'"));
	СписокВыборка.Добавить(2, НСтр("ru='2';en='2'"));
	СписокВыборка.Добавить(5, НСтр("ru='5';en='5'"));
	СписокВыборка.Добавить(10, НСтр("ru='10';en='10'"));
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокПредставлениеСобытияВКалендаре
...
Процедура КомандаПоУмолчанию(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Будут загружены настройки по умолчанию. Продолжить?';en='The default settings will be unloaded. Continue?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет, Заголовок);
КонецПроцедуры
...
Процедура ОбновитьПримерПредставленияСобытия()
	...
	СтрТема = НСтр("ru='Обсуждение заказа';en='Discussion of the order'");
	СтрКлиент = НСтр("ru='Пилигрим ООО';en='Piligrim Ltd.'");
	СтрПользователь = НСтр("ru='(Абдулов Юрий Владимирович)';en='(Abdulov Yuri Vladimirovich)'");
...
Функция ПроверитьСохранить()
	...
	Если ОграничениеВремениС > ОграничениеВремениПо Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время начала ограничения больше времени окончания ограничения!';en='Starting time of restriction are more than ending time of restriction!'"), СтатусСообщения.Внимание);
		Результат = Ложь;
	КонецЕсли;
	
	Если РабочийДеньС > РабочийДеньПо Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время начала рабочего времени больше времени окончания рабочего времени!';en='Starting time of working hours are more than ending time of working hours!'"), СтатусСообщения.Внимание);
		Результат = Ложь;
	КонецЕсли;
