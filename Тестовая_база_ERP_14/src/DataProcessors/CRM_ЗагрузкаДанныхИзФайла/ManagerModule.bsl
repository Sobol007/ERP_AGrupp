...
Функция СформироватьДеревоРеквизитов(ИмяМакета, Отбор) Экспорт
...
Функция СоздатьОбъект(ИмяМетаданных, ЗначенияПолей, ЗначенияПоУмолчанию = Неопределено, МассивКИ = Неопределено, МассивДопРеквизитов = Неопределено, Комментарий="") Экспорт
...
Процедура ЗагрузитьКлиентов(Параметры, АдресХранилища=Неопределено) Экспорт
	...
	КомментарийЗагрузки = " ("+ НСтр("ru = 'Импортировано'")+" "+ Формат(ТекущаяДатаСеанса, "ДФ='dd.MM.yyyy HH:mm'") +")";
	КомментарийОбновления = " ("+ НСтр("ru = 'Обновлено'")+" "+ Формат(ТекущаяДатаСеанса, "ДФ='dd.MM.yyyy HH:mm'") +")";
		...
		Если КлиентСсылка = Неопределено Тогда
			...
			Для каждого ТипОтношений из Параметры.CRM_ВсеТипыОтношений Цикл
				Если ТипОтношений.Пометка Тогда
					СтруктураРеквизитов.Вставить(ТипОтношений.Значение, Истина);
				КонецЕсли;
			КонецЦикла;
			КлиентСсылка = Обработки.CRM_ЗагрузкаДанныхИзФайла.СоздатьОбъект("Справочник.Партнеры", СтруктураРеквизитов, СтруктураКлиентаПоУмолчанию, МассивКИ, МассивДопРеквизитов, КомментарийЗагрузки);
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Загружено новых клиентов: %1
                          ...
                          |Создано новых интересов клиентов: %3'"), КоличествоНовыхКлиентов, КоличествоНовыхКонтактов, КоличествоНовыхИнтересов);
	
	ПоместитьВоВременноеХранилище(Новый Структура("Заголовок, ТекстСообщения", НСтр("ru='Загрузка данных завершена';en='Data import completed'"), ТекстСообщения), АдресХранилища);
