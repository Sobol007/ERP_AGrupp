...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.CRM_ИспользоватьКлассификаторы" ИЛИ РеквизитПутьКДанным = "" Тогда
		Элементы.СправочникЗначенияКлассификаторов.Доступность = НаборКонстант.CRM_ИспользоватьКлассификаторы;
	КонецЕсли;
КонецПроцедуры
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСкидкиНаценки() 
	...
	Пока ВыборкаВидыСкидокНаценок.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			Сообщение.Текст = НСтр("ru='В базе используются виды скидок, наценок! Снятие опции запрещено!';en='Types of discounts, margins are used in the database! Option de-selection is not allowed!'");	
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьХарактеристики() 
	...
	Если ЕстьДвижения Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='В базе есть движения по характеристикам! Снятие флага запрещено!';en='Database reflects transactions of the characteristics! Check box de-selection is not allowed!'");	
...
Процедура CRM_СквознаяАналитикаПриИзменении(Элемент)
	...
	Если НаборКонстант.CRM_СквознаяАналитика Тогда
		// Установим использование набора дополнительных реквизитов и создадим доп. реквизиты
		УстановитьИспользованиеДопРеквизитов();
	КонецЕсли;
	...
	Если НаборКонстант.CRM_СквознаяАналитика Тогда
		Лицензии = CRM_СистемаСквознойАналитикиЛицензированиеСервер.ПроверитьЛицензиюМодуляСквознойАналитики(Ложь);
		
		Если Лицензии.Свойство("Ошибка") И ЗначениеЗаполнено(Лицензии.Ошибка) Тогда
			//НаборКонстант.CRM_СквознаяАналитика = Ложь;
			
			ТекстВопроса = Лицензии.Ошибка;
			
			КнопкиВыбора = Новый СписокЗначений();
			КнопкиВыбора.Добавить("ОткрытьСЛК", "Открыть менеджер СЛК");
			КнопкаПоУмолчанию = КнопкиВыбора.Добавить("Отмена");

			Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаОткрытияСЛК", ЭтаФорма);
			ПоказатьВопрос(Оповещение, ТекстВопроса, КнопкиВыбора, 15, "Отмена", "ВНИМАНИЕ!", "Отмена");
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаОткрытияСЛК(РезультатВопроса, ДополнительныеПараметры) Экспорт

	Если РезультатВопроса = "ОткрытьСЛК" Тогда
...
Функция ПриИзмененииРеквизитаСквознаяАналитика(ИмяЭлемента)
	...
	Если НаборКонстант.CRM_СквознаяАналитика Тогда
		CRM_СистемаСквознойАналитикиСервер.УстановитьПредопределенныеЭлементыИнтерваловВремени();
	КонецЕсли;
...
&НаСервере
Функция ПроверкаСозданногоДопРеквизита(Имя)
  
    Возврат ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",Имя).Пустая();
    
КонецФункции

&НаКлиенте
Процедура УстановитьИспользованиеДопРеквизитов()
  
  ЗаполнитьИспользованиеНабораДополнительныхРеквизитов();
  
  Если ПроверкаСозданногоДопРеквизита("ИДСквознаяАналитика")  Тогда
    
    Структура = Новый Структура;
    Структура.Вставить("Имя","ИДСквознаяАналитика");
    Структура.Вставить("Заголовок", "ID");
  ...
  Если ПроверкаСозданногоДопРеквизита("ИДМаркетинговоеМероприятие") Тогда
    
    Структура = Новый Структура;
    Структура.Вставить("Имя","ИДМаркетинговоеМероприятие");
    Структура.Вставить("Заголовок", "ID");
    Структура.Вставить("НаборСвойств",ПредопределенноеЗначение("Справочник.НаборыДополнительныхРеквизитовИСведений.Справочник_МаркетинговыеМероприятия"));
    Структура.Вставить("ТипЗначения", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки("20")));
    Структура.Вставить("Доступен",Истина);
    Структура.Вставить("Виден",Истина);
...
&НаСервере
Процедура ЗаполнитьИспользованиеНабораДополнительныхРеквизитов()
	
	CRM_Заявка = Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_CRM_Заявка.ПолучитьОбъект();
	
	CRM_Заявка.Используется = Истина;
