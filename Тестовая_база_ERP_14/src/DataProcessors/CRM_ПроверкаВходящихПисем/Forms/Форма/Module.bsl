...
Процедура ПеребратьПисьма(Команда)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Обработка завершена';en='Processing completed'"));
		...
		Пока ВыборкаПисем.Следующий() Цикл
			Попытка
				...
				Если НЕ Тема = ПисьмоОбъект.Тема Тогда
					ТекстСообщения_1 = Символы.ПС + НСтр("ru='Скорректирована тема письма.';en='The subject of the letter are correct.'");
				...
				Если НЕ Текст = ПисьмоОбъект.Текст Тогда
					ТекстСообщения_2 = Символы.ПС + НСтр("ru='Скорректирован текст письма.';en='The text of the letter are correct.'");
				...
				Если НЕ СписокПолучателейПисьма = ПисьмоОбъект.СписокПолучателейПисьма Тогда
					ТекстСообщения_3 = Символы.ПС + НСтр("ru='Скорректирован список получателей письма.';en='The list of receivers of the letter are correct.'");
				...
				Если НЕ СписокПолучателейКопий = ПисьмоОбъект.СписокПолучателейКопий Тогда
					ТекстСообщения_4 = Символы.ПС + НСтр("ru='Скорректирован список получателей копий письма.';en='The list of receivers of copies of the letter are correct.'");
				...
				Если НЕ ОтправительПредставление = ПисьмоОбъект.ОтправительПредставление Тогда
					ТекстСообщения_5 = Символы.ПС + НСтр("ru='Скорректировано представление отправителя.';en='Corrected the view of the sender.'");
