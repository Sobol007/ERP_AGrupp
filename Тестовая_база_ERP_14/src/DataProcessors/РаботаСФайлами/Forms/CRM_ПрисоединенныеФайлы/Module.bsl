...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Файл" Тогда
		...
		Если Параметр.Свойство("ЭтоНовый") И Параметр.ЭтоНовый Тогда
			...
			// +CRM
			ОписаниеОповещения = Новый ОписаниеОповещения("ВыборКонтекстаФайлаЗавершение", ЭтотОбъект, Источник);
			ОткрытьФорму("Справочник.CRM_ВариантыКонтекстаФайла.ФормаВыбора", , ЭтотОбъект,,,, ОписаниеОповещения,);
			// -CRM
...
&НаКлиенте
Процедура ВыборКонтекстаФайлаЗавершение(Контекст, Источник) Экспорт
	Если Контекст = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьКонтекстДляФайлов(Источник, Контекст);
	Элементы.Список.Обновить();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьКонтекстДляФайлов(Источник, Контекст)
	
	Если ТипЗнч(Источник) = Тип("Массив") Тогда
		Для Каждого Файл из Источник Цикл
			ФайлОбъект = Файл.ПолучитьОбъект();
			ФайлОбъект.CRM_ВариантКонтекстаФайла = Контекст;
			ФайлОбъект.Записать();
		КонецЦикла;
	Иначе
		ФайлОбъект = Источник.ПолучитьОбъект();
		ФайлОбъект.CRM_ВариантКонтекстаФайла = Контекст;
		ФайлОбъект.Записать();
	КонецЕсли;
