...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВидМастера")
		...
		Заголовок = НСтр("ru='Действия по продвижению';en='Actions on advancement'") + ": " + ВидМастера;
		
		Элементы.ДекорацияНажмитеКнопкуДалее.Заголовок = НСтр("ru='Для продолжения нажмите кнопку ""Далее""';en='For continuation push the Further button'");
		Элементы.ВидМастера.Видимость = Ложь;
		
	Иначе
		
		Элементы.ДекорацияШагОбщаяИнформация.Заголовок = НСтр("ru='Общая информация';en='Common information'") + Символы.ПС + НСтр("ru='и выбор мастера';en='and choice of the master'");
		ВидМастера = Перечисления.CRM_ВидыМастераПоПродвижению.МастерТелефонногоОбзвона;
		Элементы.ДекорацияНажмитеКнопкуДалее.Заголовок = НСтр("ru='Для продолжения выберите вид мастера и нажмите кнопку ""Далее""';en='For continuation select a type of the master and push the Further button'");		
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		Если ЗадаватьВопросПриЗакрытии Тогда
			...
			ПоказатьВопрос(ОбратныйВызов, НСтр("ru='Завершить работу с мастером?';en='Complete operation with the master?'"), РежимДиалогаВопрос.ОКОтмена);
		КонецЕсли;
	КонецЕсли;	
КонецПроцедуры
...
Процедура КомандаДалее(Команда)
	...
	Если Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаСодержаниеДействия Тогда
		Если Элементы.Ответственный.Видимость И Не ЗначениеЗаполнено(Ответственный) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран Ответственный!';en='It is not selected Responsible!'"),, "Ответственный");
			Возврат;
		КонецЕсли;
		...
		Если (ВидМастера = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПоПродвижению.МастерEmailРассылки") ИЛИ
			...
			Если Не ЗначениеЗаполнено(Тема) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнена Тема';en='The Subject is not filled in'"),, "Тема");
				Возврат;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ТекстПисьмаФорматированныйДокумент.ПолучитьТекст()) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан текст сообщения!';en='The text of the message are not specif!'"),, "ФорматированныйДокументТекст");
				Возврат;
			КонецЕсли;
		...
		Если ВидМастера = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПоПродвижению.МастерSmsРассылки") И Не ЗначениеЗаполнено(Содержание) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан текст sms-сообщения!';en='The text of a sms are not specif!'"),, "содержание");
			Возврат;
	...
	ИначеЕсли Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаОтборКлиентов Тогда
		...
		Если НЕ ВключатьКлиентов И НЕ ВключатьКЛ Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо включить хотя бы один вид отборов!';en='It is necessary Include at least an one type of selections!'"));
			Возврат;
		КонецЕсли;
		...
		Если ВключатьКлиентов Тогда
			...
			Если НЕ ЕстьОтборыКлиентов Тогда
				...
				ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Нет установленных отборов по клиентам! В выборку будет включена вся клиентская база!
							   |Продолжить?'"), РежимДиалогаВопрос.ОКОтмена,,,НСтр("ru='Внимание!';en='Attention!'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаСписокКлиентов Тогда
		Если ТаблицаКонтактов.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не выбраны клиенты!';en='No customers selected!'"));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаРаспределениеПоОтветственным Тогда
		...
		Если ВыбранныеИсполнители.Количество() = 0 Тогда
			Если ВидРаспределенияПоИсполнителям = 1 Тогда
				ПоказатьПредупреждение(, НСтр("ru='Не выбрано ни одного ответственного!';en='Not selected any responsible!'"));
				Возврат;
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для некоторых клиентов не указан основной менеджер - нужно выбрать ответственного для клиентов, которым не назначен ответственный менеджер!';en='For some customers the main manager is not specified - you need to select the person responsible for customers who are not assigned to responsible manager!'"),, "ПользовательОтветственныйПоУмолчанию");
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ТекСтраница = Элементы.СтраницаСписокКлиентов Или ТекСтраница = Элементы.СтраницаРаспределениеПоОтветственным Тогда
		Если ТекСтраница = Элементы.СтраницаРаспределениеПоОтветственным Или ВидМастера <> ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПоПродвижению.МастерТелефонногоОбзвона") Тогда
			...
			Элементы.КомандаДалее.Заголовок = НСтр("ru=' Завершить ';en='Finish'");
			Элементы.КомандаДалее.Картинка = БиблиотекаКартинок.CRM_ЗеленаяГалка;
			
			ТекстЗаголовка = НСтр("ru = 'При нажатие на  кнопку «Завершить» будет открыт документ '");
...
Функция РаспределитьПартнеровПоИсполнителям()
	...
	Если МассивПользователей.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru='Не выбрано ни одного ответственного!';en='Not selected any responsible!'");
	КонецЕсли;
	Если ТаблицаКонтактов.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru='Не выбрано ни одного клиента!';en='It is not selected any customer!'");
	КонецЕсли;
...
Процедура ОчиститьОтборыКлиентов(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Очистить таблицу отборов?';en='Clear the selection table?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура ВставитьШаблонСМС(Команда)
    Если НЕ ПустаяСтрока(Содержание) Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Текст сообщения будет очищен. Продолжить?';en='The text of the message will be cleared. Continue?'"), РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.Отмена);
	Иначе	
...
Процедура ВставитьШаблонСМСПродолжение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПараметрыФормы.Вставить("РежимВыбора", Истина);
	ПараметрыФормы.Вставить("ЗакрыватьПриВыборе", Истина);
	ОткрытьФорму("Справочник.SMS4B_ШаблоныСообщений.Форма.ФормаВыбора", ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура АктуальностьСМСПриИзменении(Элемент)
	...
	Если АктуальностьСМС < НачалоОтправкиСМС + 900 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время актуальности сообщения не может быть меньше 15 минут от времени начала рассылки';en='Time of an urgency of the message could not be less than 15 minutes from starting time of mailing'"));
