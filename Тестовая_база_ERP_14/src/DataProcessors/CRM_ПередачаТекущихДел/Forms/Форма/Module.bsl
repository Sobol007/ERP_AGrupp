...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокТипов.Добавить(Новый Структура(КлючиСтруктуры, "Документ.CRM_Взаимодействие"				,"Партнер"				,"Содержание",""			,"Ответственный"));
	...
	Элементы.ФлагДобавлятьУровеньДоступа.Видимость = CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM();
...
Процедура КомандаДалее(Команда)
	...
	Если Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаОтбор Тогда
		Если НЕ ЗначениеЗаполнено(Объект.СотрудникПолучатель) Тогда
			ПоказатьПредупреждение(, НСтр("ru='Введите нового ответственного.';en='Enter the new responsible.'"));
			Возврат;
		КонецЕсли;
		Если Объект.СотрудникИсточник = Объект.СотрудникПолучатель Тогда
			ПоказатьПредупреждение(, НСтр("ru='Новый ответственный должен отличаться от старого!';en='The new responsible should differ from the old!'"));
			Возврат;
		КонецЕсли;
		...
		Элементы.КомандаДалее.Заголовок = НСтр("ru='Выполнить';en='Execute'");
		
		ОбработкаОтбор();
	ИначеЕсли Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаОбработка Тогда
		ОбработкаВыполнить();
		ПоказатьПредупреждение(, НСтр("ru='Выполнение обработки завершено.';en='Execution of handling is completed.'"), 60, НСтр("ru='Передача текущих дел';en='Transmission of current affairs'"));
	КонецЕсли;
...
Процедура КомандаНазад(Команда)
	...
	Если Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаОбработка Тогда
		...
		Элементы.КомандаДалее.Заголовок = НСтр("ru='Далее';en='Further'");
	КонецЕсли;
...
Процедура ОбработкаОтбор()
	...
	Для Каждого Строка Из СписокТиповОбъектов Цикл
		...
		ИначеЕсли ПолноеИмяОбъекта = "Документ.ЭлектронноеПисьмоИсходящее" Тогда
			Текст = СтрЗаменить(Текст, "//Отбор", "И НЕ Объект.Удалить_Рассмотрено");
		ИначеЕсли ПолноеИмяОбъекта = "Документ.CRM_Взаимодействие" Тогда
			Текст = СтрЗаменить(Текст, "//Отбор", "И НЕ Объект.СтатусВзаимодействия = Значение(Справочник.CRM_СостоянияСобытий.Завершено) И НЕ Объект.СтатусВзаимодействия = Значение(Справочник.CRM_СостоянияСобытий.Отменено)");
		ИначеЕсли ПолноеИмяОбъекта = "Документ.CRM_Интерес" Тогда
			Текст = СтрЗаменить(Текст, "//Отбор", "И НЕ Объект.Завершен"); 
		ИначеЕсли ВидОбъекта = "БизнесПроцесс" Тогда
	...
	Если Результат = "" Тогда
		...
		Если Документы.ТипВсеСсылки().СодержитТип(ТипОбъекта) Тогда
			Если ТипОбъекта = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") ИЛИ ТипОбъекта = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
				ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(НовыйОтветственный.ИдентификаторПользователяИБ);
				ОсновнаяУчетнаяЗапись = ХранилищеОбщихНастроек.Загрузить("ОсновнаяУчетнаяЗапись", "ОсновнаяУчетнаяЗапись",, ПользовательИБ.Имя);
				Если ЗначениеЗаполнено(ОсновнаяУчетнаяЗапись) И ТекОбъект.УчетнаяЗапись<>ОсновнаяУчетнаяЗапись Тогда
					СтараяПапка = Взаимодействия.СтруктураРеквизитовВзаимодействия(ОбъектСсылка).Папка;
					ТекОбъект.УчетнаяЗапись = ОсновнаяУчетнаяЗапись;
					НоваяПапка = CRM_УправлениеЭлектроннойПочтой.ПолучитьПапкуЭлектронногоПисьма(ОсновнаяУчетнаяЗапись, СтараяПапка.CRM_Вид);
					ВзаимодействияВызовСервера.УстановитьПапкуЭлектронногоПисьма(ОбъектСсылка, НоваяПапка, Ложь);
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработкаВыполнить()
	...
	Для каждого Строка Из Список Цикл
		...
		ИначеЕсли Результат = "НеПолучен" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось заблокировать';en='It were not possible to disable'") + " """ + СокрЛП(Строка.Объект) + """. " + НСтр("ru='Обратитесь к администратору.';en='Contact the administrator.'"));
		ИначеЕсли Результат = "НеЗаписан" ИЛИ Результат = "НеОбработанПартнер" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось записать';en='It were not possible to write down'") + " """ + СокрЛП(Строка.Объект) + """. " + НСтр("ru='Обратитесь к администратору.';en='Contact the administrator.'"));
		ИначеЕсли Результат = "НетУровняДоступа" Тогда
			КолвоБезДоступа = КолвоБезДоступа + 1;
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось обработать';en='It were not possible to process'") + " """ + СокрЛП(Строка.Объект) + """.");
		КонецЕсли
	КонецЦикла;
	...
	Если КолвоБезДоступа > 0 Тогда
		...
		Если ЕстьРольCRM_ПередачаДелОтветственному ИЛИ ЕстьРольПолныеПрава Тогда
			Текст = Текст + НСтр("ru='Установите Флаг ""Добавлять уровнь доступа"".';en='Set the flag ""Add access level"".'");
		Иначе
			Текст = Текст + НСтр("ru='Обратитесь к администратору.';en='Contact the administrator.'");
		КонецЕсли;
		Текст = "ru = '"+Текст+"'";
