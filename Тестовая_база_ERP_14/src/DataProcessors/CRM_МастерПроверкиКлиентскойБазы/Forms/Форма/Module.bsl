...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВидМастера")
		...
		Если ВидМастера = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольПотенциалаКлиентов
			...
			//
			ВызватьИсключение НСтр("ru='Нарушение прав доступа!';en='Access right violation'");
			Возврат;
		КонецЕсли;
		...
		Если ВидМастера = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов Тогда
			...
			Элементы.ДекорацияОписаниеСтраницыСтраницаКонтрольЗаполненияКИВыборПолучателейПоручений.Заголовок = НСтр("ru='Распределение документов по исполнителям';en='Distribution of documents by performers'");
		КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТаблицаИсполнителей.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Очистить список исполнителей?';en='Clear the list of executors?'");
...
Процедура ДекорацияОтборНажатие(ИмяРеквизитаАдресОтбор, ОписаниеОповещенияОЗавершении)
	...
	//
	ПараметрыСтруктура.Вставить("Заголовок", НСтр("ru='Настройка отбора по клиентам и контактным лицам';en='Settings of selection on customers and contact persons'"));
...
Процедура ПодразделениеПриИзменении(Элемент)
	...
	Если ФормулаРасчета = "" Тогда
		...
		Элементы.ДекорацияФормулаРасчетаПотенциала.Заголовок   = НСтр("ru='Для подразделения не определена формула расчета потенциала клиентов. После настройки расчета потенциала необходимо выполнить команду ""Сохранить и пересчитать потенциалы""';en='For the department, the formula for calculating the potential of customers is not defined. After setting the calculation of the potential, you must execute the command ""Save and recalculate the potentials""'");
	...
	Иначе	
		...
		Элементы.ДекорацияФормулаРасчетаПотенциала.Заголовок   = НСтр("ru='Формула расчета потенциала:';en='The formula for calculating the potential is:'") + Символы.ПС + ФормулаРасчета;
		Элементы.ДекорацияФормулаРасчетаПотенциала.Видимость   = Истина;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДеревоФормирование
...
Процедура КомандаДалее(Команда)
	...
	Если Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаКонтрольЗаполненияВыборПолучателейПоручений Тогда
		Если Не ЗначениеЗаполнено(ДатаИсполнения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрана дата исполнения!';en='Date of execution are not selected!'"),, "ДатаИсполнения");
			Возврат;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ДатаКонтроля) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрана дата контроля!';en='Date of monitoring are not selected!'"),, "ДатаКонтроля");
			Возврат;
		КонецЕсли;
		Если ДатаКонтроля < ДатаИсполнения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата контроля не может быть меньше даты исполнения!';en='Control date must not be earlier than execution date!'"),, "ДатаКонтроля");
			Возврат;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Проверяющий) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран проверяющий!';en='The check are not selected!'"),, "Проверяющий");
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.ЗаполнениеКонтактнойИнформации") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольЗаполненияКИВыборВидовКИ Тогда
			...
			Если ВыбранныеВидыКИ.Партнер.Количество() = 0 И ВыбранныеВидыКИ.КонтактноеЛицо.Количество() = 0 Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбрано ни одного вида контактной информации!';en='It is not selected any type of the contact information!'"));
				Возврат;
			КонецЕсли;
			
			Состояние(НСтр("ru='Формируется список клиентов...';en='The list of customers is formed...'"));
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольЗаполненияКИФормированиеСписков Тогда
			Если Не ПроверитьНаВыбранныеЭлементы(ДеревоПартнеровКИ.ПолучитьЭлементы()) Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбрано ни одного клиента!';en='It is not selected any customer!'"));
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.ЗаполнениеДополнительныхРеквизитов") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольЗаполненияДопРеквизитовВыборДопРеквизитов Тогда
			...
			Если ВыбранныеДопРеквизиты.Партнер.Количество() = 0 И ВыбранныеДопРеквизиты.КонтактноеЛицо.Количество() = 0 Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбрано ни одного дополнительного реквизита!';en='It is not selected any additional detail!'"));
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.ДублированиеКлиентов") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольДублированияВыборРеквизитов Тогда
			...
			Если ВыбранныеРеквизиты.Партнер.Количество() = 0 И ВыбранныеРеквизиты.КонтактноеЛицо.Количество() = 0 Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбрано ни одного реквизита!';en='It is not selected any detail!'"));
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольПотенциалаКлиентов") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольПотенциалаВыборРеквизитов Тогда
			Если Не КонтрольПотенциалаФлагБезЗаполненногоПотенциала И (Не ВедетсяИсторияРеквизитовВлияющихНаПотенциалКлиента Или Не КонтрольПотенциалаФлагИзменилисьРеквизитыВлияющиеНаПотенциал) Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбрано ни одно условие поиска!';en='No search terms selected!'"));
				Возврат;
			КонецЕсли;
			Если Подразделение.Пустая() Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбрано подразделение!';en='No division is selected!'"));
				Возврат;
			КонецЕсли;
			Если ПолучитьФормулуРасчетаПотенциала() = "" Тогда
				ПоказатьПредупреждение(,НСтр("ru='У подразделения не заполнена формула расчета потенциала!';en='The unit does not have a formula for calculating the potential!'"));
				Возврат;
			КонецЕсли;
			Состояние(НСтр("ru='Формируется список клиентов...';en='The list of customers is formed...'"));
	...
	ИначеЕсли ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольРегистрацииКлиентовВыборРеквизитов Тогда
			Если Не КонтрольРегистрацииКлиентовФлагСобытия И Не КонтрольРегистрацииКлиентовФлагЭлектронныеПисьма Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбран ни один вид документа!';en='No document type selected!'"));
				Возврат;
			КонецЕсли;
			
			Состояние(НСтр("ru='Формируется список событий...';en='A list of events is generated ...'"));
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольРегистрацииКлиентовФормированиеСписков Тогда
			Если Не ПроверитьНаВыбранныеЭлементы(ДеревоСобытияКонтрольРегистрацииКлиентов.ПолучитьЭлементы()) Тогда
				ПоказатьПредупреждение(,НСтр("ru='Не выбрано ни одного объекта!';en='No objects selected!'"));
				Возврат;
			КонецЕсли;
...
Процедура КомандаЗакрытьМастер(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Завершить работу с мастером?';en='Complete operation with the master?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура КомандаТаблицаИсполнителейОчистить(Команда)
	Если ТаблицаИсполнителей.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Очистить список исполнителей?';en='Clear the list of executors?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьЗаголовокОтбор(АдресОтбор, ЭлементыДекорация)
	Если ЭтоАдресВременногоХранилища(АдресОтбор) Тогда
		...
		Если ТипЗнч(ТекОтбор) = Тип("ОтборКомпоновкиДанных") И ТекОтбор.Элементы.Количество() > 0 Тогда
			ЭлементыДекорация.Заголовок = НСтр("ru='Отбор:';en='Selection:'") + " " + Строка(ТекОтбор);
		Иначе
			ЭлементыДекорация.Заголовок = НСтр("ru='Настройка отбора...';en='Tuning selection ...'");
		КонецЕсли;
	Иначе
		ЭлементыДекорация.Заголовок = НСтр("ru='Настройка отбора...';en='Tuning selection ...'");
	КонецЕсли;
...
Процедура СформироватьДеревоПартнеровКИ()
	...
	Если ВыбранныеВидыКИ.Партнер.Количество() > 0 Тогда
		Для Каждого ВидКИ Из ВыбранныеВидыКИ.Партнер Цикл
			...
			Если ВидКИ.Родитель = Справочники.ВидыКонтактнойИнформации.СправочникПартнеры Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И Партнеры.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.КомпанияЧастноеЛицо.Компания)", "");
			ИначеЕсли ВидКИ.Родитель = Справочники.ВидыКонтактнойИнформации.CRM_СправочникПартнерыЧастноеЛицо Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И Партнеры.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.КомпанияЧастноеЛицо.Компания)", "И Партнеры.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.КомпанияЧастноеЛицо.ЧастноеЛицо)");
			КонецЕсли;
	...
	Если ПоОтветственнымМенеджерам Тогда
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			Иначе
				...
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для некоторых клиентов не указан основной менеджер - нужно выбрать иполнителя для клиентов, которым не назначен ответственный менеджер!';en='For some customers the main administrator are not specified - it is necessary to select ipolnitel for customers by which responsible administrator are not assigned!'"),, "ПользовательОтветственныйПоУмолчанию");
...
Функция ПолучитьНаименованиеПоручения()
	Если ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ЗаполнениеКонтактнойИнформации Тогда
		Возврат НСтр("ru='Заполнение контактной информации клиентов и контактных лиц';en='Filling of the contact information of customers and contact persons'");
		
	ИначеЕсли ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ЗаполнениеДополнительныхРеквизитов Тогда
		Возврат НСтр("ru='Заполнение реквизитов клиентов и контактных лиц';en='Filling of details of customers and contact persons'");
		
	ИначеЕсли ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ДублированиеКлиентов Тогда
		Возврат НСтр("ru='Исправление дублирующихся записей по клиентам';en='Correction of doubl records on customers'");
		
	ИначеЕсли ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольПотенциалаКлиентов Тогда
		Возврат НСтр("ru='Заполнить портрет клиентов для расчета потенциала';en='Fill in a portrait of customers to calculate the potential'");
		
	ИначеЕсли ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов Тогда
		Возврат НСтр("ru='Заполнение клиента в электронных письмах и телефонных звонках';en='Filling customer in e-mails and phone calls'");
...
Функция ПолучитьОписаниеПоручения()
	Если ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ЗаполнениеКонтактнойИнформации Тогда
		...
		Текст = НСтр("ru='Проверить заполненность / заполнить следующие виды контактной информации клиентов и контактных лиц:';en='Check up a zapolnennost / to fill the following types of the contact information of customers and contact persons:'");
		Если ВидыКИ.Партнер.Количество() > 0 Тогда
			Текст = Текст + Символы.ПС;
			Текст = Текст + НСтр("ru='Клиенты:';en='Customers:'");
		...
		Если ВидыКИ.КонтактноеЛицо.Количество() > 0 Тогда
			Текст = Текст + Символы.ПС;
			Текст = Текст + НСтр("ru='Контактные лица:';en='Contact persons:'");
	...
	ИначеЕсли ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ЗаполнениеДополнительныхРеквизитов Тогда
		...
		Текст = НСтр("ru='Проверить заполненность / заполнить следующие дополнительные реквизиты клиентов и контактных лиц:';en='Check up a zapolnennost / to fill the following additional details of customers and contact persons:'");
	...
	ИначеЕсли ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ДублированиеКлиентов Тогда
		Возврат НСтр("ru='Исправить дублирующиеся записи по клиентам';en='Correct doubl records on customers'");
	...
	ИначеЕсли ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов Тогда
		Возврат НСтр("ru='Заполнить клиента в электронных письмах и телефонных звонках';en='Fill customer in e-mails and phone calls'");
...
Функция НайтиДубли(СсылкаНаОбъект, СтруктураПоиска, СписокКонтактов)
	...
	Для каждого КлючИЗначение Из СтруктураПоиска Цикл
		...
		Если ИмяРеквизита = "CRM_ИНН" Тогда
			Если CRM_ОбщегоНазначенияСервер.НайтиВМетаданныхПоИмени("Справочники", "Контрагенты") Тогда
				Если CRM_ОбщегоНазначенияСервер.НайтиВМетаданныхПоИмени("Справочники", "Партнеры") Тогда
					Запрос = Новый Запрос("ВЫБРАТЬ
					                      |	Контрагенты.ГоловнойКонтрагент.ИНН КАК ИНН
					                      |ИЗ
					...
					Если Выборка.Следующий() Тогда
						ЗначениеРеквизита = Выборка.ИНН;
					КонецЕсли;
				Иначе
					ЗначениеРеквизита = СсылкаНаОбъект.ИНН;
				КонецЕсли;
			Иначе
				ЗначениеРеквизита = СсылкаНаОбъект[ИмяРеквизита];
			КонецЕсли;
		Иначе
			Если СтрНайти(ИмяРеквизита, "Контакт_")>0 Тогда
				ЭлементКонтакт = СписокКонтактов.НайтиПоЗначению(СтрЗаменить(ИмяРеквизита, "Контакт_", ""));
				Если ЭлементКонтакт<>Неопределено Тогда
					ЗначениеРеквизита = ЭлементКонтакт.Представление;
				КонецЕсли;
			Иначе
				ЗначениеРеквизита = СсылкаНаОбъект[ИмяРеквизита];
			КонецЕсли;
	...
	Иначе	
		Для Каждого СтрокаМассива Из МассивНайденных Цикл
			Если СтрокаМассива.Ссылка = СсылкаНаОбъект Тогда Продолжить; КонецЕсли;
...
Процедура ФормированиеПоручений()
	...
	Если ВидРаспределенияПоИсполнителям = 1 Тогда
		...
		Если ВыбранныеИсполнители.Количество() = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ru='Не выбрано ни одного исполнителя!';en='It is not selected any executor!'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ФормированиеПорученийЗавершение(Ответ, ВыбранныеИсполнители) Экспорт
	...
	Состояние(НСтр("ru='Формирование поручений...';en='Formation of commissions...'"));
