...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.БизнесПроцесс) Тогда
		...
		Заголовок = НСтр("ru='Карта маршрута бизнес-процесса:';en='Route map of business process:'") + " " + CRM_БизнесПроцессыСервер.СформироватьПредставлениеБизнесПроцесса(БизнесПроцесс);
...
#Если ВебКлиент Тогда
	ПоказатьПредупреждение(, НСтр("ru='В Веб-клиенте данная операция недоступна.';en='In the Web customer, this operation is not available.'"));
	Возврат;
#КонецЕсли
	ОчиститьСообщения();
	...
	Если Не ЗначениеЗаполнено(БизнесПроцесс) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо указать бизнес-процесс.';en='It is necessary to specify business process.'"),,
			"БизнесПроцесс");
		Возврат;
	КонецЕсли;
	...
	Если ТекЭлемент = Неопределено ИЛИ
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо выбрать точку действия или вложенный бизнес-процесс карты маршрута.';en='It is necessary to select a point of action or nested business process of a route map.'"),,
			"КартаМаршрута");
		Возврат;
	КонецЕсли;
	...
	ЗаголовокФормы = НСтр("ru='Задачи по точке маршрута бизнес-процесса';en='Tasks on a route point of business process'");
