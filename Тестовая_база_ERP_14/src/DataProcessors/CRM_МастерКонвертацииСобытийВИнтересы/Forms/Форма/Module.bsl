...
Процедура КонвертироватьНаСервере()
	...
	Если CRM_ИспользоватьБизнесПроцессы И КонвертироватьСделки Тогда
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьCRM_Событие.Проект КАК Проект,
	...
	Запрос.УстановитьПараметр("СостояниеИнтереса", ВыявлениеПотребностей);
	ТаблицаВидов = Запрос.Выполнить().Выгрузить();
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Событие = ВыборкаДетальныеЗаписи.Ссылка;
		НаправлениеСобытия = ?(ЗначениеЗаполнено(Событие.ВходящееИсходящее), Событие.ВходящееИсходящее, Перечисления.CRM_ВходящееИсходящееСобытие.Исходящее);
		СтруктураПоиска = Новый Структура("Направление,ВидСобытия", НаправлениеСобытия, Событие.ВидСобытия);
		МассивСтрок = ТаблицаВидов.НайтиСтроки(СтруктураПоиска);
		СтрокаВида = ?(МассивСтрок.Количество() = 0, Неопределено, МассивСтрок[0]);
		Если СтрокаВида = Неопределено Тогда
			НаименованиеВида = Строка(Событие.ВидСобытия) + " (" + Строка(НаправлениеСобытия) + ")";
			...
			Если ВидВзаимодействия = Справочники.CRM_ВидыВзаимодействий.ПустаяСсылка() Тогда
				...
				ВидВзаимодействия.Направление = НаправлениеСобытия;
				ВидВзаимодействия.СостояниеИнтереса = ВыявлениеПотребностей;
				ВидВзаимодействия.Записать();
			КонецЕсли;
		Иначе
			ВидВзаимодействия = СтрокаВида.Ссылка;
		КонецЕсли;
		...
		ДокументВзаимодействие.СостояниеИнтереса = ВыявлениеПотребностей;
		...
		ДокументВзаимодействие.Тема = Событие.Тема;
		...
		ДокументВзаимодействие.СторонниеЛица.Загрузить(Событие.СторонниеЛица.Выгрузить());
		ДокументВзаимодействие.СвоиЛица.Загрузить(Событие.СвоиЛица.Выгрузить());
		ДокументВзаимодействие.Категории.Загрузить(Событие.Категории.Выгрузить());
		ДокументВзаимодействие.РядСобытий = Событие.РядСобытий;
		ДокументВзаимодействие.ИзмененоВРяде = Событие.ИзмененоВРяде;
		ДокументВзаимодействие.Периодическое = Событие.Периодическое;
		...
		Если Событие.Товары.Количество() > 0 Тогда
			...
			ДокументКП.Соглашение				= Событие.Соглашение;
			...
			Если Не ЗначениеЗаполнено(ДокументКП.Соглашение) Тогда
				ДокументКП.ЗаполнитьУсловияПродажПоУмолчанию();
			Иначе
				ДокументКП.Валюта				= ДокументКП.Соглашение.Валюта;
			КонецЕсли;
			...
			ТаблицаЗапасов = Событие.Товары.Выгрузить();
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьCRM_Событие.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.УдалитьCRM_Событие КАК УдалитьCRM_Событие
	|ГДЕ
	|	НЕ УдалитьCRM_Событие.ПометкаУдаления
	...
	Если Аналитика = "Партнер" Тогда
		Пока ВыборкаВерхнийУровень.Следующий() Цикл
			...
			Пока ВыборкаСобытие.Следующий() Цикл
				ДокументИнтерес.Проект = ВыборкаСобытие.Проект;
				...
				Если Событие.Товары.Количество() > 0 Тогда
					...
					Иначе
						...
						ИмяВарианта = Нстр("ru = 'Вариант '") + Строка(НомерВариантаКП);
						Для Каждого СтрокаЗапасов Из ТаблицаЗапасов Цикл
							НоваяСтрока = ДокументКПОбъект.Товары.Добавить(); 
							ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаЗапасов);
							НоваяСтрока.CRM_ИмяВарианта = ИмяВарианта;
							НоваяСтрока.CRM_ИДВарианта = НомерВариантаКП;
						КонецЦикла;
				...
				Попытка
					...
					И (Событие.СторонниеЛица.Количество() > 0 ИЛИ Событие.СвоиЛица.Количество() > 0) Тогда
						ДокументМероприятие = Документы.CRM_Взаимодействие.СоздатьДокумент();
						ДокументМероприятие.ВидВзаимодействия = Справочники.CRM_ВидыВзаимодействий.Встреча;
						ДокументМероприятие.Дата = Событие.Дата;
						ДокументМероприятие.ПлановаяДата = Событие.Дата;
						...
						ДокументМероприятие.ПлановаяДатаЗавершение = Событие.ОкончаниеСобытия;
						ДокументМероприятие.ДатаЗавершенияВзаимодействия = Событие.ОкончаниеСобытия;
						ДокументМероприятие.Содержание = Событие.Описание;
						...
						ДокументМероприятие.СтатусВзаимодействия = Событие.СостояниеСобытия;
			...
			Если ПоследнееСобытиеИнтереса <> Неопределено Тогда
				...
				ДокументИнтерес.СостояниеИнтереса = ?(ДокументИнтерес.Завершен, ИнтересЗакрыт, ВыявлениеПотребностей);
...
Процедура УстановитьУВзаимодействийСостояниеЗавершено()
	...
	Пока Выборка.Следующий() Цикл
		...
		ВзаимодействиеОбъект.СостояниеИнтереса = ИнтересЗакрыт;
	...
	//Если СписокКартМаршрута.Количество() > 0 Тогда
		...
		Для Каждого Выборка Из РезультатЗапроса Цикл
			...
			Если ТипЗнч(Выборка.Основание) = Тип("ДокументСсылка.удалитьCRM_Сделка") Тогда
				...
				ВыборкаСценариев		= Справочники.CRM_СостоянияИнтересов.Выбрать(Справочники.CRM_СостоянияИнтересов.ПустаяСсылка());
				Если ВыборкаСценариев.Следующий() Тогда
					СценарийИнтереса	= ВыборкаСценариев.Ссылка;
				КонецЕсли;
				СостояниеИнтереса 		= ?(ДокументОснование.БизнесПроцесс.ВариантЗавершения.УспешностьЗавершения, 
					Справочники.CRM_СостоянияИнтересов.УспешноеЗавершениеСценария(СценарийИнтереса),
					Справочники.CRM_СостоянияИнтересов.НеудачноеЗавершениеСценария(СценарийИнтереса));
			Иначе		
				Продолжить;
			...
			Если НЕ БПЗавершен Тогда
				ДокументИнтерес.СостояниеИнтереса = ВыявлениеПотребностей;
			Иначе
			...
			Для Каждого ВыборкаОбъект Из ТаблицаОбъектовПоБП Цикл
				...
				Если ТипЗнч(ДокументБП) = Тип("ДокументСсылка.УдалитьCRM_Событие") Тогда
					...
					СтруктураПоиска = Новый Структура("Направление,ВидСобытия", ДокументБП.ВходящееИсходящее, ДокументБП.ВидСобытия);
					МассивСтрок = ТаблицаВидов.НайтиСтроки(СтруктураПоиска);
					СтрокаВида = ?(МассивСтрок.Количество() = 0, Неопределено, МассивСтрок[0]);
					Если СтрокаВида = Неопределено Тогда
					...
					ДокументВзаимодействие.Тема					= ДокументБП.Тема;
					...
					ДокументВзаимодействие.СторонниеЛица.Загрузить(ДокументБП.СторонниеЛица.Выгрузить());
					ДокументВзаимодействие.СвоиЛица.Загрузить(ДокументБП.СвоиЛица.Выгрузить());
					ДокументВзаимодействие.Категории.Загрузить(ДокументБП.Категории.Выгрузить());
					
					ДокументВзаимодействие.РядСобытий = ДокументБП.РядСобытий;
					ДокументВзаимодействие.ИзмененоВРяде = ДокументБП.ИзмененоВРяде;
					ДокументВзаимодействие.Периодическое = ДокументБП.Периодическое;
					...
					CRM_ПрисоединенныеФайлы.СкопироватьПрисоединенныеФайлы(ОбъектСобытие.Ссылка, ДокументВзаимодействие.Ссылка);
					
					// Если в событии заполнена ТЧ "Интерес клиента", то создаём КП с вариантами, где один вариант - одно событие
					Если ДокументБП.Товары.Количество() > 0 Тогда
						...
						Если ТекущееКП = Неопределено Тогда
							...
							ДокументКП.Соглашение				= ДокументБП.Соглашение;
							...
							ТаблицаЗапасов 	= ДокументБП.Товары.Выгрузить();
							ИмяВарианта 	= Нстр("ru = 'Вариант '") + Строка(НомерВариантаКП);
							Для Каждого СтрокаЗапасов Из ТаблицаЗапасов Цикл
								НоваяСтрока 			= ДокументКП.Товары.Добавить(); 
								ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаЗапасов);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;	
	КонвертироватьСделки = Истина;
	ГруппировкаПоОтветственным = Истина;
	ГруппировкаПоПроектам = Истина;
	
	ВыборкаСценариев		= Справочники.CRM_СостоянияИнтересов.Выбрать(Справочники.CRM_СостоянияИнтересов.ПустаяСсылка());
	Если ВыборкаСценариев.Следующий() Тогда
		СценарийИнтереса	= ВыборкаСценариев.Ссылка;
	КонецЕсли;
	ИнтересЗакрыт = Справочники.CRM_СостоянияИнтересов.УспешноеЗавершениеСценария(СценарийИнтереса);
...
Процедура Конвертировать(Команда)
	...
	Если Не ЗначениеЗаполнено(ВыявлениеПотребностей) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбрано состояние выявления потребности!'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПолучитьРодителя(ВыявлениеПотребностей)) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Необходимо выбрать состояние сценария, а не сам Сценарий!'"));
		Возврат;
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Конвертация событий завершена!';en='Event Conversion Complete!'"));
