...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеСервер.ПодсистемаCRMИспользуется() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно открыть форму планирования проектов. Подсистема 1С:CRM не используется! (см. Персональные настройки пользователя)';en='You can not open the project planning form. Subsystem 1C: CRM is not used! (see Personal settings of the user)'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	ОтборЗначениеИсполнитель = Пользователи.ТекущийПользователь();
	ОтборЗначениеОтветственный = ОтборЗначениеИсполнитель;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокЗаписей
...
Процедура СписокЗаписейПлановПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если НЕ ЗначенияЗаполнения.Свойство("Проект") Тогда
		...
		Если ЗначениеЗаполнено(ТекущийПроект) Тогда
			ТекущийПроектЭтоГруппа = Ложь;
			Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
				ТекущийПроектЭтоГруппа = ПолучитьЗначениеРеквизита(ТекущийПроект, "ЭтоГруппа");
			КонецЕсли;
			
			Если НЕ ТекущийПроектЭтоГруппа Тогда
...
Процедура СписокКонтрольныеТочкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если НЕ Копирование Тогда
		...
		ФормаДляОткрытия = ПолучитьФорму("Документ.CRM_ЭтапКалендарногоПлана.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
		ФормаДляОткрытия.Открыть();
	КонецЕсли
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПланыДерево
...
Процедура КомандаСписокЗаписейПлановРазбитьПоМесяцам(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Выбранные записи будут разбиты по месяцам, продолжить?';en='Selected records will be br on months, to continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура РазбитьЗаписиПоМесяцам(Знач МассивЗаписей)
	...
	Для Каждого КлючЗаписи Из МассивЗаписей Цикл
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка!';en='Error!'")
				+ " "
...
Процедура ПриИзмененииПериодаПланирования(ПриоритетДатыНачала = Истина)
	...
	КонецЦикла;
	Если КоличествоМесяцевПланирования > 12 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Нельзя указывать период планирования больше 1 года!';en='It is impossible to specify the period of scheduling more than 1 year!'"), 5);
...
Процедура ДобавитьЗаголовокПланов(ДеревоЗначенийПланы)
	...
	СтрокаДереваПланов.ГруппировкаПредставление = НСтр("ru='НОРМА ВРЕМЕНИ';en='Norm of time'") + " (" + Строка(ПроизводственныйКалендарь) + ")";
...
Процедура СформироватьДиаграммуГанта(ТекСтрокаДерева)
	...
	Если РежимПланирования = 1 Тогда // по проектам
		Если бСтрокаИтогов И флВыводитьПодразделения Тогда
			СписокИзмерений.Добавить("Подразделение", НСтр("ru='Подразделение';en='Department'"));
		КонецЕсли;
		СписокИзмерений.Добавить("Проект",		НСтр("ru='Проект';en='Project'"));
		СписокИзмерений.Добавить("Этап",		НСтр("ru='Этап';en='Stage'"));
		СписокИзмерений.Добавить("Исполнитель",	НСтр("ru='Исполнитель';en='Executor'"));
...
Процедура ПерезаполнитьДанныеПланов()
	...
	Если бНеЗаполненКалендарьЗаПериод Тогда	
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В выбранном периоде не заполнен календарь! Вывод без учета календаря!';en='In the selected period the calendar is not filled in! Show without calendar!'"));
	ИначеЕсли Не бРассчитаноСУчетомГрафика И Не ПустаяСтрока(РасшифровкаРассчитанныхБезУчетаГрафика) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен календарь за период! Вывод без учета календаря:';en='The calendar during the period is not filled in! An output without a calendar:'") + Символы.ПС + РасшифровкаРассчитанныхБезУчетаГрафика);
	КонецЕсли;
...
Процедура ЗаполнитьОформление()
	...
	Для НомерСтроки = 1 По Макет.ВысотаТаблицы Цикл
		ЗначениеИдентификатор	= Макет.Область(НомерСтроки, КолонкаИдентификатор, НомерСтроки, КолонкаИдентификатор).Параметр;
		...
		Попытка
			ЗначениеКолонкаИспользоватьШрифт = Булево(Макет.Область(НомерСтроки, КолонкаИспользоватьШрифт, НомерСтроки, КолонкаИспользоватьШрифт).Параметр);
		Исключение
			ЗначениеКолонкаИспользоватьШрифт = Ложь;
		КонецПопытки;
		Попытка
			ЗначениеИспользоватьЦветТекста = Булево(Макет.Область(НомерСтроки, КолонкаИспользоватьЦветТекста, НомерСтроки, КолонкаИспользоватьЦветТекста).Параметр);
		Исключение
			ЗначениеИспользоватьЦветТекста = Ложь;
		КонецПопытки;
		Попытка
			ЗначениеИспользоватьЦветФона = Булево(Макет.Область(НомерСтроки, КолонкаИспользоватьЦветФона, НомерСтроки, КолонкаИспользоватьЦветФона).Параметр);
		Исключение
...
Процедура ВывестиДеревоПланов()
	...
	Если флВыводитьНераспланированныеРесурсы Тогда
		...
		Если НезапланированныеПроектыЭтапы.Количество() > 0 Тогда
			...
			ИмяВетки = ?(НЕ ЗначениеЗаполнено(Элементы.СписокПроектов.ТекущаяСтрока) ИЛИ Элементы.СписокПроектов.ТекущаяСтрока.ЭтоГруппа, НСтр("ru='[НЕЗАПЛАНИРОВАННЫЕ ПРОЕКТЫ]';en='[UNPLANNED PROJECTS]'"), НСтр("ru='[НЕЗАПЛАНИРОВАННЫЕ ЭТАПЫ]';en='[UNPLANNED STAGES]'"));
...
Процедура ОбновитьДеревоПланов(ПеречитыватьДанные = Истина)
	...
	Если ПеречитыватьДанные Тогда // с перечитыванием из базы - перезаполняем данные ТЧ Планы.
		...
		Если ВыводитьСостояние Тогда
			Состояние(НСтр("ru='Перезаполняются данные планов ...';en='Перезаполняются data of plans...'"));
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ДатаНачала) ИЛИ НЕ ЗначениеЗаполнено(Объект.ДатаКонца) ИЛИ Объект.ДатаНачала>Объект.ДатаКонца Тогда
		Если НЕ ЗапретНаОбновлениеДерева Тогда
			ПоказатьПредупреждение(, НСтр("ru='Некорректный период планирования!';en='Incorrect period of scheduling!'"), 5);
		КонецЕсли;
		Возврат;
	КонецЕсли;

	Если ВыводитьСостояние Тогда
		Состояние(НСтр("ru='Выполняется вывод плановых данных ...';en='The output of the planned data are fulfill...'"));
	КонецЕсли;
