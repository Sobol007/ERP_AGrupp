...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьБизнесПроцессыИЗадачи") Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Использование бизнес процессов и задач отключено в настройках системы.';en='Usage business of processes and tasks are disconnect in settings of system.'"));
		Возврат;
	КонецЕсли;
...
Процедура КомандаВыгрузить(Команда)
	Если Не ЗначениеЗаполнено(Проект) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Проект не выбран!';en='The project are not selected!'"),, "Проект");
		Возврат;
	КонецЕсли;
	
	Если СгенерироватьЭтапы() Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("КомандаВыгрузитьЗавершение", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Отметить все выгруженые задачи как выполненные?';en='Mark all preempt tasks as fulfill?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
КонецПроцедуры
...
Функция СгенерироватьЭтапы()
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрана ни одна задача!';en='Any task are not selected!'"),, "ДеревоСписокЗадач");
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при создании этапов:';en='Error at creation of stages:'")
