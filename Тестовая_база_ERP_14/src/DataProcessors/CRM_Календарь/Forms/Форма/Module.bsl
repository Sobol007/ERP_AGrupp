...
Процедура КалендарьТабличныйДокументПриАктивизацииОбласти(Элемент)
	...
	Если Не АктивизцияВнеОбластиКалендаря Тогда
		...
		Если ТекущиеДанные <> Неопределено И ТекущиеДанные.МассивРасшифровокСобытий.Количество() = 1 И
			 	ТипЗнч(ЗначениеРасшифровки.Событие) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
			 		ДоступностьКнопкаПеренаправитьСобытиеИзКалендаря = Ложь;
		КонецЕсли;
	Иначе
...
Процедура КалендарьСписокТекстПоискаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокДляВыбора.Добавить("[ПоискПоПодстроке]", НСтр("ru='[Поиск по подстроке]';en='[Поиск по подстроке]'"));
...
Процедура СписокЗадачПередУдалением(Элемент, Отказ)
	...
	Если МассивЗадач.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Удалять можно только личные задачи!';en='It is possible to delete only personal tasks!'"), 10);
		Возврат;
	КонецЕсли;
	
	Если бЕстьПодчиненныеЗадачи Тогда
		ТекстВопроса = НСтр("ru='Удалить задачи, включая все подчиненные задачи?';en='Delete tasks, including all subordinate tasks?'");
	Иначе
		ТекстВопроса = НСтр("ru='Удалить выбранные задачи?';en='Delete the select tasks?'");
	КонецЕсли;
			...
			Если ТипЗнч(РасшифровкаЗначение.Событие) = Тип("ДокументСсылка.УдалитьCRM_Событие") Тогда
				...
				Если НЕ РанееСозданаяЗадача = Неопределено Тогда
					ПоказатьОповещениеПользователя(
						НСтр("ru='Событие уже принято к исполнению:';en='Event are already accepted to execution:'"), 
				...
				Если бМассивЗадачКорректный Тогда
					...
					Если ЗначениеЗаполнено(ТекПроект) Или ЗначениеЗаполнено(ТекПартнер) Тогда
						Если бЗадаватьВопросПерезаписатьПроектПартнер Тогда
							...
							ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Связать задачу с проектом и клиентом родительской задачи?';en='Connect the task with the project and customer of the parent task?'"), РежимДиалогаВопрос.ДаНетОтмена);
							Возврат;
						Иначе
...
Процедура КомандаДеревоПользователейДиспетчеризацииДобавитьГруппу(Команда)
	...
	НоваяСтрока.Представление = НСтр("ru='Новая группа';en='New group'");
...
Процедура ПеренестиЗадачуВИерархию(Команда)
	...
	Если МассивСтруктурЗадач.Количество() = 0 Тогда
		Если бЕстьНеЛичныеЗадачи Тогда
			ПоказатьПредупреждение(, НСтр("ru='Переносить в иерархию можно только личные задачи!';en='Transfer to hierarchy it is possible only personal tasks!'"), 10);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьУдалитьЭлементВДеревеРесурсов(ДобавитьГруппу = Ложь,
	...
	Если ДобавитьГруппу Тогда
		...
		ПоказатьВводСтроки(ОписаниеОповещения, СтрокаОписание, НСтр("ru='Название группы';en='Title of group'"),, Ложь);
	Иначе
...
Процедура КомандаСписокЗадачВыгрузитьВПроект(Команда)
	...
	ИначеЕсли бЕстьЗадачи И Не бЕстьЛичныеЗадачи Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выгружать в проект можно только личные задачи!';en='Unload in the project it is possible only personal tasks!'"), 10);
		Возврат;
	КонецЕсли;
	...
	Если бВключитьОтборПоТекущемуПользователю Тогда
		Если Не ЗначениеЗаполнено(ОтборЗначениеПользователь) Или Не ОтборИспользованиеПользователь Тогда
			...
			ПоказатьОповещениеПользователя(
				"",
				"",
				СтрЗаменить(НСтр("ru='Один из отборов должен быть установлен: %Отбор%';en='One of selections it should be install: %otbor %'"), "%Отбор%", СтрокаОбязательныеОтборы + "."));
			//
...
Процедура КомандаКонтекстногоМенюКалендаряОбщая(Команда)
	...
	Если ИмяКоманды = "КомандаСоздатьКонтрольнуюТочкуИзКалендаря" Тогда
		СоздатьСобытиеИзКалендаря("Документ.CRM_ЭтапКалендарногоПлана");
	...
	Иначе
		ВызватьИсключение НСтр("ru='Для кнопки';en='For the button'") + " """ + ИмяКоманды + """ " + НСтр("ru='не определено действие!';en='action are not defin!'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьКонтекстныеМеню()
	...
	Если	ПолучитьФункциональнуюОпцию("CRM_ИспользоватьПодсистемуУправленияПроектами")
		...
		И	ПравоДоступа("Добавление", Метаданные.Документы.CRM_ЭтапКалендарногоПлана) Тогда
		//
		...
		НоваяКомандаФормы.Заголовок = НСтр("ru='Контрольная точка';en='Check Point'");
		НоваяКнопка = Элементы.Добавить("КомандаСоздатьКонтрольнуюТочку", Тип("КнопкаФормы"), ГруппаКонтрольнойТочки);
		НоваяКнопка.Заголовок = НСтр("ru='Контрольная точка';en='Check Point'");
	...
	НоваяКомандаФормы.Действие = "КомандаКонтекстногоМенюКалендаряОбщая";
	НоваяКомандаФормы.Заголовок = НСтр("ru='Перенаправить';en='Redirect'");
	НоваяКнопка = Элементы.Добавить("КомандаПеренаправитьСобытиеИзКалендаря", Тип("КнопкаФормы"), ГруппаПеренаправитьСобытия);
	НоваяКнопка.Заголовок = НСтр("ru='Перенаправить';en='Redirect'");
	...
	НоваяКомандаФормы.Действие = "КомандаКонтекстногоМенюКалендаряОбщая";
	НоваяКомандаФормы.Заголовок = НСтр("ru='Удалить';en='Delete'");
	НоваяКнопка = Элементы.Добавить("КомандаУдалитьСобытиеИзКалендаря", Тип("КнопкаФормы"), ГруппаУдаленияСобытия);
	НоваяКнопка.Заголовок = НСтр("ru='Удалить';en='Delete'");
...
Функция ДляСобытияРазрешеноПеретаскиваниеВКалендарь(РасшифровкаСобытия)
	Если РасшифровкаСобытия = Неопределено Тогда
		Возврат Ложь;
	Иначе
		Если ТипЗнч(РасшифровкаСобытия.Событие) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
			Возврат Ложь;
		Иначе
			Возврат Истина;
		КонецЕсли;
...
Процедура ПеренестиСобытиеНаДатуНаСервере(Событие, ДатаНачалоСобытия, ДатаОкончаниеСобытия, ПользовательСобытия, ПеренестиНаДатуВремя, СобытиеНаВесьДень, ПользовательОтбор, ПомещениеОтбор, СтарыйПользовательОтбор, СтароеПомещениеОтбор, ОбновлятьКалендарь = Ложь, СтруктураДанныхКлиент = Неопределено)
	...
	//	Сообщить(НСтр("ru='При переносе объекта произошли ошибки:';en='At transfer of object there errors:'")
	//		+ Символы.ПС
	//		+ Ошибки);
	//	//
	// КонецЕсли;
	...
	Для Каждого Событие Из МассивСобытий Цикл
		...
		Если СобытиеОбъект.Ответственный <> НаПользователя Тогда
			Если ЗначениеЗаполнено(СобытиеОбъект.Ответственный) Тогда
				...
				СобытиеОбъект.Описание = НСтр("ru='Переадресовано от';en='Redirected from'") + " " + СобытиеОбъект.Ответственный + " " + CRM_ОбщегоНазначенияСервер.ПолучитьТекущуюДатуСеанса()
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при записи события';en='Error at record of event'")
...
Процедура СоздатьСобытиеИзКалендаря(ТипСобытия = Неопределено, Знач ВидСобытия = Неопределено, Знач УпрощеннаяФорма = Ложь)
	...
	Если ЗадаватьВопросПередСозданиемСобытия(СтруктураДанныхСобытия) Тогда
		ТекстВопроса = НСтр("ru='День';en='Day'") + " "
			+ Формат(НачалоДня(СтруктураДанныхСобытия.ПериодНачало), "ДФ='дд ММММ гггг'") + НСтр("ru=' г.';en='year(s)'")
			+ НСтр("ru = ' отмечен в производственном календаре как не рабочий.'")
			+ Символы.ПС
			+ НСтр("ru='Создать событие?';en='Create event?'");
		//
...
Процедура УдалитьСобытиеНаСервере(МассивУдаляемыхСобытий, ОбновлятьКалендарь = Ложь, СтруктураДанныхКлиент = Неопределено)
	...
	//  
	// Если ЗначениеЗаполнено(Ошибки) Тогда
	//	Сообщить(НСтр("ru='При удалении объектов произошли ошибки:';en='When deleting objects there errors:'")
	//		+ Символы.ПС
	//		+ Ошибки);
	//	//
	// КонецЕсли;
...
Процедура УдалитьСобытиеИзКалендаря(ОбновлятьКалендарь = Истина)
	...
	//	Ответ = Вопрос(НСтр("ru='Удалить выбранные объекты?';en='Delete the select objects?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
	//	Если Ответ = КодВозвратаДиалога.Да Тогда
	//		
	//		СтруктураДанныхКлиент = Неопределено;
	//		Если ОбновлятьКалендарь Тогда
...
Процедура ОткрытьФормуНовогоСобытия(Знач СтруктураКлюч, ТипСобытия, Знач УпрощеннаяФорма = Ложь)
	...
	Если ТипСобытия = "Документ.CRM_ЭтапКалендарногоПлана" Тогда
		СтруктураЗначенияЗаполнения.Вставить("ПлановаяДатаНачала", СтруктураКлюч.ПериодНачало);
		СтруктураЗначенияЗаполнения.Вставить("ПлановаяДатаОкончания", СтруктураКлюч.ПериодОкончание);
	КонецЕсли;
	...
	ИначеЕсли ТипСобытия = "Документ.CRM_ЭтапКалендарногоПлана" Тогда
		Форма.Объект.Дата 				   = ТекущаяДата();
		Форма.Объект.ПлановаяДатаНачала    = СтруктураКлюч.ПериодНачало;
		Форма.Объект.ПлановаяДатаОкончания = СтруктураКлюч.ПериодОкончание;
		...
		Если МассивПользователей <> Неопределено Тогда
			Для Каждого ПользовательСобытия Из МассивПользователей Цикл
				НайденныеСтроки = Форма.Объект.Участники.НайтиСтроки(Новый Структура("Пользователь", ПользовательСобытия));
				Если НайденныеСтроки.Количество() > 0 Тогда Продолжить; КонецЕсли;
				
				СтрокаПользователь = Форма.Объект.Участники.Добавить();
				СтрокаПользователь.Пользователь = ПользовательСобытия;
			КонецЦикла;
			Если Форма.Объект.Участники.Количество() > 0 Тогда
				Форма.Объект.Участники[0].Ответственный = Истина;
			КонецЕсли;
		КонецЕсли;
...
//
Процедура УстановитьНовогоОтветственного(Знач МассивДокументов, НовыйОтветственный)
...
Процедура ПеренаправитьСобытиеНаКлиенте(МассивПеренаправляемыхСобытий, СтруктураДанныхКлиент = Неопределено)
	...
	Если МассивИзбранныхПолучателей.Количество() > 0 Тогда
		...
		КонецЦикла;
		СписокВыбора.Добавить(ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка"), НСтр("ru='Выбрать из списка ...'"), , БиблиотекаКартинок.Лупа);
...
Процедура ПеренаправитьСобытиеИзКалендаря()
	...
	//	КонецПопытки;
	//	Если РасшифровкаЗначение <> Неопределено Тогда
	//		Если ТипЗнч(РасшифровкаЗначение.Событие) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") ИЛИ
	...
	// Оставлено для проверки, после исправления удалить.
	
	МассивПеренаправляемыхСобытий = Новый Массив;
	СтруктураДанныхКлиент = Новый Структура;	
	ПеренаправитьСобытиеНаКлиенте(МассивПеренаправляемыхСобытий, СтруктураДанныхКлиент);
	...
	Если ТипЗнч(СтруктураПараметрыГруппировки) = Тип("Структура") И СтруктураПараметрыГруппировки.Количество() > 0 Тогда
		Для Каждого ЗадачаСсылка Из МассивНеЛичныхЗадач Цикл
			Если Не ЗадачаСсылка.ПринятаКИсполнению Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Задача';en='Task'") + " """ + Строка(ЗадачаСсылка) + """ "
					+ НСтр("ru='не принята к исполнению, изменение реквизитов недоступно!';en='it is not accepted to execution, change of details are inaccessible!'"));
				//
				Продолжить;
			КонецЕсли;
...
Процедура ОбновитьСписокЗадачНаСервере()
	...
	Если бДобавлятьОтборПоИсполнителю Тогда
		...
		ГруппаИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(НастройкиСКД.Отбор.Элементы, НСтр("ru='Исполнитель';en='Executor'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
...
Функция ПолучитьСписокВыбораПериода()
	...
	Список.Добавить("ПроизвольныйПериод",	НСтр("ru='Произвольный период';en='The arbitrary period'"));
	Список.Добавить("Вчера",				НСтр("ru='Вчера';en='yesterday'"));
	Список.Добавить("Сегодня",				НСтр("ru='Сегодня';en='Today'"));
	Список.Добавить("Завтра",				НСтр("ru='Завтра';en='tomorrow'"));
	Список.Добавить("ПрошлаяНеделя",		НСтр("ru='Прошлая неделя';en='Last week'"));
	Список.Добавить("ЭтаНеделя",			НСтр("ru='Эта неделя';en='This week'"));
	Список.Добавить("СледующаяНеделя",		НСтр("ru='Следующая неделя';en='Next week'"));
	Список.Добавить("ПрошлыйМесяц",			НСтр("ru='Прошлый месяц';en='Last month'"));
	Список.Добавить("ЭтотМесяц",			НСтр("ru='Этот месяц';en='This month'"));
	Список.Добавить("СледующийМесяц",		НСтр("ru='Следующий месяц';en='Next month'"));
	Список.Добавить("ПрошлыйКвартал",		НСтр("ru='Прошлый квартал';en='Last quarter'"));
	Список.Добавить("ЭтотКвартал",			НСтр("ru='Этот квартал';en='This quarter'"));
	Список.Добавить("СледующийКвартал",		НСтр("ru='Следующий квартал';en='Next quarter'"));
	Список.Добавить("ПрошлыйГод",			НСтр("ru='Прошлый год';en='Last year'"));
	Список.Добавить("ЭтотГод",				НСтр("ru='Этот год';en='This year'"));
	Список.Добавить("СледующийГод",			НСтр("ru='Следующий год';en='Next year'"));
...
Функция ПолучитьОписаниеОтбораДляЭлемента(Элемент, КонтекстЭтаФорма)
	...
	ИначеЕсли ИмяПоляОтбора = "СтатусКонтрольнойТочки" Тогда
		...
		СтруктураРезультат.ОписаниеТиповОграничениеВыбора = Новый ОписаниеТипов("ПеречислениеСсылка.CRM_СтатусыЭтаповКалендарногоПлана");
		СтруктураРезультат.ПустоеЗначениеЗначение = ПредопределенноеЗначение("Перечисление.CRM_СтатусыЭтаповКалендарногоПлана.ПустаяСсылка");
		СтруктураРезультат.ВидСравненияПоУмолчанию = ВидСравненияКомпоновкиДанных.Равно;
		СтруктураРезультат.ЗначениеПоУмолчанию = ПредопределенноеЗначение("Перечисление.CRM_СтатусыЭтаповКалендарногоПлана.ПустаяСсылка");
		СтруктураРезультат.ИспользованиеПоУмолчанию = Ложь;
...
Функция ПолучитьСписокГруппировокСпискаКалендаря()
	...
	Список.Добавить("БезГруппировок", НСтр("ru='Без группировки';en='Without grouping'"));
...
Процедура ЗагрузитьНастройкиДеревоПользователейДиспетчеризации()
	...
	КонецЕсли;
	СтрокаКорень.Представление = НСтр("ru='Пользователи';en='Users'");
...
Процедура СформироватьКалендарьНаСервере(СтруктураДанныхКлиент)
	...
	Заголовок = НСтр("ru='Календарь';en='Calendar'") + " " + ПолучитьПредставлениеПериода(МинимальнаяДата, МаксимальнаяДата);
КонецПроцедуры
...
Функция РазметитьБланкСтрокиДень(	ТабДокументДень,
																	...
																	МассивПересекаемыхСобытийПоГоризонтальнымИндексам = Неопределено)
	//
		...
		Если НомерТекущегоДняНедели > 7 Тогда
			...
			Для Каждого СтрокаОбластьСобытия Из ТаблицаОбластиВыводаСобытий Цикл
				Если СтрокаОбластьСобытия.ОбластьРазвернуть = Истина Тогда
					...
					Область.Объединить();
					Область.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Обрезать;
					Область.Текст = "+" + НСтр("ru='еще';en='still'") + " " + СтрокаОбластьСобытия.КоличествоРазвернуть;
...
Процедура ВывестиДанныеНаДеньВКалендарьСписок(	ДатаДень,
	...
	Если СписокКалендаряГруппироватьПоДням Тогда
		...
		Если НачалоДня(ДатаДень) = НачалоДня(ДатаДеньВчера) Тогда
			СтрокаДобавить = НСтр("ru='вчера';en='yesterday'");
		ИначеЕсли НачалоДня(ДатаДень) = НачалоДня(ДатаДеньСегодня) Тогда
			СтрокаДобавить = НСтр("ru='сегодня';en='today'");
		ИначеЕсли НачалоДня(ДатаДень) = НачалоДня(ДатаДеньЗавтра) Тогда
			СтрокаДобавить = НСтр("ru='завтра';en='tomorrow'");
		КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ИсточникДанныхДляВывода Цикл
		...
		Если бВыводитьПользователей Тогда
			Попытка		МассивПользователей = СтрокаДанных.МассивПользователей;
			Исключение	МассивПользователей = Неопределено;
			КонецПопытки;
			Если МассивПользователей = Неопределено Или МассивПользователей.Количество() = 0 Тогда
				МассивПользователей = Новый Массив();
				МассивПользователей.Добавить(СтрокаДанных.Пользователь);
			КонецЕсли;
			Если Не ПустаяСтрока(СтрПредставление) Тогда
				СтрПредставление = СтрПредставление + " ";
			КонецЕсли;
			Для н = 0 По МассивПользователей.ВГраница() Цикл
				Если н = 0 Тогда
					СтрПредставление = СтрПредставление + "(";
				КонецЕсли;
				СтрПредставление = СтрПредставление + Строка(МассивПользователей[0]);
				Если н = МассивПользователей.ВГраница() Тогда
					СтрПредставление = СтрПредставление + ")";
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура СформироватьСписокНаСервере(СтруктураДанныхКлиент)
	...
	ОбластьКолонки.Параметры.ПериодЗаголовок = ?(СписокКалендаряГруппироватьПоДням, НСтр("ru='Время';en='Time'"), НСтр("ru='Период';en='Period'"));
...
Процедура СформироватьРасписаниеНаСервере(СтруктураДанныхКлиент);
	...
	Пока ТекДата <= НачалоДня(МаксимальнаяДата) Цикл
		...
		ОбластьЛинияСнизу.Текст = Формат(ТекДата, "ДФ='дд ММММ гггг'") + НСтр("ru='г';en='0'");
...
Процедура ЗаполнитьДанныеОбработки()
	...
	Для н = -ДобавитьМесяцев По ДобавитьМесяцев Цикл
		...
		ПредставлениеМесяца = Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='in'");
		СписокВыборМесяца.Добавить(ДобавляемыйМесяц, ПредставлениеМесяца);
	КонецЦикла;
	СтрЕще = НСтр("ru='Еще...';en='More...'");
	...
	Для Каждого ОбъектСтрока Из РегистрируемыеОбъекты Цикл
		Если	ОбъектСтрока.Объект = Метаданные.Документы.CRM_ЭтапКалендарногоПлана И
			(	НЕ	ПолучитьФункциональнуюОпцию("CRM_ИспользоватьПодсистемуУправленияПроектами")
				ИЛИ	НЕ ПравоДоступа("Добавление", Метаданные.Документы.CRM_ЭтапКалендарногоПлана)
			) Тогда
			//
		  Продолжить;
		КонецЕсли;
	...
	Если ИспользоватьБизнесПроцессыИЗадачи Тогда
		...
		НоваяСтрока.Представление = НСтр("ru='Задачи';en='Tasks'");
				...
				Для Каждого ТаблицаДеньКлючЗначение Из СоответствиеРезультат Цикл
					...
					Для Каждого СтрокаЗадача Из ТаблицаОтображаемыхЗадач Цикл
						Если СтрокаЗадача.СрокИсполнения >= НачалоДня(ТаблицаДеньКлючЗначение.Ключ) И СтрокаЗадача.СрокИсполнения <= КонецДня(ТаблицаДеньКлючЗначение.Ключ) Тогда
							...
							НоваяСтрока.Тема					= СтрЗаменить(НСтр("ru='Задача: %Тема%';en='Task: %tema %'"), "%Тема%", СтрокаЗадача.Тема);
							НоваяСтрока.СобытиеНаВесьДень		= Истина;
		...
		Если СписокКалендаряГруппироватьПоДням Тогда
			...
			Если бИспользуетсяПолнотекстовыйПоискСписка Тогда
				...
				Если Не ЗначениеЗаполнено(ТекстОшибкиГлобальный) И РезультатТаблица.Количество() = 0 Тогда
					ТекстОшибкиГлобальный = НСтр("ru='Ничего не найдено';en='It is found nothing'");
				КонецЕсли;
			Иначе
				МассивВыбранныхДат = СтруктураДанныхКлиент.МассивВыбранныхДат;
			КонецЕсли;
	...
	Если РасширитьВниз Тогда
		...
		Для н = 1 По ДобавитьМесяцев Цикл
			...
			Если н = 1 Тогда
				...
				СписокВыборМесяца[СписокВыборМесяца.Количество() - 1].Представление = Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='in'");
			Иначе
				СписокВыборМесяца.Добавить(ДобавляемыйМесяц, Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='in'"));
			КонецЕсли;
		КонецЦикла;
		СписокВыборМесяца.Добавить(Дата("99990101"), СтрЕще);
	Иначе 
		...
		Для н = 1 По ДобавитьМесяцев Цикл
			...
			Если н = 1 Тогда
				...
				СписокВыборМесяца[0].Представление = Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='in'");
			Иначе
				СписокВыборМесяца.Вставить(0, ДобавляемыйМесяц, Формат(ДобавляемыйМесяц, "ДФ='ММММ гггг'") + " " + НСтр("ru='г.';en='in'"));
			КонецЕсли;
		КонецЦикла;
		СписокВыборМесяца.Вставить(0, Дата("00010101"), СтрЕще);
	КонецЕсли;
...
Процедура НастроитьВидимостьИДоступностьФормы()
	...
	Если Объект.ТекущееОтображение = "Неделя" Тогда
		Элементы.КнопкаКалендарьСегодня.Заголовок = НСтр("ru='Текущая неделя';en='Current week'");
	ИначеЕсли Объект.ТекущееОтображение = "Месяц" Тогда
		Элементы.КнопкаКалендарьСегодня.Заголовок = НСтр("ru='Текущий месяц';en='Current month'");
	Иначе
		Элементы.КнопкаКалендарьСегодня.Заголовок = НСтр("ru='Сегодня';en='Today'");
	КонецЕсли;
		...
		Пока ИндексТекущейВыделеннойЗадачи < МассивЗадач.Количество() Цикл
			...
			Если ЗапрещеноРедактированиеСрокаВыполненияЗадачи(ТекущаяВыделеннаяЗадача) Тогда
				...
				Если МассивЗадачКоличество = 1 Тогда
					...
					ТекстСообщения = НСтр("ru='Запрещено редактирование срока выполнения текущей задачи.';en='It is forbidden to edit the deadline for the current task.'");
...
Процедура СообщениеОНевозможностиГрупповойОбработкиЗадач(МассивЗадачНепригодныхДляГрупповойОбработки)
	...
	ПоказатьОповещениеПользователя(
		...
		СтрЗаменить(НСтр("ru='Невозможна групповая обработка: %Обработка%';en='Group handling are impossible: %obrabotka %'"), "%Обработка%", Строка(МассивЗадачНепригодныхДляГрупповойОбработки[0])));
	//
КонецПроцедуры
