...
Процедура УстановитьПараметрыРегламентногоЗадания(РегламентноеЗаданиеОбъект, РасписаниеРегламентногоЗадания, ТекущийОбъект)
	...
	НаименованиеРегламентногоЗадания = НСтр("ru='Формирование состава значений классификатора: %1';en='Formation of the classifier values:%1'");
...
Процедура ЗаписатьРегламентноеЗадание(Отказ, РегламентноеЗаданиеОбъект)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru='Произошла ошибка при сохранении расписания. Подробное описание ошибки: %1';en='An error occurred while saving the schedule. Detailed error description:%1'");
...
Процедура ВключениеИспользованияТегов() Экспорт
	
	Если НЕ Константы.ИспользоватьРазделениеПоОбластямДанных.Получить() Тогда
		ПВХАналитика = ПланыВидовХарактеристик.CRM_АналитикаКлассификаторов.СоздатьЭлемент();
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Тип("СправочникСсылка.СтруктураПредприятия"));
		ПВХАналитика.ТипЗначения = Новый ОписаниеТипов(МассивТипов);
		ПВХАналитика.Наименование = "Подразделение";
		ПВХАналитика.ВариантПолучения = Перечисления.CRM_ВариантыПолученияАналитикиКлассификатора.НастройкиПользователя;
		ПВХАналитика.Записать();
		
		ПВХКлассификатор = ПланыВидовХарактеристик.CRM_Классификаторы.СоздатьЭлемент();
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Тип("СправочникСсылка.Партнеры"));
		ПВХКлассификатор.ТипЗначения = Новый ОписаниеТипов(МассивТипов);
		ПВХКлассификатор.Наименование = "Теги";
		ПВХКлассификатор.ВидОтображения = Перечисления.CRM_ВидыОтображенияКлассификаторов.Тег;
		ПВХКлассификатор.СпособФормирования = Перечисления.СпособыФормированияСегментов.ФормироватьВручную;
		ПВХКлассификатор.ИспользуемаяАналитика = ПВХАналитика.Ссылка;
		ПВХКлассификатор.ИнтерактивныйВводЗначений = Истина;
		СтрПринадлежность = ПВХКлассификатор.Принадлежность.Добавить();
		СтрПринадлежность.ИмяТаблицы = "Справочник.Партнеры";
		ПВХКлассификатор.Записать();
		
		Константы.CRM_ИспользоватьКлассификаторы.Установить(Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПереносТеговВКлассификаторы() Экспорт
	
	КлассификаторТегов = ПланыВидовХарактеристик.CRM_Классификаторы.НайтиПоНаименованию("Теги");
	
	Если КлассификаторТегов = Неопределено Тогда Возврат; КонецЕсли;
	
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      |	ПартнерыТеги.Тег.Наименование КАК ТегНаименование
	                      |ИЗ
	                      |	Справочник.Партнеры.УдалитьCRM_Теги КАК ПартнерыТеги");
	
	ВыборкаТеги = Запрос.Выполнить().Выбрать();
	Пока ВыборкаТеги.Следующий() Цикл
		ЗначениеКлассификатора = Справочники.CRM_ЗначенияКлассификаторов.СоздатьЭлемент();
		ЗначениеКлассификатора.Владелец = КлассификаторТегов;
		ЗначениеКлассификатора.Наименование = ВыборкаТеги.ТегНаименование;
		ЗначениеКлассификатора.Записать();
	КонецЦикла;
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ПартнерыТеги.Ссылка КАК Объект,
	                      |	CRM_ЗначенияКлассификаторов.Ссылка КАК Значение,
	                      |	ПартнерыТеги.Тег.Владелец КАК Владелец
	                      |ИЗ
	                      |	Справочник.Партнеры.УдалитьCRM_Теги КАК ПартнерыТеги
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.CRM_ЗначенияКлассификаторов КАК CRM_ЗначенияКлассификаторов
	                      |		ПО ПартнерыТеги.Тег.Наименование = CRM_ЗначенияКлассификаторов.Наименование
	                      |ИТОГИ ПО
	                      |	Объект");
	
	Выборка = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока Выборка.Следующий() Цикл
		ВыборкаТег = Выборка.Выбрать();
		...
		Пока ВыборкаТег.Следующий() Цикл
			...
			ЗаписьРегистра.Объект = ВыборкаТег.Объект;
			ЗаписьРегистра.Аналитика = ВыборкаТег.Владелец;
			ЗаписьРегистра.ЗначениеКлассификатора = ВыборкаТег.Значение;
		КонецЦикла;
