...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если НЕ ЗначениеЗаполнено(Запись.Получатель) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен получатель';en='Incomplete recipient'"));
		Отказ = Истина;
	ИначеЕсли Запись.Получатель = Запись.Пользователь Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен получательи пользователь не должны совпадать';en='The recipient is not full and the user does not have to match'"));
		Отказ = Истина;
	КонецЕсли;		
КонецПроцедуры
