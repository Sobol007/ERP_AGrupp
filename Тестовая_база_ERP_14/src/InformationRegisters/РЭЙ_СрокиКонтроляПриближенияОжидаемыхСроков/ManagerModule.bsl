Функция ПолучитьИнициализационныеНастройки(ДатаНастроек)
	...
	Для Каждого текКолонка Из Метаданные.РегистрыСведений.РЭЙ_СрокиКонтроляПриближенияОжидаемыхСроков.Измерения Цикл
		ИнициализационнаяТаблица.Колонки.Добавить(текКолонка.Имя, текКолонка.Тип);
	КонецЦикла;
	Для Каждого текКолонка Из Метаданные.РегистрыСведений.РЭЙ_СрокиКонтроляПриближенияОжидаемыхСроков.Ресурсы Цикл
		ИнициализационнаяТаблица.Колонки.Добавить(текКолонка.Имя, текКолонка.Тип);
	КонецЦикла;
	...
	Для текСтрока = 2 По Макет.ВысотаТаблицы Цикл
		...
		НоваяСтрока.ВидСправки = СокрЛП(Макет.Область(текСтрока, 2).Текст);
...
Процедура ОчиститьРегистр(Дата) Экспорт
	...
	|	Регистр.Период
	|ИЗ
	|	РегистрСведений.РЭЙ_СрокиКонтроляПриближенияОжидаемыхСроков КАК Регистр
	|ГДЕ
