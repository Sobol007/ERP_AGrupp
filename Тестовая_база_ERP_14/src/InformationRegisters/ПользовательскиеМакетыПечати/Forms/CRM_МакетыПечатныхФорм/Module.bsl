...
Процедура ЗаполнитьТаблицуМакетовПечатныхФорм()
	...
	Для Каждого ОбъектМетаданныхМакет Из Метаданные.ОбщиеМакеты Цикл
		...
		Если ТипМакета = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ДобавитьОписаниеМакета("ОбщийМакет." + ОбъектМетаданныхМакет.Имя, ОбъектМетаданныхМакет.Синоним, НСтр("ru='Общий макет';en='Common template'"), ТипМакета);
	КонецЦикла;
	// +CRM
...
Процедура CRM_ДобавитьВнешниеМакетыПечатныхФорм()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если МассивСтрок.Количество() = 0 Тогда
			ДанныеВнешнегоМакета = Выборка.Макет.Получить();
			
			Если ТипЗнч(ДанныеВнешнегоМакета) = Тип("ТабличныйДокумент") Тогда
				ТипМакета = "MXL";
			ИначеЕсли ТипЗнч(ДанныеВнешнегоМакета) = Тип("ДвоичныеДанные") Тогда
				ТипМакета = ВРег(УправлениеПечатьюСлужебный.ОпределитьРасширениеФайлаДанныхПоСигнатуре(ДанныеВнешнегоМакета));
			КонецЕсли;
