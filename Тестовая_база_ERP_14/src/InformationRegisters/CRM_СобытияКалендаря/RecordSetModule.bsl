...
Перем Объект Экспорт;	 // Содержит текущий объект, событие календаря.
Перем Параметры Экспорт; // Параметры работы с календарем. 
...
Процедура ПеренестиОбъект_Документ_CRM_ЭтапКалендарногоПлана(Ошибки)
	// Если нужно переносить документ CRM_ЭтапКалендарногоПлана на другую дату
...
Функция ПеренестиОбъект() Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект) Тогда
		Ошибки = Ошибки + ?(ПустаяСтрока(Ошибки), "", Символы.ПС);
		Ошибки = Ошибки + НСтр("ru='Не выбран объект для переноса!';en='The object for transfer are not selected!'");
	КонецЕсли;
	Если ТипЗнч(Параметры) <> Тип("Структура") Тогда
		Ошибки = Ошибки + ?(ПустаяСтрока(Ошибки), "", Символы.ПС);
		Ошибки = Ошибки + НСтр("ru='Данные для действия не инициализированы!';en='Data for action are not initializ!'");
	КонецЕсли;
	Если ТипЗнч(Параметры) = Тип("Структура") И (Не Параметры.Свойство("ПеренестиНаДатуВремя") Или Не ЗначениеЗаполнено(Параметры.ПеренестиНаДатуВремя)) Тогда
		Ошибки = Ошибки + ?(ПустаяСтрока(Ошибки), "", Символы.ПС);
		Ошибки = Ошибки + НСтр("ru='Не указана дата на которую переносится объект!';en='Date on which are not specified the object are transfer!'");
	КонецЕсли;
	...
	Если ТипЗнч(Объект) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
		ПеренестиОбъект_Документ_CRM_ЭтапКалендарногоПлана(Ошибки);
...
Процедура УдалитьОбъект_Документ_CRM_ЭтапКалендарногоПлана(УдаляемыйОбъект, Ошибки)
	УдалитьСсылочныйОбъектПоУмолчанию(УдаляемыйОбъект, Ошибки, Ложь);
КонецПроцедуры
...
Функция УдалитьОбъект() Экспорт
	...
	Для Каждого УдаляемыйОбъект Из УдаляемыеОбъекты Цикл
		...
		ИначеЕсли ТипЗнч(УдаляемыйОбъект.Объект) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
			УдалитьОбъект_Документ_CRM_ЭтапКалендарногоПлана(УдаляемыйОбъект, Ошибки)
		Иначе
