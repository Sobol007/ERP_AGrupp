...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если НЕ CRM_ЛицензированиеСервер.ПодсистемаCRMИспользуется() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно открыть ""Мои трудозатраты"". Подсистема 1С:CRM не используется! (см. Персональные настройки пользователя)';en='It''s not possible to open ""My work"". Subsystem 1C: CRM is not used! (see Personal settings of the user)'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура СформироватьЗаголовок()
	...
	ЭтотОбъект.Заголовок = НСтр("ru='Трудозатраты';en='Expenses of labor'")+?(ТекущийПользователь.Пустая(), "", " / "+ ТекущийПользователь) +?(ЗначениеЗаполнено(ПредставлениеПериода), " / " + ПредставлениеПериода, "");
...
Функция ПроверитьДоступностьУдаления(Пользователь)
	...
	Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		РуководительПодразделения = Пользователь.Подразделение.ТекущийРуководитель;
	Иначе
		Запрос = Новый Запрос;
		...
		Запрос.УстановитьПараметр("ФизическоеЛицо", Пользователь.Подразделение.ТекущийРуководитель);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			РуководительПодразделения = Выборка.Пользователь;
		Иначе
			РуководительПодразделения = Неопределено;
		КонецЕсли;
	КонецЕсли;
	
	Возврат (Пользователь <> Пользователи.АвторизованныйПользователь()) И (НЕ РольДоступна("ПолныеПрава")) И (НЕ РуководительПодразделения = Пользователи.АвторизованныйПользователь());
