...
Процедура ПередЗаписью(Отказ)
	...
	РаботаСФайлами.УстановитьПометкуУдаленияФайловДокументовПередЗаписью(ЭтотОбъект, Ложь, РежимЗаписиДокумента.Запись, РежимПроведенияДокумента.Неоперативный);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЭтотОбъект.Стартован И ЭтотОбъект.ДатаСтарта < ТекущаяДатаСеанса() И ЭтотОбъект.ДатаСтарта <> Дата('00010101')И (НачалоДня(ЭтотОбъект.ДатаСтарта) <> НачалоДня(ЭтотОбъект.Дата)) Тогда
		...
		ТекстСообщения = НСтр("ru='Дата старта не может быть меньше текущей даты.';en='Start date must not be earlier than current date!'");
...
Процедура СтартПередСтартом(ТочкаМаршрутаБизнесПроцесса, Отказ)
	...
	Если мНеСтартовать Тогда
		Если ТекстСообщения = "" Тогда
			ТекстСообщения = НСтр("ru='Бизнес-процесс не удалось стартовать.';en='Business process could not be started.'")
		КонецЕсли;
