		...
		Если ВидКонтроляЗадачиПоИсправлениюКлиентскойБазы = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ДублированиеКлиентов Тогда
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				...
				Если Не МенеджерЗаписи.Выбран() Тогда
					ВызватьИсключение НСтр("ru='Не найден ключ записи';en='The key of record is not found'");
				КонецЕсли;
...
Процедура ДобавлениеКнопокНаПанельОбъектов()
	...
	Если ТаблицаТипов.Количество() > 0 Тогда
		...
		НоваяКомандаФормы.Заголовок                 = НСтр("ru='На основании задачи';en='Based on the task'");
		НоваяКомандаФормы.Действие                  = "ВвестиОбъект";
		НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Истина;
		НоваяКомандаФормы.Подсказка					= НСтр("ru='Создать новый документ на основании текущей задачи';en='Create a new document based on the current task'");				
	...
	Если ТаблицаТипов.Количество() = 1 Тогда
		...
		НовыйПунктМеню.Заголовок			= НСтр("ru='Создать ""%Синоним%""';en='Create ""% Synonym%""'");
		...
		НовыйПунктМеню.Заголовок			= НСтр("ru='Выбрать ""%Синоним%""';en='Select ""% Synonym%""'");
		НовыйПунктМеню.Заголовок			= СтрЗаменить(НовыйПунктМеню.Заголовок, "%Синоним%", Синоним);								
		...
		НоваяКомандаФормы.Действие                  = "ВвестиОбъект";
		НоваяКомандаФормы.ИзменяетСохраняемыеДанные = Истина;
		НоваяКомандаФормы.Подсказка					= НСтр("ru='Создать новый документ %Синоним% на основании текущей задачи';en='Create a new document% Synonym% based on the current task'");
		НоваяКомандаФормы.Подсказка = СтрЗаменить(НоваяКомандаФормы.Подсказка, "%Синоним%", Синоним);		
		...
		НовыйПунктМеню.Заголовок			= НСтр("ru='На основании задачи';en='Based on the task'");
		НовыйПунктМеню.Картинка				= БиблиотекаКартинок.СоздатьЭлементСписка;
		НовыйПунктМеню.Отображение			= ОтображениеКнопки.КартинкаИТекст;								
	...
	ИначеЕсли ТаблицаТипов.Количество() > 1 Тогда
		...
		ПунктМенюДобавить.Заголовок	= НСтр("ru='На основании задачи';en='Based on the task'");
...
Процедура ВыполнитьПослеВыбораДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		Если ВыбраннаяДата < ТекущаяДата() Тогда
			ТекстСообщения = НСтр("ru='Укажите дату больше или равную текущей!';en='Please enter a date greater than or equal to the current date!'");
