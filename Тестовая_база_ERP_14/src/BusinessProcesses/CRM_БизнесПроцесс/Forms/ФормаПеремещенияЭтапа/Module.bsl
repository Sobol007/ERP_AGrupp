...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеСервер.ПодсистемаCRMИспользуется() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно открыть форму перемещения этапа. Подсистема 1С:CRM не используется! (см. Персональные настройки пользователя)';en='You can not open the phase transfer form. Subsystem 1C: CRM is not used! (see Personal settings of the user)'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если CRM_ЛицензированиеСервер.ВариантПоставкиСТАРТ() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Функция не доступна для варианта поставки ""СТАРТ""!';en='The function is not available for the ""START"" option!'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура КартаМаршрутаВыбор(Элемент)
	...
	Если (ТипЗнч(Элемент.ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыДействие"))
		...
		ТекстВопроса = НСтр("ru='Установить';en='Set'") + " """+ТекДанные.ТочкаМаршрута + """" + НСтр("ru = ' как текущий этап бизнес-процесса?'") + "
...
#Область ОбработчикиСобытийЭлементовТаблицыФормытабМаршрут_
