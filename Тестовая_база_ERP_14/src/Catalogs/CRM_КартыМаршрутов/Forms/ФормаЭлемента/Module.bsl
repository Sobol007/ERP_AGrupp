...
&НаКлиенте
Перем ИмяТекущегоЭлементаСхемы, РедактируемыйЭлементСхемы;
...
Функция ПроверитьНаНаличиеИзменений()
	...
	Для Каждого Реквизит Из РеквизитыОбъекта Цикл
		...
		Если ИмяРеквизита = Нстр("ru='ГрафическаяСхема';en='Graphic Scheme'")
			ИЛИ ИмяРеквизита = Нстр("ru='Редактируется';en='It is edit'")  Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПрочитатьГрафическуюСхемуСформироватьДеревоТочек()
	...
	Для Каждого СтрокаТаб Из СтруктураДанных.ТабТочекМаршрута Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаб);
		СтрокиИзмененныхИмен = ИзмененныеИменаТочек.НайтиСтроки(Новый Структура("НовоеИмя", НоваяСтрока.ИмяЭлемента));
		Если СтрокиИзмененныхИмен.Количество()>0 Тогда
			НоваяСтрока.СтароеИмяЭлемента = СтрокиИзмененныхИмен[0].СтароеИмя;
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	Элементы.УказыватьСрокВыполненияЭтапов.Доступность = (Объект.ТипПроцесса <> ПредопределенноеЗначение("Перечисление.bpmТипыПроцессов.ПроцессОбъекта"));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеСервер.ПодсистемаCRMИспользуется() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно открыть карту маршрута. Подсистема 1С:CRM не используется! (см. Персональные настройки пользователя)';en='Unable to open route map. Subsystem 1C: CRM is not used! (see Personal settings of the user)'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Объект.ДинамическиРассчитыватьСрокиВыполненияЭтапов = Истина;
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовПоТипу(ПриСоздании = Ложь)
	...
	Если Объект.УказыватьСрокВыполненияЭтапов И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
		Объект.УказыватьСрокВыполненияЭтапов = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.УказыватьСрокВыполненияЭтапов.Доступность = (Объект.ТипПроцесса <> Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	...
	Элементы.СценарийИнтереса.Видимость = (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта И Объект.ВидОбъектаСтрокой	= "CRM_Интерес");
		...
		Если Выборка.ТипЗначения <> NULL
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				Если РезультатыЗапроса[1].Пустой() Тогда
					ПредставлениеЗначений = НСтр("ru='Значения еще не введены';en='Values ​​not yet entered'");
				Иначе
					ПредставлениеЗначений = НСтр("ru='Значения помечены на удаление';en='Values ​​are marked for deletion'");
				КонецЕсли;
			Иначе
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если ЗавершениеРаботы Тогда
		...
		Если НЕ ПустаяСтрока(ТекстПредупреждения) Тогда
	        Отказ = Истина;
			ТекстПредупреждения = ТекстПредупреждения + НСтр("ru='При закрытии формы все данные будут утеряны. Вы хотите';en='When the form is closed, all data will be lost. Do you want to'");
		КонецЕсли;	
		Возврат;
	КонецЕсли;
	Если НЕ (ОтветПередЗакрытием = Ложь) Тогда
		...
		Если Объект.Редактируется Тогда
			...
			ПоказатьВопрос(ОбратныйВызов, НСтр("ru='Внимание! Карта маршрута находится в режиме редактирования. Продолжить c закрытием формы?';en='Attention! The route map is in edit mode. Continue with closing the form?'"), РежимДиалогаВопрос.ДаНет);
		Иначе
...
Процедура ПередЗакрытиемЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПроверитьНаНаличиеИзменений() Тогда 
		...
		ПоказатьВопрос(ОбратныйВызов, Нстр("ru='Форма была изменена. Сохранить изменения?';en='The form was changed. Save changes?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура ЗагрузитьИзФайлаНаДиске(Команда)
	...
	Если РасширениеПодключено Тогда
		...
		Если Объект.Редактируется Тогда
			ВыборФайла.Фильтр = НСтр("ru='Графическая схема (.grs) (*.grs)|*.grs';en='The graphical scheme (.grs) (* .grs) | * .grs'");
		Иначе
	...
	Иначе 
		ПоказатьПредупреждение(, НСтр("ru='В Веб-клиенте без установленного расширения работы с файлами добавление файлов не поддерживается.';en='Web customers with no file treatment extensions installed do not support files adding.'"));
	КонецЕсли;
...
Процедура СохранитьВФайл(Команда)
	...
	Иначе  // веб клиент
		ПоказатьПредупреждение(, НСтр("ru='В Веб-клиенте без установленного расширения работы с файлами данная функция не поддерживается.';en='In the Web customer without the file extension installed, this function is not supported.'"));
	КонецЕсли;
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Если СтруктураДанных.НеКритичныеОшибки.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='При проверке карты маршрута обнаружено';en='At check of a route map it is revealed'")+" "+Строка(СтруктураДанных.НеКритичныеОшибки.Количество()) + НСтр("ru = ' предупреждений. |Выполнить обновление точек маршрута?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура Редактировать(Команда)
	...
	Объект.Редактируется = Истина;
	УстановитьСвойстваЭлементовФормы();
	ТекущийЭлемент = Элементы.СхемаМаршрута;
...
Процедура ОтменитьРедактирование(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Будут отменены все изменения, сделанные в карте маршрута. Продолжить?';en='All the changes made to the route map will be cancelled. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
		...
		Если Объект.ВидОбъектаСтрокой = "CRM_Интерес" Тогда
			ПроверяемыеРеквизиты.Добавить("СценарийИнтереса");
		КонецЕсли;
...
Процедура ПолучитьСписокДоступныхОбъектов()
	...
	Для Каждого ТипОбъекта ИЗ Метаданные.ОпределяемыеТипы.CRM_ПроцессныеОбъекты.Тип.Типы() Цикл
		...
		Если НЕ ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(МетаДокумент)
			ИЛИ НРег(Лев(МетаДокумент.Имя,7)) = "удалить" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СхемаМаршрутаПриАктивизации(Элемент)
	Если Элемент.ТекущийЭлемент <> Неопределено Тогда
		ИмяТекущегоЭлементаСхемы = Элемент.ТекущийЭлемент.Имя;
		РедактируемыйЭлементСхемы = Элемент.ТекущийЭлемент;
	КонецЕсли;
	ТекущийЭлемент = Элементы.СхемаМаршрута;
КонецПроцедуры

&НаКлиенте
Процедура СхемаМаршрутаПриИзменении(Элемент)
	Если РедактируемыйЭлементСхемы <> Неопределено И РедактируемыйЭлементСхемы.Имя <> ИмяТекущегоЭлементаСхемы Тогда
		СтрокиИзмененныхИмен = ИзмененныеИменаТочек.НайтиСтроки(Новый Структура("НовоеИмя", ИмяТекущегоЭлементаСхемы));
		Если СтрокиИзмененныхИмен.Количество() = 0 Тогда
			НовСтрока = ИзмененныеИменаТочек.Добавить();
			НовСтрока.СтароеИмя = ИмяТекущегоЭлементаСхемы;
			НовСтрока.НовоеИмя = РедактируемыйЭлементСхемы.Имя;
		Иначе
			Если СтрокиИзмененныхИмен[0].СтароеИмя = РедактируемыйЭлементСхемы.Имя Тогда
				ИзмененныеИменаТочек.Удалить(ИзмененныеИменаТочек.Индекс(СтрокиИзмененныхИмен[0]));
			Иначе
				СтрокиИзмененныхИмен[0].НовоеИмя = РедактируемыйЭлементСхемы.Имя;
			КонецЕсли;
		КонецЕсли;
		Если Элемент.ТекущийЭлемент = Неопределено Тогда
			ИмяТекущегоЭлементаСхемы = Неопределено;
			РедактируемыйЭлементСхемы = Неопределено;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
