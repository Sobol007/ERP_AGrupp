...
Процедура РедактироватьСхемуКомпоновкиДанных(Команда)
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru='Настройка шаблона расчета для целевого показателя ""%1""';en='Set up a billing template for target "" %1""'");
...
Процедура РедактироватьСхемуКомпоновкиДанныхРасшифровки(Команда)
	...
	// Открыть редактор настроек схемы компоновки данных
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru='Настройка шаблона расшифровки для целевого показателя ""%1""';en='Configure the decryption template for target "" %1""'");
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = СтрЗаменить(ЗаголовокФормыНастройкиСхемыКомпоновкиДанных, "%1", Объект.Наименование);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	|ГДЕ
	|	Виджеты.ИсточникДанных = &Показатель");
	...
	Если РезультатЗапроса.Пустой() Тогда
		Если Объект.Включен Тогда
			...
			ВиджетОбъект.ИсточникДанных = Объект.Ссылка;
			ВиджетОбъект.Записать();
		КонецЕсли;
	Иначе
		Если Не Объект.Включен Тогда
			Выборка = РезультатЗапроса.Выбрать();
			Выборка.Следующий();
			НаборЗаписей = РегистрыСведений.CRM_НастройкиВиджетов.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Виджет.Установить(Выборка.Ссылка);
