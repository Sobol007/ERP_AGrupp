...
Процедура ВидыТелефоновПриАктивизацииСтроки(Элемент)
	...
	//	ТД.Вид												= НСтр("ru='Внутренний номер';en='Internal number'");
...
Процедура ДобавитьГруппуВТелефоннуюКнигу(Команда)
	...
	ПоказатьВводСтроки(ОписаниеВвода, "", НСтр("ru='Введите наименование новой группы';en='Enter the name of the new group'"), , Ложь); 	
...
Процедура ПроверитьЗаполнениеТаблицыВидовТелефонов()
	Для Каждого СтрокаВида Из Объект.ВидыТелефонов Цикл
		Если НЕ СтрокаВида.Пометка Тогда
			Продолжить;
		//ИначеЕсли СтрокаВида.Наименование = НСтр("ru='Внутренний номер';en='Internal number'") Тогда 	
		//	Продолжить;
