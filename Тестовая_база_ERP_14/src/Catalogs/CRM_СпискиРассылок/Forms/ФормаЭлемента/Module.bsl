...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Проверим что в метаданных есть ФО "CRM_ИспользоватьСегментацию" и установим 
	// видимость кнопки "Заполнить по сегменту".
...
Процедура СоставАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокАдресов.ПоказатьВыборЭлемента(Оповещение, Нстр("ru='Выберите адрес';en='Choose an address'"), ЭтотОбъект);
...
Процедура ЗаполнитьПоМаркетинговойКампанииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если НЕ (Результат = Неопределено) Тогда
		Если Объект.Состав.Количество() > 0 Тогда
			ТекстВопроса = Нстр("ru='Cписок получателей содержит записи. Заменить или дополнить?';en='The recipient list contains entries. Replace or supplement?'");
