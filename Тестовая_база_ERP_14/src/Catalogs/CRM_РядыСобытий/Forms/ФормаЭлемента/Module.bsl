...
Процедура ПриОткрытии(Отказ)
	Попытка
		Если ТипЗнч(ВладелецФормы.Объект.Ссылка) <> Тип("ДокументСсылка.CRM_Взаимодействие") Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	Исключение
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОбратныйВызов, НСтр("ru='Данные были изменены. Сохранить данные?';en='Data has been changed. Save data?'"), РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;	
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если МассивДатРасписания.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Расписание настроенно некорректно - в указанный период не попадает ни одного события!';en='The schedule are adjust incorrectly - to the specified period did not get any event!'"));
	Иначе
		Модифицированность = Ложь;
...
Функция ПроверитьНаКорректность()
	Если Не ЗначениеЗаполнено(Объект.ДатаНачала) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата начала должна быть заполнена!';en='The date started should be fill!'"),, "ДатаНачала", "Объект");
		Возврат Ложь;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.ДатаОкончания) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата окончания должна быть заполнена!';en='The expiry date should be fill!'"),, "ДатаОкончания", "Объект");
		Возврат Ложь;
	КонецЕсли;
	
	Если НачалоДня(Объект.ДатаОкончания) < НачалоДня(Объект.ДатаНачала) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата окончания должна быть больше даты начала!';en='The expiry date should be more date started!'"),, "ДатаОкончания", "Объект");
		Возврат Ложь;
	КонецЕсли;
	
	//Если Не ЗначениеЗаполнено(ВремяНачала) Тогда
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время начала должно быть заполнено!';en='Starting time should be fill!'"),, "ВремяНачала");
	//	Возврат Ложь;
	//КонецЕсли;
	//
	//Если Не ЗначениеЗаполнено(ВремяКонца) Тогда
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время окончания должно быть заполнено!';en='Ending time should be fill!'"),, "ВремяКонца");
	//	Возврат Ложь;
	//КонецЕсли;
	
	//Если ЗначениеЗаполнено(ВремяКонца) И ВремяКонца < ВремяНачала Тогда
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время окончания должно быть больше времени начала!';en='Ending time should be more starting time!'"),, "ВремяКонца");
	//	Возврат Ложь;
	//КонецЕсли;
