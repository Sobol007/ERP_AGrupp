...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл" Тогда
		Если ФормаОткрытаИзОтветов Тогда
			Если Параметр.Свойство("ЭтоНовый") И Параметр.ЭтоНовый И Параметр.ВладелецФайла = Объект.Ссылка Тогда
...
Процедура ПрисоединенныеФайлы(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПрисоединенныеФайлыЗавершение", ЭтотОбъект, Ложь);
...
Процедура СформироватьПредставлениеСвязанныхРешений()
	Если Элементы.СвязанныеРешения.Видимость Тогда
		ПредставлениеСвязанныхРешений	= НСтр("ru='свернуть связанные решения';en='to contract the bound decisions'");
	Иначе	
		...
		Если КоличествоРешенийВсего = 0 Тогда
			ПредставлениеСвязанныхРешений	= НСтр("ru='нет связанных решений';en='there was no bound decisions'");
		Иначе
