...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация")
		И ДействияСПользователемСервиса <> Неопределено Тогда
			// +CRM (Закомментировано для Fresh)
			//МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ЗаголовокВопросов = НСтр("ru='Запись пользователя информационной базы';en='Record of the user of information base'");
	   ...
	   И (Не ПараметрыЗаписи.Свойство("НеКопироватьПраваПользователя")
		...
		ПоказатьВопрос(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Скопировать также права пользователя ""%1""?';en='Do you also copy user rights "" %1""?'"), Строка(ЗначениеКопирования)),
		   ...
		   И ПользовательИБРоли.Количество() = 0 Тогда
			...
			Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
				...
				ПоказатьВопрос(
					...
					НСтр("ru='Пользователю информационной базы не установлено ни одной роли. Продолжить?';en='The user of information base it is not installed any role. Continue?'"),
					РежимДиалогаВопрос.ДаНет,
					,
					,
					ЗаголовокВопросов);
	...
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект) Тогда
		...
		Если ПараметрыЗаписи.Свойство("ССозданиемПервогоАдминистратора") Тогда
			ТекущийОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
				НСтр("ru='Первый пользователь информационной базы назначается администратором.';en='Add administrator as the first user of the infobase.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ВходВПрограммуРазрешен
	   ...
	   И СрокДействия <= НачалоДня(ТекущаяДатаСеанса()) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Ограничение должно быть до завтра или более.';en='The restriction must be up to tomorrow or more.'"),, "ВходВПрограммуРазрешен",, Отказ);
	КонецЕсли;
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" не найдена в метаданных.';en='The role "" %1"" is not found in the metadata.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 не найдена в метаданных.';en='The role of "" %1"" in line % %1 is not found in the metadata.'"), Строка.Синоним));
			КонецЕсли;
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" недоступна для пользователей.';en='Role "" %1"" is not available to users.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 недоступна для пользователей.';en='The role "" %1"" in line % %1 is not available to users.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
Процедура ВходВПрограммуРазрешенПриИзменении(Элемент)
	...
	Если Не УровеньДоступа.ИзменениеРазрешенияНаВход
	   И Не ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru='После записи вход в программу сможет разрешить только администратор.';en='After writing, logon to the application can be allowed only by administrator.'"));
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='ПолныеПрава';en='0'");
...
Процедура СменитьПарольПослеПолученияПароля(Результат, Контекст) Экспорт
	...
	УстановитьНаличиеПароля(ЭтотОбъект, ЗначениеЗаполнено(ПользовательИБПароль), ПользователиКлиент.АвторизованныйПользователь());
	УстановитьДоступностьСвойств(ЭтотОбъект);
...
Процедура ПроверитьПервогоАдминистратора() 
	...
	ЗаголовокВопроса = НСтр("ru='Запись пользователя информационной базы';en='Record of the user of information base'");
	ПоказатьВопрос(
...
Функция ПодсказкаПотребоватьСменуПароляПриВходе()
	...
	Подсказка.Добавить(НСтр("ru='Требования к длине и сложности пароля задаются отдельно.';en='The requirements for the length and complexity of the password are set separately.'"));
	Подсказка.Добавить(Символы.ПС);
	Подсказка.Добавить(НСтр("ru='См.';en='Cm.'"));
	Подсказка.Добавить(" ");
	
	Ссылка = НСтр("ru='Настройки входа пользователей';en='User logon settings'");
...
Процедура ПроверитьЗаполнениеEmail(Отказ)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ru='Не заполнен адрес электронной почты';en='Incomplete email address'"),,
...
Процедура ПрочитатьПользователяИБ(ПриКопированииЭлемента = Ложь)
	...
	Иначе
		...
		Если ПрочитанныеСвойства = Неопределено Тогда
			УстановитьНаличиеПароля(ЭтотОбъект, Ложь, Пользователи.АвторизованныйПользователь());
		Иначе
			УстановитьНаличиеПароля(ЭтотОбъект, ПрочитанныеСвойства.ПарольУстановлен, Пользователи.АвторизованныйПользователь());
		КонецЕсли;
	КонецЕсли;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
		...
		Если УточнениеСвойств.Количество() > 0 Тогда
			...
			Если ПоказатьКомандыУстраненияРазличий Тогда
				Рекомендация = Символы.ПС
					+ НСтр("ru='Нажмите ""Записать"", чтобы устранить различия и не выводить это предупреждение.';en='Click ""Write"" to resolve the differences and not to show this message again.'");
			
			ИначеЕсли Не Пользователи.ЭтоПолноправныйПользователь() Тогда
				Рекомендация = Символы.ПС
					+ НСтр("ru='Обратитесь к администратору, чтобы устранить различия.';en='Please contact your administrator to resolve the differences.'");
			Иначе
				Рекомендация = "";
			КонецЕсли;
	...
	Если ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
		Рекомендация = Символы.ПС
			+ НСтр("ru='Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.';en='Click ""Write"" to resolve this issue and not to show this message again.'");
	...
	Элементы.СопоставлениеПояснениеНесоответствия.Заголовок =
		НСтр("ru='Пользователь информационной базы не найден.';en='The user of the information database was not found.'") + Рекомендация;
