...
Процедура НастроитьОтображениеДатыАктуальности()
	...
	Если ЗначениеЗаполнено(Объект.CRM_ДатаАктуальности) И (Объект.CRM_ДатаАктуальности>Объект.ДатаОкончания) Тогда
		...
		пЗаголовок = НСтр("ru = 'дней '") + НСтр("ru='( до ';en='( till'") + Формат(Объект.CRM_ДатаАктуальности,"ДЛФ=DD")+")";
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.CRM_СквознаяАналитика И Объект.CRM_Неограничено Тогда
	...
	Если НЕ Объект.CRM_СквознаяАналитика Тогда
		...
		Для Каждого СтрокаТаблицы Из Объект.CRM_СвоиУчастники Цикл
			...
			Если	(ЗначениеЗаполнено(СтрокаТаблицы.ДатаНачала) И Не ЗначениеЗаполнено(СтрокаТаблицы.ДатаОкончания))
				...
				+ НСтр("ru='Строка';en='Строка'") + " " + СтрНомерСтроки + ": "
...
&НаКлиенте
Процедура ДекорацияСтартоватьМероприятиеНажатие(Элемент)
...
&НаКлиенте
Процедура ДекорацияОстановитьРаспознаваниеНажатие(Элемент)
...
Процедура НеограниченоПриИзменении(Элемент)
	...
	Если Объект.CRM_Неограничено Тогда
...
Процедура СформироватьПредставлениеВложений()
	...
	Если Объект.Ссылка.Пустая() Тогда
		ВложенияПредставление = НСтр("ru='Файлы(0)';en='Fales(0)'");
	...
	УчасникиКоличество = Объект.ПартнерыИКонтактныеЛица.Количество()+Объект.CRM_СвоиУчастники.Количество();
	Элементы.ОткрытьУчастники.Заголовок = НСтр("ru='Участники('")+УчасникиКоличество+")";
	//НСтр("ru='Участники('"+()+")");
...
Процедура ОткрытьУчастникиЗавершение(ПродолжитьВыполнение, ПараметрыВыполнения) Экспорт
	...
	Если ПродолжитьВыполнение Тогда
		...
		ОткрытьФорму("Справочник.МаркетинговыеМероприятия.Форма.CRM_ФормаУчастники",ПараметрыФормы, ЭтотОбъект,,,,Новый ОписаниеОповещения("УчасникиЗавершениеЗаполнения", ЭтотОбъект),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	
КонецПроцедуры //ОткрытьУчастникиЗавершение()

&НаСервере
Процедура УчасникиЗавершениеЗаполнения(Результат, Контекст)
