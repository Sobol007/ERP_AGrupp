...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.ТекстЗапроса = СтрЗаменить(Список.ТекстЗапроса, "СправочникПартнеры.Ссылка,", "СправочникПартнеры.Ссылка,
												|    ВЫБОР
												|        КОГДА (СправочникПартнеры.CRM_ПроцентЗаполненностиКИ + СправочникПартнеры.CRM_ПроцентЗаполненностиПортрет) / 2 < 25
												|            ТОГДА 0
												|        КОГДА (СправочникПартнеры.CRM_ПроцентЗаполненностиКИ + СправочникПартнеры.CRM_ПроцентЗаполненностиПортрет) / 2 < 50
												|            ТОГДА 1
											 	|       КОГДА (СправочникПартнеры.CRM_ПроцентЗаполненностиКИ + СправочникПартнеры.CRM_ПроцентЗаполненностиПортрет) / 2 < 75
												|          ТОГДА 2
												|        КОГДА (СправочникПартнеры.CRM_ПроцентЗаполненностиКИ + СправочникПартнеры.CRM_ПроцентЗаполненностиПортрет) / 2 < 90
												|            ТОГДА 3
												|        ИНАЧЕ 4
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	//Если Настройки["ПометкаПоказатьСкрытьДопИнформацию"] = Неопределено Тогда
	//	ПометкаПоказатьСкрытьДопИнформацию = Истина;
	//Иначе
	//	ПометкаПоказатьСкрытьДопИнформацию = Настройки["ПометкаПоказатьСкрытьДопИнформацию"];
	//КонецЕсли;
	//
	...
	//Элементы.ГруппаПанельФильтры.Видимость			= ПометкаПоказатьСкрытьДопИнформацию;
	//
	//Если Настройки["ПометкаПоказатьСкрытьСодержание"] = Неопределено Тогда
	//	ПометкаПоказатьСкрытьСодержание = Истина;
	//Иначе
	//	ПометкаПоказатьСкрытьСодержание = Настройки["ПометкаПоказатьСкрытьСодержание"];
	//КонецЕсли;
	//
...
Процедура ИнтересыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если НЕ Копирование Тогда
		...
		Если ТипЗнч(ТекущийКлиент) = Тип("СправочникСсылка.Партнеры") Тогда
			...
			ОткрытьФорму("Документ.CRM_Интерес.ФормаОбъекта",СтруктураЗаполнения,Элемент,УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
