...
Функция ПолучитьПредставлениеТочкиМаршрута(ТочкаМаршрута)
	Если (ТипЗнч(ТочкаМаршрута) = Тип("СправочникСсылка.CRM_ТочкиМаршрутов")) 
		...
		Возврат НСтр("ru='Событие';en='Event'");
	Иначе
...
Процедура ДобавитьЭтапЗавершение(Этап, ДополнительныеПараметры) Экспорт
	Если Этап = Неопределено Тогда Возврат; КонецЕсли;
		...
		Если ДополнительныеПараметры = "СостояниеИнтереса" Тогда
			НоваяСтрока.КартаМаршрута = CRM_ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(Этап, "Родитель");
		...
		ИначеЕсли ДополнительныеПараметры = "Документ" Тогда
			...
			Иначе
				НоваяСтрока.КартаМаршрута = НСтр("ru='Документ';en='Document'");
				НоваяСтрока.ТочкаМаршрута = Этап;
				НоваяСтрока.ПредставлениеТочкиМаршрута = ПолучитьПредставлениеТочкиМаршрута(Этап);
			КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Объект.Ссылка.Пустая() Тогда
		Объект.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоСостояниямИнтересов;
	КонецЕсли;
	ВидВоронкиПриИзмененииСервер();
	...
	ТолькоПросмотр = ЗначениеЗаполнено(Объект.Родитель.СценарийИнтереса);
...
&НаСервере
Процедура ВидВоронкиПриИзмененииСервер()
	Если Объект.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоСостояниямИнтересов Тогда
		Элементы.СоставКартаМаршрута.Заголовок = НСтр("ru='Сценарий';en='Script'");
		Элементы.СоставПредставлениеТочкиМаршрута.Заголовок = НСтр("ru='Состояние';en='State'");
		ПараметрыВыбораВида = Новый Массив;
		ПараметрыВыбораВида.Добавить(Новый ПараметрВыбора("Отбор.Родитель", Справочники.CRM_СостоянияИнтересов.ПустаяСсылка()));
		Элементы.СоставКартаМаршрута.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораВида);
	ИначеЕсли Объект.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоЭтапам Тогда
		Элементы.СоставКартаМаршрута.Заголовок = НСтр("ru='Карта маршрута';en='Keywords:'");
		Элементы.СоставПредставлениеТочкиМаршрута.Заголовок = НСтр("ru='Точка маршрута';en='Route point'");
		ПараметрыВыбораВида = Новый Массив;
		Элементы.СоставКартаМаршрута.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораВида);
	ИначеЕсли Объект.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоДокументам Тогда
		Элементы.СоставКартаМаршрута.Заголовок = НСтр("ru='Тип';en='Type'");
		Элементы.СоставПредставлениеТочкиМаршрута.Заголовок = НСтр("ru='Объект';en='Object'");
		ПараметрыВыбораВида = Новый Массив;
		Элементы.СоставКартаМаршрута.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораВида);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ВидВоронкиПриИзменении(Элемент)
	ВидВоронкиПриИзмененииСервер();
КонецПроцедуры
