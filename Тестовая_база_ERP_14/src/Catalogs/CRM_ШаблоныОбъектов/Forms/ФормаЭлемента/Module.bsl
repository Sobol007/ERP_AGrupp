...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ЕстьПометки(СписокРеквизитов) Тогда
		...
		Сообщение.Текст = Нстр("ru='Необходимо отметить хотя бы один реквизит!';en='It is necessary to mark at least the one detail!'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДеревоРеквизитовЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если (ТекущиеДанные.ТипРеквизита.Типы().Количество() = 1) И (ТекущиеДанные.ТипРеквизита.Типы()[0] = Тип("Строка")) Тогда
		...
		ПоказатьВводСтроки(Оповещение,ТекущиеДанные.Значение, Нстр("ru='Установка значения реквизита';en='Setting the value of the props'"), 0, Истина);
	КонецЕсли;		
