...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если Объект.Подключение.СерверMSExchange <> Объект.УчетнаяЗапись.СерверMSExchange Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Реквизит ""Сервер MS Exchange"" в подключении отличается от значения, указанного в учетной записи!';en='The detail of ""server MS Exchange"" in connection differs from the value specified in the login account!'"),, "Объект.Подключение");
		Возврат;
	КонецЕсли;
...
Процедура ВыборПапкиMSExchange(Элемент)
	Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбрана учетная запись!';en='Login account is not selected!'"));
		Возврат;
	КонецЕсли;
...
Процедура ВопросЗагрузитьСтруктураПапокЗавершение(Ответ, Элемент) Экспорт
	...
	Если Ответ <> Неопределено Тогда
		Если Ответ = КодВозвратаДиалога.Да Тогда
			...
			Состояние(НСтр("ru='Выполняется загрузка структуры папок MS Exchange...';en='Loading of structure of folders of MS Exchange are fulfill...'"));
...
Процедура Далее(Команда)
	...
	Иначе
		...
		Если ЭтоПоследняяСтраница() Тогда
			Элементы.Далее.Заголовок = НСтр("ru='Завершить';en='Complete'");
...
Процедура Назад(Команда)
	...
	Если Элементы.Далее.Заголовок <> НСтр("ru='Далее';en='Further'") Тогда
		Элементы.Далее.Заголовок = НСтр("ru='Далее';en='Further'");
...
Процедура ПроверитьПодключение(Команда)
	Если Не ЗначениеЗаполнено(Объект.Подключение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрано подключение!';en='Connection are not selected!'"),, "Объект.Подключение");
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрана учетная запись!';en='Login account is not selected!'"),, "Объект.УчетнаяЗапись");
		Возврат;
	КонецЕсли;
...
Функция СформироватьТекстЗапросаСобытия()
	...
	|ИЗ
	|	Документ.удалитьCRM_Мероприятие КАК Мероприятия
	|";
...
Процедура СкопироватьОтборКомпоновкиДанных(ЭлементыПриемник, ЭлементыИсточник, ДоступныеПоляОтбора = Неопределено)
