...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ДеревоШаблонаАнкеты.Строки[0].Строки.Найти("","Формулировка",Истина) <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не все формулировки или имена разделов заполнены.';en='Not all statements or names of sections was fill.'"),,"ДеревоАнкеты");
		Отказ = Истина;
	КонецЕсли;
	...
	Если НайденныеСтроки.Количество() <> 0 Тогда
		Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если НайденнаяСтрока.ТипВопроса <> Перечисления.ТипыВопросовШаблонаАнкеты.Табличный
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не все вопросы заполнены.';en='Not all questions was fill.'"),,"ДеревоАнкеты");
				Отказ = Истина;
				Прервать;
...
Процедура ФормаДереваАнкетыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	СписокВыбора.Добавить(НСтр("ru='Раздел';en='Section'"));
	СписокВыбора.Добавить(НСтр("ru='Простой вопрос';en='simple question'"));
	СписокВыбора.Добавить(НСтр("ru='Комплексный вопрос';en='complex question'"));
	СписокВыбора.Добавить(НСтр("ru='Условный вопрос';en='Conditional question'"));
	СписокВыбора.Добавить(НСтр("ru='Табличный вопрос';en='table question'"));
	
	ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ВыборТипаДобавляемогоЭлементаПриЗавершении", ЭтотОбъект);
	СписокВыбора.ПоказатьВыборЭлемента(ОбработчикОповещенияОЗакрытии, НСтр("ru='Выберите тип добавляемого элемента.';en='Select the type of the item to add.'"),СписокВыбора[0]);
...
Процедура ДеревоАнкетыЗаметкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(ОповещениеОЗакрытии, Элемент.ТекстРедактирования, НСтр("ru='Заметки';en='Notes'"));
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='Вопрос';en='Query'");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДеревоАнкеты.ТипСтроки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='Корень';en='Root'");
...
Процедура ДобавитьВопрос(ТекущиеДанные,ТипВопроса)
	...
	Если Родитель.ТипСтроки = "Корень" Тогда
		ПоказатьПредупреждение(,НСтр("ru='Вопросы нельзя добавлять в корень анкеты.';en='Questions cannot be add in a root of the questionnaire.'"),15,НСтр("ru='Ошибка добавления';en='Error of adding'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыборТипаДобавляемогоЭлементаПриЗавершении(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыбранныйЭлемент = Неопределено Тогда
		...
		Если ВыбранныйЭлемент.Значение = НСтр("ru='Раздел';en='Section'") Тогда
			
			ДобавитьРаздел(Команды.ДобавитьРаздел);
			
		ИначеЕсли ВыбранныйЭлемент.Значение = НСтр("ru='Простой вопрос';en='simple question'") Тогда
			
			ДобавитьПростойВопрос(Команды.ДобавитьПростойВопрос)
			
		ИначеЕсли ВыбранныйЭлемент.Значение = НСтр("ru='Комплексный вопрос';en='complex question'") Тогда
			
			ДобавитьКомплексныйВопрос(Команды.ДобавитьПростойВопрос)
			
		ИначеЕсли ВыбранныйЭлемент.Значение = НСтр("ru='Условный вопрос';en='Conditional question'") Тогда
			
			ДобавитьВопросСУсловием(Команды.ДобавитьВопросСУсловием)
			
		ИначеЕсли ВыбранныйЭлемент.Значение = НСтр("ru='Табличный вопрос';en='table question'") Тогда
