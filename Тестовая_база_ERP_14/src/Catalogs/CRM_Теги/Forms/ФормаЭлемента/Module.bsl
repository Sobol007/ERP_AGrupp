...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЕстьРоль И НЕ РуководительПодразделения Тогда
		ТекстСообщения = НСтр("ru='Нет права на добавление и изменение тегов подразделения';en='There is no right to add and edit department tags'") + " <" + Объект.Владелец + ">.";
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЕстьДублиТегов.ЕстьДубли Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Создаваемый тег уже существует у подразделения';en='The created tag already exists for the unit'") + " <" + ЕстьДублиТегов.Подразделение + ">.";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли; 
КонецПроцедуры
